...
function Util.digestFile(filename, dtype, rawOutput)
	local digest = CCCrypto:createDigest(dtype)
	if digest == nil then
		return ""
	end
	...
	file:close()
	return digest:getDigest(rawOutput)
end
...
function Util.dirname(file)
	...
	if index ~= nil then
		return file:sub(0, file:len() - index)
	else
		return file
	end
end
...
-- 递归的创建一个目录, zhangqi, 20140617
function Util.mkDirRecursive( tdir )
	local noExist = {}

	local dir = tdir
	if (tdir:sub(-1, -1) == "/") then
		dir = tdir:sub(0, -2)
	end
		
	while (GameUtil:access(dir,GameUtil.EFOk) ~= 0) do
		Logger.debug("dir = %s", dir)
		table.insert(noExist, dir)
		dir = Util.dirname(dir)
	end

	for i = #noExist, 1, -1 do
		Logger.debug("%d = %s", i, noExist[i])
		if (GameUtil:mkdir(noExist[i],Util.octal2Decimal("0700")) ~= 0) then
			Logger.warning("mkdir:%s failed", noExist[i])
			return false
		end
		Logger.debug("mkdir:%s ok", noExist[i])
	end
	return true
end
...
function Util.copyDir(sdir, tdir, mv)
	...
	if not GameUtil:isDir(sdir) then
		if mv then
			local ret = GameUtil:rename(sdir,ddir)
			if ret ~= 0 then
				Logger.warning("rename file:%s to %s failed", sdir, ddir)
			end

			return ret == 0
		else
			local sfile = io.open(sdir,"r")
			local tfile = io.open(ddir,"w")
			if sfile == nil or tfile == nil then
				Logger.warning("open file:[%s, %s] failed", sdir, ddir)
				return false
			end
			while true do
		...
		if 0 ~= GameUtil:access(ddir,GameUtil.EFOk) then
			local ret = Util.mkDirRecursive(ddir)
			if (not ret) then
				return false
			end
		end
...
---
-- 用于模拟多继承的情况，将一个父类的所有属性拷贝给子类
-- @function [parent=#Util] extends
-- @param child
-- @param parent
function Util.extends(child, parent)
	if type(parent) ~= "table" then
		return
	end

	for k, v in pairs(parent) do
		if k ~= "class" and child[k] == nil then
			child[k] = v
		end
	end

	Util.extends(child, parent.class)
end
...
---
-- zhangqi, 删除本地缓存的资源更新目录，便于测试
-- @function [parent=#Util] removeCacheDir
-- @param
-- @return #string
function Util.removeCacheDir( ... )
	local resPath = string.format("%s%s/%s", g_ResPath, g_ProjName, g_ResRoot)
	Logger.debug("removeCacheDir:" .. resPath)
	Util.removeDir(resPath)
end
