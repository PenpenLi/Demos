...
function Downloader:doHeader()
	...
	logger:debug("doHeader:status code:" .. tostring(status))
	...
	-- zhangqi, 2015-12-25, 参考部落下载更新包出现的301或302状态但更新包还可以正常下载的问题
	-- 把299以上状态码作为出错处理修改为303以上
	-- if tonumber(status) > 299 then
	if tonumber(status) > 303 then
		self:dispatch({
			type=kETFinished,
			err=message
		})
...
function Downloader:onBodyData(data)
	...
	if self.mHeaderOk then
		...
		if self.mNowBytes == self.mRangeSize then
			self.mFile:flush()
			self.mFile:close()
			self.mFile = nil -- zhangqi, 2016-01-19, 文件句柄close后置为nil，以备后续的处理检测其有效性
...
function Downloader:onFinished(client, response)
	logger:debug("onFinished:status code:" .. response:getResponseCode())
	...
	if response:getResponseCode() > 299 or response:getResponseCode() < 0 then
		-- zhangqi, 2016-01-19, 如果响应出错将状态码和错误信息发送到线上错误日志
		loggerHttp:fatal("download failed: code:%s, error:%s",
				tostring(response:getResponseCode()), response:getErrorBuffer())

		Logger.fatal("download failed:%s", response:getErrorBuffer())
		-- zhangqi, 2016-01-19, self.mFile可能会在onBodyData中被close后置为nil，需要先检测其有效性再处理
		if (self.mFile) then
			self.mFile:flush()
			self.mFile:close()
		end
