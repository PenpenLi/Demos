...
m_bKickOff = false -- zhangqi, 2015-12-23, 是否后端主动断开连接，会在推送中置为true
...
-- 2015-11-24, 通过设置控制是否显示loading, 避免没有返回的请求加了loading要等超时才能移除
function setShowLoading( bShow )
...
function connect (phost, pport)
	...
	m_bKickOff = false -- zhangqi, 2015-12-23
...
function lastRpcBack( ... )
	logger:debug({Network_lastRpcBack_m_tbRpcBack = m_tbRpcBack})
...
function rpc(cbFunc, cbFlag, rpcName, args, autoRelease)
	...
	if (not m_bShowLoading) then
		LayerManager.addLoading(nil, true) -- zhangqi, 2014-05-12, 显示网络请求Loading
		m_bShowLoading = true
	end
end
-- 网络统一接口
function networkHandler(cbFlag, dictData, bRet)
	...
	if (not bRet) then
		if (g_debug_mode) then -- 调试模式先调错误页面
