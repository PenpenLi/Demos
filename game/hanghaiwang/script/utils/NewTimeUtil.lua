--Filename: NewTimeUtil.lua
--Author：zhangqi
--Date：2015-01-22
--Purpose: 日期时间转换相关的公共方法

module("NewTimeUtil",package.seeall)
...
function getSvrTimeByOffset( second_num )
    local curServerTime = BTUtil:getSvrTimeInterval() -- 当前服务器时间
...
function expireTimeString( nGenTime, nDuration )
    local _, svrNow = getServerDateTime()
    --logger:debug("nGenTime = " .. nGenTime .. " svrNow = " .. svrNow)
    local nViewSec = (nDuration or 0) - (svrNow - nGenTime)
...
function getDifferDay(timeInt, offset)
    ...
    if(tonumber(os.date("%j",curTime)) == 1 and tonumber(os.date("%j",timeInt - offset)) ~= 1)then
    	return tonumber(os.date("%j",curTime)) - (tonumber(os.date("%j",timeInt - offset)) - tonumber(os.date("%j",curTime-24*60*60)))
    else--if(os.date("%j",curTime) ~= os.date("%j",timeInt - offset))then
    	return tonumber(os.date("%j",curTime)) - tonumber(os.date("%j",timeInt - offset))
    end
end

-- 指定一个日期时间字符串，返回与之对应的服务器时区时间戳, zhangqi, 2015-01-20
-- sTime: 格式 "2013-07-02 20:00:00"
function getIntervalByTimeString( sTime )
	local t = string.strsplit(sTime, " ")
	local tDate = string.strsplit(t[1], "-")
	local tTime = string.strsplit(t[2], ":")
...
-- 给一个时间如:153000,得到相对于服务器时区的当天 15:30:00 的时间字符串
...
-- m_time：时间戳  return：时间格式：2014-06-01 01:01:01
...
-- para：时间戳  return：时间格式：2014-06-01 01:01
function getTimeFormatYMDHMdot( m_time )
	return getTimeStringWithFormat(m_time, "%Y.%m.%d %H:%M")
end
