...
local energy_restore_full_body = gi18n[5712]
...
function addRestoreEnergyNotification()
	...
	if(rest_time <= 0)then
		...
		local fireTimeInterval = TimeUtil.getSvrTimeByOffset()+rest_time
...
local body_longTimeNoSee = gi18n[5713]

-- 长时间未登录通知
function addLongTimeNoSeeNotification()
	local longTime = 3600*24*3
	local fireTimeInterval = TimeUtil.getSvrTimeByOffset()+longTime
...
local body_worldBossStart = gi18n[5714]
...
-- 世界boss即海王类开启推送
-- 2015-12-7 add by lvnanchun
function addWorldBossStartNotification( )
	WorldBossModel.init()
	local startTimeInterval = WorldBossModel.getBeginTime()
	local curTimeInterval = TimeUtil.getSvrTimeByOffset()
	local fireTimeInterval = 0
	if (curTimeInterval>=startTimeInterval) then
	 	fireTimeInterval = startTimeInterval + 3600 * 24
	...
	logger:debug({fireTimeInterval = fireTimeInterval})
	NotificationManager:addLocalNotificationBy(kWorldBossStart_key, body_worldBossStart, fireTimeInterval, kCFCalendarUnitDay_BT)
end
