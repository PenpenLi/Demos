...
local function initPlayerCards()
    ...
    for i=0,5 do
        ...
        cardBg:setAnchorPoint(CCP_HALF)
        ...
        if(m_formation["" .. i]~=nil and (m_formation["" .. i])~=0) then
            --local card = CCXMLSprite:create(IMG_PATH .. "card/card_" .. (i%2+1) .. ".png")
            --card:retain()
            --card:initXMLSprite(CCString:create(IMG_PATH .. "card/card_" .. (i%2+1) .. ".png"));
            --card:setAnchorPoint(CCP_HALF)
            ----print("------------------ BattleCardUtil.getBattlePlayerCard:",m_formation["" .. i])
            --local card = BattleCardUtil.getBattlePlayerCard(m_formation["" .. i])
            local card = BattleLayer.createBattleCard(m_formation["" .. i])
            card:setTag(1000+i)
            m_playerCardLayer:addChild(card,2)
    ...
    for i=0,5 do
        ...
        if(cardBg:getTag()-1000==m_onMovingCard:getTag()) then
            ...
            if(location.x>=0 and location.y>=0 and location.x<=cardBg:getContentSize().width and location.y<=cardBg:getContentSize().height) then
                ...
                require "script/module/formation/FormationUtil"  -- yangna  2015.1.27 删除script/ui ,
                ...
                if(FormationUtil.isOpenedByPosition(index2)==false or FormationUtil.isOpenedByPosition(index1)==false) then
                    isSwitched = false
                    require "script/module/public/AnimationTip"
                    AnimationTip.showTip( "该位置尚未开启...")
                    break
                end
...
function setSwitchable(switchable)
    ...
    require "script/module/formation/FormationUtil"    --2015.1.27  yangna 删除script/ui
