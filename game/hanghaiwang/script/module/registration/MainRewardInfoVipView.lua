-- FileName: MainRewardInfoVipView.lua
-- Author:  wangming
-- Date: 2014-12-25
-- Purpose: 签到物品界面展示(vip完整领取)
--[[TODO List]]

module("MainRewardInfoVipView", package.seeall)
...
require "db/i18n"
 -- 资源文件
local reward_sign = "ui/sign_reward_vip.json" 
...
function destroy(...)
    package.loaded["MainRewardInfoVipView"] = nil
end

function moduleName()
    return "MainRewardInfoVipView"
end
...
function createView(color,typeName,reward_des,time,des,bget,alreadySign,goodNum,imagePath,imageIcon,goodType,goodid,info,vipLevel,beishu,rewardid)
    ...
    local layCenterVip = m_fnGetWidget(layBg,"IMG_ITEM_BG_VIP")
    ...
    initListView(layCenter, layBg , sign_reward , false)

    initListView(layCenterVip, layBg , sign_reward , true)
...
function setBaseInfoOnView( layBg , goodNum , info ,goodid ,goodType,reward_des,des,qualitys , isVip)
...
-- 我日所有奖励都没乘倍数 ？？？？  add by yn 2015.7.23
function changeSignResult( goodType ,infoResult)
    ...
    elseif tonumber(goodType) == 1 then 
        ...
        UserModel.addSilverNumber( tonumber(infoResult)*nNum)
    ...
    elseif  tonumber(goodType) == 3 then 
        ...
        UserModel.addGoldNumber( tonumber(infoResult)*nNum)
        getTip = m_i18n[2632] .. "[" ..infoResult .. reward_dess .. "]" .."×" .. nNum  
    elseif tonumber(goodType) == 5 then 
        if m_CurBei~= nil and tonumber(m_CurVip)>= tonumber(m_vipLevel) then 
            nNum = nNum * m_CurBei
        end
        UserModel.addStaminaMaxNumber(  tonumber(infoResult)*nNum  )
        getTip = m_i18n[2632] .. "[" ..infoResult .. reward_dess .. "]" .."×" .. nNum  
    elseif tonumber(goodType) == 4 then 
        if m_CurBei~= nil and tonumber(m_CurVip)>= tonumber(m_vipLevel) then 
            nNum = nNum * m_CurBei
        end
        UserModel.addEnergyValue(  tonumber(infoResult)*nNum   )
        getTip = m_i18n[2632] .. "[" ..infoResult .. reward_dess .. "]" .."×" .. nNum  
...
function initListView( layBg , mainBack,sign_reward , isVip)
    ...
    local   main_back                          =  m_fnGetWidget(mainBack,"img_bg") 
    local   mlistViewBack = layBg
    -- local   layout                             =  Layout:create()
    ...
    local itm = nil
    if(isVip) then
        itm =  m_fnGetWidget(main_back,"img_item_bg")
    else
        itm =  m_fnGetWidget(main_back,"IMG_ITEM_BG_VIP")
    end
    cells = m_fnGetWidget(itm,"img_cell_bg")
    ...
    for i=1,#daysRewardArr do
        ...
        -- item = 
        setBaseInfoOnView( cells , goodNum , daysRewardArr[i] ,array[1] ,daysRewardType[i],daysRewardName[i],mian_desc,daysRewardQuality[i] , isVip)
