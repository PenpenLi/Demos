-- FileName: WABattleLoseCtrl.lua
-- Author: Xufei
-- Date: 2015-02-19
-- Purpose: 海盗激斗 失败结算
--[[TODO List]]

module("WABattleLoseCtrl", package.seeall)
...
function getBtnFunByName( funName )
	...
	btnEvent.onClose = function ( sender, eventType )
		if (eventType == TOUCH_EVENT_ENDED) then
			...
			logger:debug("Lose_WA_Battle_onClose")
	...
	btnEvent.onFormation = function ( sender, eventType )
		if (eventType == TOUCH_EVENT_ENDED) then
			AudioHelper.playCommonEffect()
			logger:debug("Lose_WA_Battle_onFormation")
	...
	btnEvent.onReplay = function ( sender, eventType )
		if (eventType == TOUCH_EVENT_ENDED) then
			AudioHelper.playCommonEffect()
			logger:debug("Lose_WA_Battle_onReplay")
	...
	btnEvent.onSeeData = function ( sender, eventType )
		if (eventType == TOUCH_EVENT_ENDED) then
			AudioHelper.playCommonEffect()
			logger:debug("Lose_WA_Battle_onSeeData")
...
function destroy(...)
	package.loaded["WABattleLoseCtrl"] = nil
end

function moduleName()
    return "WABattleLoseCtrl"
end
...
function create( tbArgs, damageMap, isPassFight )
	...
	logger:debug({wabattlelose_damagemap = damageMap})

	_damageMap = damageMap

	local instanceView = WABattleLoseView:new()
