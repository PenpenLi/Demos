-- FileName: GuideImpelDownView.lua
-- Author: huxiaozhou
-- Date: 2015-12-09
-- Purpose: 深海监狱新手引导
-- ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
module("GuideImpelDownView", package.seeall)
...
function destroy(...)
	package.loaded["GuideImpelDownView"] = nil
end

function moduleName()
    return "GuideImpelDownView"
end

function create(step, opacity, pos, callfunc)
	guideStep = step
	local json = "ui/new_prison.json"
	...
	logger:debug("LAY_PRISON_STEP" .. step)
	logger:debug("count".. count)

	for i=1,count do
		local layKeyName = "LAY_PRISON_STEP" .. i
