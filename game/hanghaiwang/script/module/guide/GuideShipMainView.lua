-- FileName: GuideShipMainView.lua
-- Author: huxiaozhou
-- Date: 2015-12-08
-- Purpose: 主船系统新手引导
-- ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
module("GuideShipMainView", package.seeall)
...
function destroy(...)
	package.loaded["GuideShipMainView"] = nil
end

function moduleName()
    return "GuideShipMainView"
end


function create(step, opacity, pos, callfunc)
	guideStep = step
	local json = "ui/new_ship.json"
	...
	logger:debug("LAY_SHIP_STEP" .. step)
	logger:debug("count".. count)

	for i=1,count do
		local layKeyName = "LAY_SHIP_STEP" .. i
