-- FileName: GuideFiveLevelGiftView.lua
-- Author: huxiaohzou
-- Date: 2014-06-23
...
module("GuideFiveLevelGiftView", package.seeall)
...
function destroy(...)
	package.loaded["GuideFiveLevelGiftView"] = nil
end

function moduleName()
    return "GuideFiveLevelGiftView"
end

-- modified by zhangqi, 2014-07-15, 添加参数 pos, 副本引导中会传入当前据点的坐标
function create(step, opacity, pos, callfunc)
	guideStep = step
	local json = "ui/new_level_reward.json"
	...
	local LAY_NOMASK1
	logger:debug("LAY_LEVEL_STEP" .. step)
	logger:debug("count".. count)
	
	local worldPosTemp = nil
	local rectTemp1 = nil
	for i=1,count do
		local layKeyName = "LAY_LEVEL_STEP" .. i
		...
		if(tonumber(step) == i) then
			...
			if i==6 then
				...
				local lay_step2_1 = m_fnGetWidget(lay_formation_step, "lay_step6_1")
				local lay_step2_2 = m_fnGetWidget(lay_formation_step, "lay_step6_2")
				local lay_step2_3 = m_fnGetWidget(lay_formation_step, "lay_step6_3")
