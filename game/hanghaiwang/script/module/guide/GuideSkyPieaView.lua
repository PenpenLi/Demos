...
module("GuideSkyPieaView", package.seeall)
...
function destroy(...)
	package.loaded["GuideSkyPieaView"] = nil
end

function moduleName()
    return "GuideSkyPieaView"
end

function create(step,opacity,touchCallback)
	guideStep = step
	local json = "ui/new_air.json"
	...
	local rectTemp1 = nil
	for i=1,count do
		local layKeyName = "LAY_AIR_STEP" .. i
