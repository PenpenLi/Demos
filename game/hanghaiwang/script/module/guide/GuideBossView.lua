...
module("GuideBossView", package.seeall)
...
function destroy(...)
	package.loaded["GuideBossView"] = nil
end

function moduleName()
    return "GuideBossView"
end

function create(step, opacity, pos, callfunc)
	guideStep = step
	local json = "ui/new_boss.json"
	...
	logger:debug("LAY_BOSS_STEP" .. step)
	logger:debug("count".. count)

	for i=1,count do
		local layKeyName = "LAY_BOSS_STEP" .. i
