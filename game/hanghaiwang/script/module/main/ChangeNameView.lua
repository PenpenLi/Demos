...
function ChangeNameView:create( tbData )
	...
	tbArgs = { layRoot = self.layMain, bgName = "IMG_NAME_BG", sHolder = m_i18n[1915], holderColor = ccc3(0xba, 0x9d, 0x4f),
				FontName = g_sFontName, FontSize = 28, FontColor = ccc3(0x57, 0x1e, 0x01), maxLen = self.MAX_INPUT}
				...
				RequestCenter.user_getRandomName(function ( cbFlag, dictData, bRet )
					if (bRet) then
						...
						if(table.isEmpty(self.m_tbNames)) then
							ShowNotice.showShellInfo(m_i18n[1917]) -- 随机名用完
							return
						end
						self.m_nCurIndex = 1
