...
local m_pathRootImg = "images/fight/"
 -- m_tbGenData, 以提升类型(当前9种)为 index 保存计算每种提升类型当前值和上限值的function，还包括 "前往" 按钮的回调方法
local m_tbGenData = {{}, {}, {}, {}, {}, {}, {}, {}, {},}
...
local m_SQUAD_MAX = MainFormationTools.fnGetSquadNum() -- 阵容最大上阵数
local m_NO_ZERO = 0.1 -- 某个属性上限（max）值为0时为了获得0%的结果，必须指定其为一个不为0的数字，2015-06-08
local m_HERO_MAX = 6 -- 阵上伙伴最大数量
local m_TREASNUM_MAX = 3 -- 每个伙伴可穿戴饰品最大数量，2016-01-09
local m_EQUIPNUM_MAX = 4 -- 每个伙伴可穿戴装备最大数量，2016-01-09
...
function getRaiseLists( ... )
	...
	for i, item in ipairs(m_tbGenData) do
		...
		local data = {id = i, icon_path = m_pathRootImg .. db.icon, recommend = db.index}
		-- data.title_path = m_pathRootImg .. db.title -- 2015-11-25
		data.curNum = m_tbGenData[i].cur()
		data.maxNum = m_tbGenData[i].max()
		data.fnBtnCallback = m_tbGenData[i].onClick

		data.typeTitle = db.name -- 2015-11-25
		...
		logger:debug("0 id = %d, index = %d, curNum = %d, maxNum = %d, percent = %d%%", db.id, db.index, data.curNum, data.maxNum, percent)

		if (percent == 100) then -- 完美无瑕
			if (data.maxNum >= 0) then -- maxNum < 0 表示功能未开启，需隐藏，所以只处理 >= 0 的情况
				logger:debug("3 id = %d, index = %d, curNum = %d, maxNum = %d, percent = %d%%", db.id, db.index, data.curNum, data.maxNum, percent)
				table.insert(tbLists[3], data)
			end
		elseif (percent >= tonumber(db.percentage) ) then -- 有待提高
			logger:debug("2 id = %d, index = %d, curNum = %d, maxNum = %d, percent = %d%%", db.id, db.index, data.curNum, data.maxNum, percent)
			table.insert(tbLists[2], data)
		else
			logger:debug("1 id = %d, index = %d, curNum = %d, maxNum = %d, percent = %d%%", db.id, db.index, data.curNum, data.maxNum, percent)
...
m_tbGenData[3].onClick = function ( ... )
	...
	local pageNum = 0 -- zhangqi, 2015-05-19, 默认显示阵容的第一个伙伴
...
m_tbGenData[4].max = function ( ... )
	...
	return (m_heroSquad + m_heroBench) * m_EQUIPNUM_MAX
end
m_tbGenData[4].onClick = function ( ... )
	gotoFormation(0)
end
...
m_tbGenData[5].max = function ( ... )
	...
	local num = m_equipNum * (m_usrLv * 2)
	return num == 0 and m_NO_ZERO or num -- 装备强化等级上限为: 玩家等级*2
end
...
m_tbGenData[6].max = function ( ... )
	...
	return m_equipMagicMaxLv == 0 and m_NO_ZERO or m_equipMagicMaxLv 
end
...
m_tbGenData[7].max = function ( ... )
	...
	return (m_heroSquad + m_heroBench) * m_TREASNUM_MAX
end
...
m_tbGenData[8].max = function ( ... )
	if(not SwitchModel.getSwitchOpenState(ksSwitchTreasureForge)) then
		return -1 -- 如果宝物强化未开启则返回0
	end

	return m_treasStrenthMaxLv == 0 and m_NO_ZERO or m_treasStrenthMaxLv
end
...
m_tbGenData[9].max = function ( ... )
	if(not SwitchModel.getSwitchOpenState(ksSwitchTreasureFixed)) then
		return -1 -- 如果宝物精炼未开启则返回0
	end

	return m_treasEvolveMaxLv == 0 and m_NO_ZERO or m_treasEvolveMaxLv
end
