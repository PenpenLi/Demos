...
-- Date: 2015-01-00
-- Purpose: 提升战斗力的UI实现
-- modified: zhangqi, 2015-11-25, 部分正式UI修改
--[[TODO List]]
...
local c3Selected = ccc3(0xff,0xff,0xff)
local c3Normal = ccc3(0xbf,0x93,0x67)
local c3Text = ccc3(0x28,0x00,0x00)
local c3TypeTitle = ccc3(0x5e,0x31,0x00)
local c3Percent = ccc3(0x8e,0x46,0x00)
...
function RaiseFightView:create( tbData )
	...
	local labFightNum = m_fnGetWidget(layRoot, "TFD_FIGHT") -- 2015-11-25
	UIHelper.labelAddNewStroke(labFightNum, UserModel.getFightForceValue(), c3Text)

	local i18n_fight = m_fnGetWidget(layRoot, "tfd_fight_txt") -- 2015-11-25
	UIHelper.labelAddNewStroke(i18n_fight, m_i18n[1178], c3Text)
	...
	for i = 1, 3 do
		...
		btnTab:setTitleText(cfgTab[i].title) -- 2015-11-25
		btnTab:setTitleColor(c3Normal)
		btnTab:setTag(i)
...
function RaiseFightView:onTab( nIdx )
	...
	local color = nil -- 2015-11-25
	for i, btn in ipairs(self.btns) do
		color = i == self.tabIdx and c3Selected or c3Normal
	...
	if (not table.isEmpty(tbRaises)) then
		...
		for i, item in ipairs(tbRaises) do
			...
			local labTitle = m_fnGetWidget(cell, "TFD_TITLE") -- 2015-11-25
			UIHelper.labelAddNewStroke(labTitle, item.typeTitle, c3TypeTitle)
			...
			local labMember = m_fnGetWidget(cell, "TFD_LEFT") -- 2015-11-25
			UIHelper.labelAddNewStroke(labMember, item.percent, c3Percent)

			local labSlash = m_fnGetWidget(cell, "tfd_slant") -- 2015-11-25
			UIHelper.labelAddNewStroke(labSlash, "/", c3Percent)

			-- local labnDenom = m_fnGetWidget(cell, "LABN_RIGHT")
			-- labnDenom:setStringValue("100")
			local labDenom = m_fnGetWidget(cell, "TFD_RIGHT") -- 2015-11-25
			UIHelper.labelAddNewStroke(labDenom, "100", c3Percent)
