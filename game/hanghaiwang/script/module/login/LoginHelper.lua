-- FileName: LoginHelper.lua
-- Author: zhangqi
-- Date: 2014-09-07
-- Purpose: 登录和重连的相关逻辑方法
--[[TODO List]]

require "script/fw/Debuger"

module("LoginHelper", package.seeall)

require "script/utils/ExceptionCollect"
require "platform/Platform"

-- 模块局部变量 --
local _i18n = gi18n

local m_nBackTime = 0 -- 2015-04-20, zhangqi, 记录转入后台时的时间戳

local m_bReconnStatus = false -- 表示是否重连
local m_userDefault = CCUserDefault:sharedUserDefault()

local m_reconnCount = 0 -- 自动重连次数，初始为0, 最大为3

local m_ExpCollect = ExceptionCollect:getInstance()

local _bSvrUpdate = false -- 记录当前是否停服更新的状态，点进入游戏按钮时用于是否进行在线更新检测的标志

local m_bReLoginStatus = false -- 2015-04-20, zhangqi, 表示不需要重新登陆, 给活动副本特殊需求用
function reloginState( bState )
	if (bState and type(bState) == "boolean") then
		m_bReLoginStatus = bState
	else
		return m_bReLoginStatus
	end
end

function isReconnect( ... )
	return m_bReconnStatus
end

-- 2014-12-29, 如果原地重连后发现新版本，则会返回登陆界面，此时需要把重连状态设置为false
function setReconnectStat( bStat )
	m_bReconnStatus = bStat
end

-- 本地读取或保存 uid 的值, 用于实现debug模式下免去每次都要输入的麻烦
function debugUID( uid )
	if (uid) then
		m_userDefault:setStringForKey("DEBUG_UID", uid)
		m_userDefault:flush()
	else
		return m_userDefault:getStringForKey("DEBUG_UID")
	end
end

-- 读取或设置自动重连次数
function reconnCount( nCount )
	if (nCount and nCount >= 0) then
		m_reconnCount = nCount
		logger:debug("LoginHelper:reconnCount = %d", m_reconnCount)
	else
		return m_reconnCount
	end
end

function autoReconnect( ... )
	LayerManager.addLoginLoading(_i18n[4746], true) -- 自动重连的loading不需要设置超时

	local stopSchedule = nil
	stopSchedule = GlobalScheduler.scheduleFunc(function ( ... ) -- 2秒定时器到时启动重连
		stopSchedule() -- 停止定时器
		stopSchedule = nil

		reconnCount(reconnCount() + 1)
		logger:debug("reconnCount has try: %d", reconnCount())

		reconnect()
	end, g_reconnInter)
end

function netWorkFailed( bShowDlg )
	logger:debug("LoginHelper-netWorkFailed: bShowDlg = %s, Network.m_status = %s", tostring(bShowDlg), tostring(Network.m_status))
	print(debug.traceback())

	if (LayerManager.networkDlgIsShow()) then
		logger:debug("Network Dlg Is Show, don't show it")
		return -- 已经显示断线提示则不再重复处理，直接返回
	end

	-- zhangqi, 2016-02-14, 网络断开第一时间重置事件分发队列，避免意外地触发回调导致自动重连
	BTEventDispatcher:getInstance():removeAll()

	-- zhangqi, 2016-02-01, 网络断开的回调有时候是由loading的超时触发的，这时socket可能还没有断开，必须手动调用断开方法
	-- 避免点了返回登录的同时网络已经恢复，导致在选服界面触发上一个请求的回调找不到相应的模块变量（因为返回登录把加载的模块全部释放了）
	Network.disconnect()

	Network.m_status = g_network_disconnected -- zhangqi, 2015-01-14, 先修改断网状态，便于战斗结束后能及时显示断网提示

	-- zhangqi, 2014-11-25, 战斗动画开播后如果网络断开不自动重连，避免更新了玩家信息导致战斗结束结算时重复增加经验值的问题
	if (BattleState.isOnPlayRecord() == true) then
		logger:debug("netWorkFailed in battle, return")
		return
	end

	GlobalNotify.postNotify(GlobalNotify.NETWORK_FAILED) -- 发送断开通知

	LayerManager.removeLoading()

	local function showFailedDlg( ... ) -- 弹出断线提示面板
		logger:debug("show network failed dlg")

		reconnCount(0) -- 2015-09-11, 重置自动重连计数

		-- 2015-04-20, zhangqi, 活动副本需求：如果在获取掉落物品信息后到领取前发生掉线，则弹出只有返回登陆按钮的面板
		if (reloginState()) then
			UIHelper.showNetworkDlg(nil, function ( ... )
				LayerManager.removeNetworkDlg()
			end, true, _i18n[4210]) -- true 模拟被抢号，只显示返回登陆按钮，但提示文字还是"网络异常"

			return
		end

		UIHelper.showNetworkDlg(nil, function ( ... )
			LayerManager.removeNetworkDlg()
		end, Network.m_bOtherLogin)
	end

	-- zhangqi, 2015-03-26, 尝试自动重连3次，每次间隔2秒；如果3次重连都失败才弹出断网提示面板
	if (bShowDlg == true) then -- 自动重连全部失败会显示断网提示
		LayerManager.removeLoginLoading()
		showFailedDlg()
	else -- 启动第一次自动重连
		-- zhangqi, 2016-01-06, 如果在创建角色界面断开则直接显示断网提示，避免自动重连导致模块释放引起找不到对象的报错
		if (LayerManager.curModuleName() == "UserNameView") then
			showFailedDlg()
			return
		end

		-- zhangqi, 2015-12-23, 如果是后端主动断开或帐号别处登陆则直接显示提示
		if (Network.m_bKickOff or Network.m_bOtherLogin or (reconnCount() >= g_reconnMax)) then
			LayerManager.removeLoginLoading()
			showFailedDlg()
		else
			autoReconnect()
		end
	end
end

local tbBaseModule = {
	"script/module/public/class", -- 最基本的公用模块
	"db/i18n",
	"script/module/config/AudioHelper",
	"script/GlobalVars",
	"script/module/public/GlobalNotify", -- 2014-09-07
	"script/module/public/GlobalScheduler",
	"script/utils/LuaUtil",
	"script/utils/TimeUtil",
	"script/module/public/UIHelper",
	"script/fw/Util", -- 游戏框架中的模块
	"script/fw/Logger",
	"script/fw/EventDispatcher",
	"script/fw/Downloader",
	"script/fw/Updater",
	"script/app/Helper",
	"script/app/Application",
	"script/module/main/LayerManager", -- 层管理器
	"script/network/Network",
	"script/battle/BattleState",
	"script/module/PlayerInfo/PlayerInfoBar", -- 玩家信息条基类
	"script/module/public/Bag/BagModel", -- 统一管理背包处理对象的模块, 2015-09-26
}
local function debugPrintTable( table )
	print("debugPrintTable begin")
	for k, v in pairs(table) do
		print(k, v)
	end
	print("debugPrintTable end")
end
-- zhangqi, 重新加载所有的基础性模块
function reloadBaseModule( ... )
	-- debugPrintTable(package.loaded)

	for i, path in ipairs(tbBaseModule) do
		package.loaded[path] = false
		print("reloadBaseModule: path = ", path)
		require(path)
	end

	-- debugPrintTable(_G)

	setLogLevel()
end

-- 2015-04-07, zhangqi, 优化：去掉对path中模块名称的解析，只需判断 package.loaded[path] 的类型即可
function releaseLoadedModule( ... )
	--liweidong 2015-12-2 释放c++缓存对象
	deleteForeverObjects()
	-- zhangqi, 2015-12-01, 从reloadBaseModule移到这里，既然是释放则需要在先
	-- 在reloadBaseModule中其他baseModule loaded以后再调用会导致lua栈溢出的错误
	autoRequire() --释放AutoRequire加载过的全局变量

	for path, mod in pairs(package.loaded) do
		if (type(mod) == "boolean") then
			print("releaseLoadedModule: boolean = ", path)
			package.loaded[path] = false
		elseif (type(mod) == "table") then
			-- zhangqi, 2016-01-26, 由于不确定某些模块在自己的destroy方法中是否有释放和处理C++对象等特殊情况
			-- 为了避免类似从战斗中点返回登录按钮导致的崩溃，先把调用destroy方法的代码注释掉
			-- if (mod.destroy) then
			-- 	print("destroy mod = ", path)
			-- 	pcall(mod.destroy) -- zhangqi, 2014-10-16, 用pcall避免在某个destroy里调用前面已被释放了的模块导致报错
			-- end

			-- logger的模块table不能释放，避免其他未释放但正在使用logger的模块报错, 随后也能重新加载
			if (not string.find(path, "logging")) then
				print("releaseLoadedModule mod = ", path)
				package.loaded[path] = nil
			end
		end
	end
end

-- 2015-04-20, zhangqi, 游戏转入后台时的回调
-- 1.记录当前时间戳，用于切换前台后的时长判断，超过30分钟就自动返回选服列表
function appToBackgroundCallback( ... )
	logger:debug("app in Background")
	m_nBackTime = TimeUtil.getSvrTimeByOffset()
	logger:debug("app in Background: m_nBackTime = " .. m_nBackTime)
end

function appToForegroundCallback( ... )
	logger:debug("app in Foreground")

	local now = TimeUtil.getSvrTimeByOffset()
	logger:debug("app in Foreground: now = %d, m_nBackTime = %d", now, m_nBackTime)
	if ( now - m_nBackTime > 1800 ) then -- 如果切入后台超过30分钟，自动切换到选服界面
		loginAgain()
		return
	end

	-- 如果切入后台到回来跨过了0点，自动切换到选服界面
	logger:debug("app in Foreground: getDifferDay = %d", TimeUtil.getDifferDay(m_nBackTime))
	if (TimeUtil.getDifferDay(m_nBackTime) > 0) then
		loginAgain()
	end
end

function setLogLevel( ... )
    logger:setLevel(logging.DEBUG)
    Logger.setLevel(Logger.kDebug)
end


function initGame( ... )
	reloadBaseModule()

	GlobalNotify.init() -- 初始化全局通知管理器
	LayerManager.init() -- 初始化层管理器，进一步加载资源更新UI

	GlobalNotify.addObserverForBackAndForegroud() -- 注册程序切入系统后台和切回前台的回调通知
	GlobalNotify.addObserverForBackground("LoginHelper.initGame", appToBackgroundCallback) -- 注册不删除的切入后台通知
	GlobalNotify.addObserverForForeground("LoginHelper.initGame", appToForegroundCallback)

	-- zhangqi, 2015-09-28, 注册背包信息变化的通知，更新各背包处理对象的总背包信息引用
	GlobalNotify.addObserver(GlobalNotify.BAG_CHANGED, BagModel.updateBagHandler, false, "BAG_CHANGED")
end

-- true 表示是重连的版本检测，不会切换到启动时版本检测的背景
function initMainApp( bReconn )
	-- 进入更新检查模块
	local mainApp = Application.create()
	mainApp:init(bReconn)
end

-- zhangqi, 2016-02-20, 停服更新时点击进入游戏按钮需要调用的方法
-- 重新走版本检测的流程，避免有和停服同步的前端更新在选服界面无法检查到导致进游戏报错
function svrUpdateWaitCheck( ... )
	svrUpdateTest = true
	local mainApp = Application.create()
	mainApp:init(true, true)
end

-- zhangqi, 2016-02-19, 设置和读取当前是否停服更新的状态标志
function svrUpdateStat( bStat )
	if (bStat ~= nil) then
		_bSvrUpdate = bStat
	else
		return _bSvrUpdate
	end
end


-- 创建socket连接和给后端发登陆请求的方法
function connectAndLogin ( ... )
	BTEventDispatcher:getInstance():removeAll() -- 重置事件派发队列
	PackageHandler:setToken("0") -- 重置网络连接的 token

	local svrInfo = g_tbServerInfo
	logger:debug({connectAndLogin_g_tbServerInfo = svrInfo})

	if (g_debug_mode and g_no_web) then -- 如果web服务异常就用默认配置
		svrInfo = {host = g_host, port = g_port}
	end

	if (Platform.isPlatform()) then
		svrInfo = _tPlatformUserTable -- 2016-03-09, zhangqi, 接入SDK时，服务器ip和端口应该从getHash返回的信息中获取才对
	end

	m_ExpCollect:start("socket_connect")

	if (Network.connect(svrInfo.host, svrInfo.port)) then
		m_ExpCollect:finish("socket_connect")

		reconnCount(0) -- 2015-09-10, 重置自动重连计数

		-- 2013-03-11, 平台统计需求，从Web端获取到Pid后开始登录游戏服务器
		if (Platform.isPlatform()) then
			Platform.sendInformationToPlatform(Platform.kEnterGameServer)
		end

		-- logger:debug("socket connected, svrInfo.host: %s, svrInfo.port: %d", svrInfo.host, svrInfo.port)
		if (m_bReconnStatus) then
			LayerManager.addLoginLoading()
		else
			LayerManager.addRegistLoading()
		end

		BTEventDispatcher:getInstance():addLuaHandler("failed", netWorkFailed, false) -- 注册网络断开的事件

		require "script/network/RequestCenter"
		require "script/network/user/UserHandler"
		m_ExpCollect:start("user_login")
		RequestCenter.user_login(UserHandler.login, getLoginNetworkArgs())
	else
		netWorkFailed()
	end
end

-- 点击进入游戏按钮调用的方法
function loginGame ( ... )
	Network.m_status = g_network_connecting

	logger:debug("loginGame m_bReconnStatus = %s", tostring(m_bReconnStatus))

	-- zhangqi, 2015-12-17, 加入线下连线上环境的条件判断
	if (g_DEBUG_env.offline) then
		local pid = g_DEBUG_env.online_pid
		loginLogicServer(pid)
		return
	end

	if (Platform.isPlatform()) then
		local pid = Platform.getPid()
		if (not pid or pid == "") then
			LayerManager.addLayout(UIHelper.createCommonDlg(_i18n[4401]))
			return
		end

		loginLogicServer(pid)
	else
		local nOnLineType = BTUtil:getBTOnlineType()
		-- 线下包Release暂时只用作体验包打包，需要屏蔽Lua报错窗口
		g_web_env.online = not (nOnLineType % 2 == 0) -- zhangqi, 2015-10-08, 改正是否线上环境的条件判断
		connectAndLogin()
	end
end

function enterGame( ... )
	m_ExpCollect:finish("hero_getAllHeroes_normal")

	Network.m_status = g_network_connected

	require "script/module/upgrade/NewUpgradeCtrl"
	GlobalNotify.addObserver(GlobalNotify.LEVEL_UP, NewUpgradeCtrl.create, nil, "NewUpgradeCtrl.create")

	-- zhangqi, 2014-12-30, 如果开启了阵容同时把判断阵容装备红点的方法注册到断线重连成功后的通知
	if (SwitchModel.getSwitchOpenState(ksSwitchFormation)) then
		require "script/module/public/ItemUtil"
		GlobalNotify.addObserver(GlobalNotify.RECONN_OK, ItemUtil.justiceBagInfo, nil, "ItemUtil.justiceBagInfo")
	end

	-- zhangqi, 2015-07-01, 从PreRequest.lua的startPreRequest方法中移来
	require "script/module/switch/SwitchCtrl"
	SwitchCtrl.registerBattleNotification() -- add by huxiaozhou 2014-06-07
	SwitchCtrl.registerLevelUpNotification()
	DataCache.regLevelUpCallBack() -- 升级时DataCache相关的回调处理
	DataCache.regVipLevelUpCallBack() -- vip等级提升需要的回调

	GlobalScheduler.schedule()

	require "script/module/copy/MainCopy"
	local bPassFirstEnt = MainCopy.isCrossFirstEntrance()
	logger:debug("有没有通过第一个副本: %s", tostring(bPassFirstEnt))

	if (bPassFirstEnt) then
		require "script/module/main/MainScene"
		MainScene.create()
	else
		require "script/module/login/NewGuyHelper"
		NewGuyHelper.enterGuide()
	end

	--LayerManager.removeLoginLoading()
	LayerManager.removeRegistLoading()
end

-- 2015-03-11, zhangqi, 注销平台帐号的重新登陆
function platformLogout( ... )
	loginAgain(true) -- 2015-12-25
end

-- 重新登录，会重新走版本检测流程和平台sdk登录流程
-- zhangqi, 2015-12-25, 增加参数：bLogout, 如果是游戏设置里点注销时为true
function loginAgain(bLogout)
	-- zhangqi, 2016-02-14
	BTEventDispatcher:getInstance():removeAll() -- 重置事件派发队列

	-- 张琦，2015-12-18，先停止所有自定义定时器，以免其中有刷新UI的处理，在后面清除游戏场景后导致报错
	GlobalScheduler.destroy()

	local curScene = CCDirector:sharedDirector():getRunningScene()
	if (UIHelper.isGood(curScene)) then -- zhangqi, 2016-01-26, 经常为nil导致崩溃，加一个有效性检查
		curScene:removeAllChildrenWithCleanup(true) -- 删除当前场景的所有子节点
	end
	CCTextureCache:sharedTextureCache():removeUnusedTextures() -- 清除所有不用的纹理资源

	-- zhangqi, 2015-12-19, 放在游戏场景清理之后，避免某些UI节点被删除时调用注册的通知却找不到模块导致报错
	releaseLoadedModule() -- zhangqi, 2014-09-03, 释放所有已加载的模块，保证重新登录可以加载新的代码
	m_bReconnStatus = false

	initGame()

	require "platform/Platform"
	-- zhangqi,2015-12-25,调用setInGameLogout通知Platform本次登录是否是游戏内注销
	-- 避免Platform中回调loginAgain方法出现SDK重复初始化的问题
	Platform.setInGameLogout(bLogout or false)
	Platform.initSDK()
	-- 进入更新检查模块
	initMainApp()
end

-- 原地重连
function reconnect( ... )
	m_bReconnStatus = true

	-- 进入更新检查模块
	initMainApp(true) -- true 表示是重连的版本检测，不会切换到启动时版本检测的背景
end

function startPreRequest( ... )
	logger:debug("LoginHelper.startPreRequest")

	-- 在取得功能节点之后获得武将战斗力信息
	local function getAlHeroesInfoAndSetFightforce( callback )
		require "script/network/RequestCenter"
		RequestCenter.hero_getAllHeroes(function ( cbFlag, dictData, bRet )
			logger:debug({hero_getAllHeroes_dictData = dictData})

			if (bRet and cbFlag == "hero.getAllHeroes") then
				require "script/model/hero/HeroModel"
				HeroModel.setAllHeroes(dictData.ret)
				UserModel.setInfoChanged(true)
				UserModel.updateFightValue() -- 然后更新战斗力数值
				updateInfoBar() -- zhangqi, 2015-12-24, 增加重连后刷新信息条的处理
				
				if (callback) then
					callback()
				end
			end
		end)
	end

	-- menghao 1027 为了创建角色处断线重连后能进入游戏
	local bNotLogin = LayerManager.curModuleName() ~= "NewLoginView" -- 是否在登录界面
	local bNotNew = not UserHandler.isNewUser -- 不是新用户
	 -- 如果是重新连接，且不是新用户以及不是登录界面的重连，则不重新拉取所有数据，避免依赖断线前状态的UI刷新处理混乱
	if (m_bReconnStatus and bNotNew and bNotLogin) then
		UserHandler.isNewUser = false -- menghao 同时要把这个置为false
		logger:debug("LoginHelper.startPreRequest: not do PreRequest")

		require "script/network/PreRequest"
		PreRequest.registerAllPushMessageHandler() -- 重新注册所有推送消息处理

		function endOfReconnect ( ... )
			m_ExpCollect:finish("hero_getAllHeroes_reconnect")

			m_bReconnStatus = false
			Network.m_status = g_network_connected
			LayerManager.removeLoginLoading()
			GlobalNotify.postNotify(GlobalNotify.RECONN_OK) -- 给连接成功的观察者发通知
			-- GlobalNotify.postNotify(GlobalNotify.USER_RECONN_OK) -- 给非登陆重连成功的观察者发通知
		end

		if (LayerManager.isInBattle()) then -- 如果是战斗场景中重连，需要先调后端接口重置战斗状态
			logger:debug("reconnect In Battle")
			require "script/network/RequestCenter"

			m_ExpCollect:start("ncopy_getAtkInfoOnEnterGame")
			RequestCenter.ncopy_getAtkInfoOnEnterGame(function ( ... )
				m_ExpCollect:finish("ncopy_getAtkInfoOnEnterGame")

				m_ExpCollect:start("hero_getAllHeroes_reconnect")
				getAlHeroesInfoAndSetFightforce(endOfReconnect) -- 拉取所有伙伴信息，更新客户端战斗力数值
			end, nil)
		else
			m_ExpCollect:start("hero_getAllHeroes_reconnect")
			getAlHeroesInfoAndSetFightforce(endOfReconnect) -- 拉取所有伙伴信息，更新客户端战斗力数值
		end

		return
	end

	-- zhangqi, 2016-03-9, 为了避免SDK记录到缓存文件的最近登录服务器和玩家实际登录的不一致，
	-- 在玩家登录游戏服成功后再设置
	if (Platform.isPlatform()) then
		Platform.setLastLoginGroup(_tPlatformUserTable.serverID)
	end

	---------- 开始拉数据 -------
	require "script/network/PreRequest"
	UserHandler.isNewUser = false -- menghao 同时要把这个置为false
	m_ExpCollect:start("startPreRequest")
	PreRequest.startPreRequest(function ( ... )
		m_ExpCollect:finish("startPreRequest")

		m_ExpCollect:start("hero_getAllHeroes_normal") -- 正常登录拉取allHeroInfo
		getAlHeroesInfoAndSetFightforce(enterGame)
	end)
end


------------------------ 三国的老代码 ------------------------------
-- local _bNotOvertureStatus = false

function getLoginNetworkArgs( ... )
	local args
	-- zhangqi, 2015-12-17, 加入线下连线上环境的条件判断
	if (Platform.isPlatform() or g_DEBUG_env.offline) then
		local userDic = CCDictionary:create()
		for k,v in pairs(_tPlatformUserTable) do
			userDic:setObject(CCString:create(tostring(v)),k)
		end
		args = CCArray:create()
		args:addObject(userDic)
	else
		local debugPid = debugUID()
		args = CCArray:createWithObject(CCInteger:create(tonumber(debugPid)))
	end
	local sKeyValue = "publish=" .. Helper.pkgVer .. ", script=" .. Helper.resVer .. ", pl="..Platform.getPlatformFlag()
	args:addObject(CCString:create(sKeyValue))

	m_ExpCollect:info("user_login", "args = " .. sKeyValue)

	return args
end

-- 网络参数 CCArray类型
function loginInServer( user_table )
	logger:debug({LoginHelper_loginInServer_user_table = user_table})
	_tPlatformUserTable = user_table

	-- 如果 web 端返回的数据里有 online_state 为 0，则为线下环境, g_web_env.online = false
	g_web_env.online = not (user_table.online_state and (user_table.online_state == 0))

	-- zhangqi, 2015-12-18, 加入线下连线上环境的条件判断
	if (g_DEBUG_env.offline) then
		g_web_env.online = false -- 设置为false，允许弹出报错提示
	end

	connectAndLogin()
end

--登陆逻辑服务器
function loginLogicServer( pid )
	-- 从服务器拉去登陆数据
	pid = pid or Platform.getPid()
	local url = Platform.getGameServerInfoUrl(pid)

	-- zhangqi, 2015-12-17, 加入线下连线上环境的条件判断
	if (g_DEBUG_env.offline) then
    	local group_id = "game" .. g_DEBUG_env.group
		local svrInfoUrl = "http://192.168.2.112:8080/api/gethash.php?&group_id=" .. group_id .. "&pid=" .. pid .. "&uuid=UUID_IS_NULL"
		url = Platform.signUrl(svrInfoUrl)
	end

	logger:debug("getHash request url:" .. url)
	LayerManager.addLoading()

	local request = LuaHttpRequest:newRequest()
	request:setRequestType(CCHttpRequest.kHttpGet)
	request:setUrl(url)
	request:setResponseScriptFunc(loginGameResult)

	m_ExpCollect:start("getHash", "url = " .. url)

	CCHttpClient:getInstance():send(request)
	request:release()
end
function loginGameResult( sender, res )
	LayerManager.removeLoading()

	if (res:getResponseCode() ~= 200) then
		m_ExpCollect:info("getHash", "responseCode = " .. tostring(res:getResponseCode()))
		LayerManager.addLayout(UIHelper.createCommonDlg(_i18n[4757]))
		return
	end

	local loginJsonString = res:getResponseData()
	logger:debug("loginJsonString:" .. loginJsonString)
	local cjson = require "cjson"
	local loginInfo = cjson.decode(loginJsonString)

	-- 2015-05-26, zhangqi, 新增返回错误码的处理
	local errInfo = {[101] = 4757, [102] = 4757, [103] = 4757, [104] = 4758, [105] = 4757}
	if (loginInfo.err_code) then
		m_ExpCollect:info("getHash", "err_code = " .. tostring(loginInfo.err_code))

		svrUpdateStat(tonumber(loginInfo.err_code) == 104) -- zhangqi, 2016-02-19, 记录当前是否停服更新

		local i18nId = errInfo[loginInfo.err_code] or errInfo[101]
		LayerManager.addLayout(UIHelper.createCommonDlg(_i18n[i18nId]))


		m_ExpCollect:finish("getHash") -- zhangqi, 2016-02-25, 停服更新应该算正常情况，需要结束getHash登录异常收集
		Platform.sendUuid("getHash","after")
		return
	end

	svrUpdateStat(false) -- zhangqi, 2016-02-19, 正常情况重置停服更新状态为false

	m_ExpCollect:finish("getHash")

	Platform.sendUuid("getHash","after")
	loginInServer( loginInfo )
end

local function fnHanlderOfServer( ... )
	loginAgain()
end

local function eventOfServer( sender, eventType )
	if (eventType == TOUCH_EVENT_ENDED) then
		fnHanlderOfServer()
	end
end

-- 服务器与平台链接无效了
function fnServerIsTimeout( ... )
	logger:debug("login game server Timeout")
	-- 需要重新调用getHash然后登陆游戏服
	if (Platform.isPlatform()) then
		loginLogicServer()
	else
		LayerManager.addLayout(UIHelper.createCommonDlg(_i18n[4774], nil, eventOfServer, 1, fnHanlderOfServer))
	end
end
-- 服务器已满
function fnServerIsFull( ... )
	LayerManager.addLayout(UIHelper.createCommonDlg(_i18n[4775], nil, eventOfServer, 1, fnHanlderOfServer))
end
-- 游戏帐号被禁用
function fnIsBanned(pBanInfo)
	-- zhangqi, 2016-02-01, 先注销断网回调的事件，避免弹出断网提示面板
	BTEventDispatcher:getInstance():removeEvent("failed")

	LayerManager.removeRegistLoading() -- 再移除登录的loading

	if (pBanInfo and pBanInfo.msg) then
		local endTime = tonumber(pBanInfo.time or 0)
		local text = string.format("%s\n%s\n%s", pBanInfo.msg or "", _i18n[2004], TimeUtil.getTimeFormatYMDHMS(endTime))
		-- 提示一个带有确定按钮的通用对话框，点击确定按钮只关闭当前对话框
		local tbArgs = {strText = text, nBtn = 1}
		LayerManager.addLayout(UIHelper.createCommonDlgNew(tbArgs))
	end
end

-- zhangqi, 2016-01-20, 根据是否集成SDK返回对应的pid
function pidGet( ... )
	if (Platform.isPlatform()) then
		return Platform.getPid()
	else
		return LoginHelper.debugUID()
	end
end

-- zhangqi, 2016-01-20, 根据默认的pid获取或保存最近登录服务器group
-- 第一个参数为最近登录服务器group
function fnRecentServerGroup( group )
	local pidEncrypt = pidGet()

	logger:debug({fnRecentServerGroup_pidEncrypt = pidEncrypt, group = group})

	if (group and pidEncrypt) then
		m_userDefault:setStringForKey("recentServerGroup" .. "_" .. pidEncrypt, group)
		m_userDefault:flush()
	elseif (pidEncrypt and pidEncrypt ~= "") then
		return m_userDefault:getStringForKey("recentServerGroup" .. "_" .. pidEncrypt)
	else
		-- zhangqi, 2016-02-25, SDK未登陆前pidEncrypt为nil, 无法根据pidEncrypt从本地缓存里读取其最近登陆信息
		-- 所以改用从Platform的方法里读取一个和pid无关的最近登陆服务器（理论上也就是当前SDK账号最近一次登陆）
		-- 如果本地没有缓存，lastGroup 就是空字符串 ""；这个最近登陆用来来设置SDK登陆前的默认选中服务器，避免一些意外报错
		local lastGroup = Platform.getLastLoginGroup()
		logger:debug({fnRecentServerGroup_lastGroup = lastGroup})
		return lastGroup
	end
end
