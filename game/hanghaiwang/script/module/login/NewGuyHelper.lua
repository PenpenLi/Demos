...
function showLuojie( callback )
	...
	createBtnSkip(layout, function ( ... )
		m_ExpCollect:finish("showLuojie")
		callback()
	end)
	...
	local function newAndAddArmature( ... )
		for k, v in pairs({...}) do
			...
			if (v == "") then
				AudioHelper.playMusic("audio/bgm/denglu.mp3", true)
			end
	...
	newAndAddArmature("", "_1", "_2") -- 2015-10-27, 1 和 2 交换层级
	m_ExpCollect:finish("showLuojie")
end
...
function enterGuide( ... )
	...
	-- -- zhangqi, 2015-10-14, 添加顶上战争
	-- require "script/battle/BattleModule"
	-- BattleModule.showTopFirstBattle(function ( ... )
	-- 	BattleModule.showTopSecondBattle(function ( ... )
	-- 		require "script/module/copy/MainCopy"
	-- 		MainCopy.enterGuildBattle()
	-- 	end)
	-- end)
	
	-- zhangqi, 2015-11-09, 封测跳过顶上战争
	require "script/module/copy/MainCopy"
	MainCopy.enterGuildBattle()
end
