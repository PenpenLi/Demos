-- FileName: AdBattleEventCtrl.lua
-- Author: liweidong
-- Date: 2015-04-02
-- Purpose: 继承血量战斗事件控件
--[[TODO List]]

module("AdBattleEventCtrl", package.seeall)
...
function destroy(...)
	package.loaded["AdBattleEventCtrl"] = nil
end

function moduleName()
    return "AdBattleEventCtrl"
end
function initUI()
	...
	UIHelper.labelNewStroke( mainLayout.TFD_PROGRESS, ccc3(0x28,0x00,0x00), 2 )
	mainLayout.img_desc_bg:setScale(g_fScaleX)
	
	mainLayout.tfd_probability:setText(m_i18n[4350])
	...
	local labExp = m_fnGetWidget(layoutMain, "TFD_EXP") -- 经验数量
	...
	for i, itemIcon in ipairs(tbItem) do
		...
		labItemName:setColor(g_QulityColor[tonumber(itemIcon.quality)])
		itemCount = i
	end
...
function updateUI()
	...
	if (eventInfoData.boss) then
		...
		local percent = (nPercent > 100) and 100 or nPercent
		bloodLoad:setPercent(percent)
		mainLayout.TFD_PROGRESS:setText(math.floor(percent/1).."%")
...
--是否显示红点
function isShowRedTip(evtId)
	local eventInfoData=AdventureModel.getEventItemById(evtId)
...
function battleWin(tbReward__)
	...
	if (AdBatlleEventModel.getBuyTimesRemainNum(eventInfoData)<=0 and resetStatus) then
...
function onBattleCallBack( cbFlag, dictData, bRet )
	if(dictData.ret~=nil and dictData.err=="ok")then
		...
		AdBatlleEventModel.setBattleType(1)
		-- updateUI()
		local eventDb=DB_Exploration_things.getDataById(eventInfoData.etid)
...
function create(id)
	...
	if (layoutMain) then
		...
		mainLayout = g_fnLoadUI("ui/magical_thing_fight_nor.json") --g_fnLoadUI
		mainLayout:setSize(g_winSize)
		layoutMain:addChild(mainLayout)

		-- local imgBg = g_fnGetWidgetByName(layoutMain, "img_bg")
