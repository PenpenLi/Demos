...
local m_i18n = gi18n
local m_nSwitchEnum
local m_sCallBackKey = "CALLBACKKEY"
...
function callBackFunc( sender,eventType )
   ...
   --  		require "script/module/guide/GuideModel"
		...
		elseif (m_nSwitchEnum == ksSwitchLitFmt) then
			...
			-- changeToMainShip()
		elseif (m_nSwitchEnum == ksSwitchMainShip) then
			-- 主船
    		require "script/module/guide/GuideModel"
			...
			changeToMainShip()
		elseif (m_nSwitchEnum == ksSwitchImpelDown) then
			--深海监狱
    		require "script/module/guide/GuideModel"
			...
			changeToMainShip()
		elseif (m_nSwitchEnum == ksSwitchAwake) then
			require "script/module/guide/GuideModel"
...
function createTipData( switchEnum )
	...
	local str = m_i18n[4801] .. "|［%s］|" .. m_i18n[2104]

	local tbRich = { str, 
						{
							{size = 26,color={r=0x82;g=0x56;b=0x00},font=g_FontCuYuan}, 
							{color={r=0x00;g=0x62;b=0x0c},size = 26,font=g_FontCuYuan},
							{size = 26,color={r=0x82;g=0x56;b=0x00},font=g_FontCuYuan},
						}
					}
	...
	if(tonumber(switchEnum) == ksSwitchFormation) then
		--阵容
		switchIcon = "images/new_player/icons/new_formation.png"
	elseif(tonumber(switchEnum) == ksSwitchShop) then
		switchIcon = "images/new_player/icons/new_bar.png"
	...
	elseif(tonumber(switchEnum) == ksSwitchArena) then
		--竞技场 
		switchIcon = "images/new_player/icons/new_arena.png"
	elseif(tonumber(switchEnum) == ksSwitchActivityCopy) then
		--活动副本 	--error
		switchIcon = "images/new_player/icons/new_acopy.png"
	...
	elseif(tonumber(switchEnum) == ksSwitchResource) then
		--资源矿
		switchIcon = "images/new_player/icons/new_resource.png"	
	...
	elseif(tonumber(switchEnum) == ksSwitchGuild) then
 		-- 军团系统
 		-- switchIcon = "images/upgrade/new_union.png"
	...
	elseif(tonumber(switchEnum) == ksSwitchEquipFixed) then
 	...
 	elseif(tonumber(switchEnum) == ksSwitchTower) then
 		-- 神秘空岛
 		switchIcon = "images/new_player/icons/new_air.png"
 	elseif(tonumber(switchEnum) == ksSwitchWorldBoss) then
 		switchIcon = "images/new_player/icons/new_boss.png"
	...
	elseif(tonumber(switchEnum) == ksSwitchExplore) then
		switchIcon = "images/new_player/icons/new_explore.png"
	elseif(tonumber(switchEnum) == ksSwitchTreasure) then
		switchIcon = "images/new_player/icons/new_treasure.png"
	elseif(tonumber(switchEnum) == ksSwitchReborn) then
		switchIcon = "ui/resolve_n.png"
	elseif (tonumber(switchEnum) == ksSwitchLitFmt) then
		switchIcon = "images/new_player/icons/new_small.png"
	elseif(tonumber(switchEnum) == ksSwitchMainShip) then
		switchIcon = "images/new_player/icons/new_ship.png"
	elseif(tonumber(switchEnum) == ksSwitchImpelDown) then
		switchIcon = "images/new_player/icons/new_prison.png"
	elseif(tonumber(switchEnum) == ksSwitchAwake) then
		switchIcon = "images/new_player/icons/new_awake.png"
	end
...
function createForthFormationView(  )
	...
	tbData.alertContent = m_i18n[5715]


	tbData.icon  = "images/new_player/icons/new_four.png"
	tbData.richText = BTRichText.create(tbRich,nil,nil,m_i18n[5716])
...
function registerBattleNotification(  )
	...
	GlobalNotify.addObserver(m_sCallBackKey,handleBattleNotificationCallback,false, "handleBattleNotificationCallback")
end
