...
function addSuiPianToWidget( widget )
	addSignToIcon(widget,"images/common/tips_fragment.png")
end


--判断某个物品在背包中是否存在,道具背包
-- zhangqi, 2015-12-21, 将计算一个物品的数量修改为计算所有同tid物品的数量，否则会获得错误数量
function getNumInBagByTid( tid )
	...
	for gid, item in pairs(propsInfo) do
		...
		if (tonumber(tid) == tonumber(itemId)) then
			item_num = item_num + tonumber(item.item_num)
		end
	end
...
-- 判断某个物品在觉醒背包里的个数
function getAwakeNumByTid( tid )
	local awakeInfo = DataCache.getRemoteBagInfo().awake
	local totalNum = 0
	for gid, item in pairs(awakeInfo) do
		local itemId = item.item_template_id
		if (tonumber(tid) == tonumber(itemId)) then
			totalNum = totalNum + item.item_num
		end
	end
...
function getUseResultBy( i_id, i_num, isModifyCache)
	...
	if (i_data.coins and i_data.coins > 0) then
		...
		result_text = m_i18nString(1514, tostring(result.coins))
	end
	if (i_data.golds and i_data.golds > 0) then
		result.golds = i_data.golds * i_num
		result_text = m_i18nString(1513, tostring(result.golds))
	end
	if (i_data.energy and i_data.energy > 0) then
		result.energy = i_data.energy * i_num
		result_text = m_i18nString(1504, tostring(result.energy))
	end
	if (i_data.endurance and i_data.endurance > 0) then
		result.endurance = i_data.endurance * i_num
		result_text = m_i18nString(1503, tostring(result.endurance))
	end

	if (i_data.use_getRime and i_data.use_getRime > 0) then
		result.rime = i_data.use_getRime * i_num
		result_text = m_i18nString(6953, tostring(result.rime))
	end
	...
	if ( result )then
		...
		if (isModifyCache) then
			...
			elseif(result.rime)then
				UserModel.addRimeNum(result.rime)
			end
		end
	end
...
function getItemTypeByTid( tid )
	local types = {isDirect = false, isGift = false, isRandGift = false, isFeed = false, isNormal = false,
		...
		isTreasure = false, isTreasureFragment = false, isConch = false, isDress = false,isExcl = false ,isExclFrag = false,
		isShip = false, isAwake = false, 
	}
	if (not tid) then
		return types
	end
	...
	elseif(i_id >= 40001 and i_id <= 50000) then -- 激活主船物品
		types.isShip = true
	...
	elseif(i_id >= 100001 and i_id <= 200000) then -- 装备类：
		types.isArm = true
	elseif(i_id >= 500001 and i_id <= 600000) then -- 饰品类：
		require "db/DB_Item_treasure"
		types.isTreasure = true
	elseif( i_id >= 5000001 and i_id <= 6000000 )then -- 饰品碎片
		types.isTreasureFragment = true
	elseif( i_id >= 70001 and i_id <= 80000 )then -- 空岛贝
		types.isConch = true
	elseif( i_id >= 80001 and i_id <= 90000 )then -- 时装
		types.isDress = true
	elseif( i_id >= 700001 and i_id <= 800000 )then -- 专属宝物（宝物）
		types.isSpeTreasure = true
	elseif (i_id >= 800001 and i_id <= 900000) then -- 觉醒物品
		types.isAwake = true
	elseif( i_id >= 7000001 and i_id <= 8000000 )then -- 专属宝物碎片（宝物碎片）
		types.isSpeTreasureFragment = true
	end
	return types
end
...
function getItemById( i_id )
	...
	elseif(i_id >= 40001 and i_id <= 50000) then -- 激活主船物品
		...
		i_data = DB_Item_ship.getDataById(i_id)
		i_data.isShip = true
		i_data.fnBagFull = fullWrapper(isPropBagFull)
	elseif(i_id >= 50001 and i_id <= 60000) then -- 坐骑饲料类：50001~80000
	...
	elseif(i_id >= 400001 and i_id <= 500000) then -- 武将碎片类：
		...
		i_data.nOwn = getCacheItemNumBy(i_id)
		i_data.nMax = i_data.need_part_num
	...
	elseif( i_id >= 5000001 and i_id <= 6000000 )then -- 宝物碎片
		...
		i_data.nOwn = TreasureData.getFragmentNum(i_id)

		i_data.nMax = TreasureData.getCompondNumByFragId(i_id)
	...
	elseif( i_id >= 700001 and i_id <= 800000 )then -- 专属宝物
		require "db/DB_Item_exclusive"
		i_data = DB_Item_exclusive.getDataById(i_id)
		i_data.isSpeTreasure = true
		i_data.desc = i_data.info
		i_data.imgFullPath = "images/base/exclusive/small/" .. i_data.icon_small
		i_data.fnBagFull = fullWrapper(isSpecialTreasBagFull)

		if (i_data.icon_big) then
			i_data.iconBigPath = "images/base/exclusive/big/" .. i_data.icon_big
		else
			i_data.iconBigPath = "images/base/exclusive/small/" .. i_data.icon_small
		end
	elseif (i_id >= 800001 and i_id <= 900000) then  -- 觉醒物品
		i_data = DB_Item_disillusion.getDataById(i_id)
		i_data.isAwake = true
		i_data.imgFullPath = "images/disillusion/" .. i_data.icon_small						  -- TODO
		i_data.iconBigPath = nil
		i_data.fnBagFull = fullWrapper(isAwakeBagFull)
	elseif( i_id >= 7000011 and i_id <= 8000000 )then -- 专属宝物碎片
		require "db/DB_Item_exclusive_fragment"
		i_data = DB_Item_exclusive_fragment.getDataById(i_id)
		i_data.isSpeTreasureFragment = true
		i_data.desc = i_data.info
		i_data.imgFullPath = "images/base/exclusive/small/" .. i_data.icon_small
		i_data.fnBagFull = fullWrapper(isSpecialTreasFragBagFull)
		if (i_data.icon_big) then
			i_data.iconBigPath = "images/base/exclusive/big/" .. i_data.icon_big
		else
			i_data.iconBigPath = "images/base/exclusive/small/" .. i_data.icon_small
...
function reduceItemByGid( i_gid, i_num, isForceDel )
	...
	local i_data_t = nil
	if (i_gid >= 2000001 and i_gid < 3000000 ) then -- 装备
		i_data_t = remoteBagInfo.arm
	elseif (i_gid >= 3000001 and i_gid < 4000000) then -- 道具
		i_data_t = remoteBagInfo.props
	elseif (i_gid >= 4000001 and i_gid < 5000000) then -- 武将碎片
		i_data_t = remoteBagInfo.heroFrag
	elseif (i_gid >= 5000001 and i_gid < 6000000) then -- 宝物
		i_data_t = remoteBagInfo.treas
	elseif (i_gid >= 6000001 and i_gid < 7000000) then -- 装备碎片
		i_data_t = remoteBagInfo.armFrag
	elseif( i_gid >= 8000001 and i_gid <= 9000000 )then
		-- 专属宝物
		i_data_t = remoteBagInfo.excl
	elseif(i_gid >= 9000001 and i_gid <= 10000000)then
		-- 专属宝物碎片
		i_data_t = remoteBagInfo.exclFrag
	elseif(i_gid >= 10000001 and i_gid <= 11000000)then
		-- 觉醒装备
		i_data_t = remoteBagInfo.awake
	end
	...
	if (not table.isEmpty(i_data_t)) then
		for r_gid, r_data in pairs(i_data_t) do
			if ( tonumber(r_gid) == i_gid) then
				...
				elseif (i_gid >= 4000001 and i_gid < 5000000) then -- 影子
				...
				elseif( i_gid >= 8000001 and i_gid <= 9000000 )then
					-- 专属宝物
					remoteBagInfo.excl = i_data_t
				elseif(i_gid >= 9000001 and i_gid <= 10000000)then
					-- 专属宝物碎片
					remoteBagInfo.exclFrag = i_data_t
				elseif(i_gid >= 10000001 and i_gid <= 11000000) then 
					-- 觉醒装备
					remoteBagInfo.awake = i_data_t
				end
		...
		--专属宝物
		if( isFind == false and not table.isEmpty(remoteBagInfo.excl))then
			-- 是不是装备碎片
			for r_gid, r_data in pairs(remoteBagInfo.excl) do
				if( tonumber(r_gid) == i_gid) then
					if(isForceDel == true)then
						remoteBagInfo.excl[r_gid] = nil
					else
						if ( tonumber(r_data.item_num) <= i_num)then
							remoteBagInfo.excl[r_gid] = nil
						else
							remoteBagInfo.excl[r_gid].item_num = tonumber(r_data.item_num) - i_num
						end
					end
					isFind = true
					DataCache.setBagInfo(remoteBagInfo)
		...
		if( isFind == false and not table.isEmpty(remoteBagInfo.exclFrag))then
			-- 是不是装备碎片
			for r_gid, r_data in pairs(remoteBagInfo.exclFrag) do
				if( tonumber(r_gid) == i_gid) then
					if(isForceDel == true)then
						remoteBagInfo.exclFrag[r_gid] = nil
					else
						if ( tonumber(r_data.item_num) <= i_num)then
							remoteBagInfo.exclFrag[r_gid] = nil
						else
							remoteBagInfo.exclFrag[r_gid].item_num = tonumber(r_data.item_num) - i_num
						end
					end
					isFind = true
					DataCache.setBagInfo(remoteBagInfo)
		...
		if( isFind == false and not table.isEmpty(remoteBagInfo.awake))then
			-- 是不是觉醒装备
			for r_gid, r_data in pairs(remoteBagInfo.awake) do
				if( tonumber(r_gid) == i_gid) then
					if(isForceDel == true)then
						remoteBagInfo.awake[r_gid] = nil
					else
						if ( tonumber(r_data.item_num) <= i_num)then
							remoteBagInfo.awake[r_gid] = nil
						else
							remoteBagInfo.awake[r_gid].item_num = tonumber(r_data.item_num) - i_num
						end
					end
					isFind = true
					DataCache.setBagInfo(remoteBagInfo)
...
function getEquipValueByIID( i_id)
	...
	-- 别人身上的装备
	if(table.isEmpty(equipData))then
		logger:debug({_tbequip = _tbequip})
		equipData = _tbequip
	...
	if( not table.isEmpty(equipData)) then
		...
		--装备提供的属性= {int[装备的属性基础值+（强化等级*强化1级增加的属性/100)]+(附魔属性成长*附魔等级)} by青云
		-- local enchantFix = 1 + enchantLevel * ( 0)/ 10000
		--附魔升级带来的属性
		...
		--附魔进阶带来的属性
		local enchantAdventAffix = getEquipAdvancedAffix(equipData)
		-- if(enchantValueUnLocked.hp > 0) then
		-- 	logger:debug(enchantValueUnLocked.hp)
		-- 	logger:debug(enchantAdventAffix)
		-- end

		-- logger:debug(enchantFix)
		-- 生命值
		t_numerial.hp 		  = math.floor(math.floor(equip_desc.baseLife + forceLevel* equip_desc.lifePL/100)  +  enchantValueUnLocked.hp +enchantAdventAffix.hp + 0.9999) 
		-- 通用攻击
		-- t_numerial.gen_att	  = math.floor(equip_desc.baseGenAtt + forceLevel* equip_desc.genAttPL/100)
		-- 物攻
		t_numerial.physical_attack	 = math.floor(math.floor(equip_desc.basePhyAtt + forceLevel* equip_desc.phyAttPL/100) + enchantValueUnLocked.phy_att+ enchantAdventAffix.phy_att +0.9999) 
		--	t_numerial.hp 		  = math.floor((equip_desc.baseLife + forceLevel* equip_desc.lifePL/100) * enchantValueUnLocked.hp)
		-- 魔攻
		t_numerial.magic_attack  = math.floor(math.floor(equip_desc.baseMagAtt + forceLevel* equip_desc.magAttPL/100) + enchantValueUnLocked.magic_att+ enchantAdventAffix.magic_att +0.9999) 
		-- 物防
		t_numerial.physical_defend 	  = math.floor(math.floor(equip_desc.basePhyDef + forceLevel* equip_desc.phyDefPL/100) + enchantValueUnLocked.phy_def+ enchantAdventAffix.phy_def +0.9999) 
		-- 魔防
		t_numerial.magic_defend  = math.floor(math.floor(equip_desc.baseMagDef + forceLevel* equip_desc.magDefPL/100) + enchantValueUnLocked.magic_def+ enchantAdventAffix.magic_def +0.9999) 
		t_numerial.percentHP = enchantAdventAffix.percentHP
		t_numerial.percentPhyAtt = enchantAdventAffix.percentPhyAtt
		t_numerial.percentMagAtt = enchantAdventAffix.percentMagAtt
		t_numerial.percentPhyDef = enchantAdventAffix.percentPhyDef
		t_numerial.percentMagDef = enchantAdventAffix.percentMagDef
	end
	logger:debug({t_numerial= t_numerial})
	return t_numerial
end
--[[
	获得装备的各种数值 
...
function getEquipNumerialByIID( i_id ,_tbequip)
	...
	if( not table.isEmpty(equipData)) then
		...
		-- local enchantPercent = 1 + enchantLevel * ( 0)/ 10000
		--附魔解锁带来的属性
		local enchantLockFix,enchantValueUnLocked = getEquipLockEnchantAffixBy(equipData)
		-- 生命值
		...
		-- 生命值
		-- t_numerial.hp 		  				= math.floor(t_numerial_noEnchant.hp  +  enchantValueUnLocked.hp + 0.9999) 
		t_numerial.hp 		  				= math.floor(t_numerial_noEnchant.hp  +  enchantValueUnLocked.hp + 0.9999) 
		-- 通用攻击
		-- 物攻
		t_numerial.phy_att	  				= math.floor(t_numerial_noEnchant.phy_att + enchantValueUnLocked.phy_att+0.9999) 
		-- 魔攻
		t_numerial.magic_att 				= math.floor(t_numerial_noEnchant.magic_att + enchantValueUnLocked.magic_att + 0.9999) 
		-- 物防
		t_numerial.phy_def 	  				= math.floor(t_numerial_noEnchant.phy_def + enchantValueUnLocked.phy_def + 0.9999) 
		-- 魔防
		t_numerial.magic_def  				= math.floor(t_numerial_noEnchant.magic_def +  enchantValueUnLocked.magic_def + 0.9999) 
		...
		local enchantAffixValue = ( 0)
		-- logger:debug(enchantValueUnLocked)
		if(enchantValueUnLocked.hp ~= 0)then
			t_numerial_enchantPL.hp						= enchantValueUnLocked.hp 
		else
			t_numerial_enchantPL.hp	 = nil
		end		

		if(enchantValueUnLocked.phy_att ~= 0)then
			t_numerial_enchantPL.phy_att				= enchantValueUnLocked.phy_att 
		else
			t_numerial_enchantPL.phy_att	 = nil
		end

		if(enchantValueUnLocked.magic_att ~= 0)then
			t_numerial_enchantPL.magic_att				= enchantValueUnLocked.magic_att 
		else
			t_numerial_enchantPL.magic_att	 = nil
		end

		if(enchantValueUnLocked.phy_def ~= 0)then
			t_numerial_enchantPL.phy_def				= enchantValueUnLocked.phy_def 
		else
			t_numerial_enchantPL.phy_def	 = nil
		end

		if(enchantValueUnLocked.magic_def ~= 0)then
			t_numerial_enchantPL.magic_def				= enchantValueUnLocked.magic_def 
		else
	...
	return t_numerial, t_numerial_pl, t_equip_score, equipData, t_numerial_enchantPL, t_numerial_noEnchant
end


--[[
	modified: yangna, 只获取t_equip_score字段，用于简化装备数据初始化
--]]
function getEquipNumerialByIID_score( i_id ,_tbequip)
	i_id = tonumber(i_id)
	assert(i_id, "not found item_id:" .. i_id)
...
function getConchFromHeroByItemId( itemId )
	local arrConchs = getConchesOnFormation()
...
--获得装备的附魔解锁属性 unLockedAttrBaseValue附魔升级带来的属性（只影响战斗力）   ext_active所有属性 
function getEquipLockEnchantAffixBy(equipData)
	--解锁属性
	local  addEnchantAffix = equipData.itemDesc.enchantAffix
	-- logger:debug({addEnchantAffix = addEnchantAffix})

	local tbAffixInfo  = mSplit(addEnchantAffix,",") or {}

	local curEnchatLv = equipData.va_item_text.armEnchantLevel or 0
	-- logger:debug({curEnchatLv = curEnchatLv})
	-- 所有的解锁属性
	local ext_active = {}
	--已解锁的属性
	local unLockedAttrBaseValue = { hp = 0, phy_att = 0 , magic_att = 0, phy_def = 0, magic_def = 0,}
	for k, str_act in pairs(tbAffixInfo) do
		local temp_act_arr = mSplit(str_act, "|")
		local t_ext_active = {}
		t_ext_active.attId 	= tonumber(temp_act_arr[1])
		t_ext_active.value = tonumber(temp_act_arr[2])
			for k,v in pairs(g_LockAttrNameConfig) do
				if(t_ext_active.attId == tonumber(k)) then
					unLockedAttrBaseValue[v] = t_ext_active.value * curEnchatLv
					break
				end
			end
	...
	-- logger:debug({ext_active = ext_active})
	logger:debug({unLockedAttrBaseValue = unLockedAttrBaseValue})
	return ext_active ,unLockedAttrBaseValue
end

--获取装备的进阶基础属性 用于计算战斗力
function getEquipAdvancedAffix(equipData)
	--解锁属性
	local  addEnchantAffix = equipData.itemDesc.addEnchantAffix
	-- logger:debug({addEnchantAffix = addEnchantAffix})
	local lockInfo = mSplit(addEnchantAffix,",")
	-- local tbAffixInfo  = mSplit(addEnchantAffix,",") or {}
	local curEnchatLv = tonumber(equipData.va_item_text.armEnchantLevel or 0)
	...
	for i,v in ipairs(lockInfo) do
		...
		local unLockAttrId 		= tonumber(tbEachInfo[2])
		local unLockAttrValue 	= tbEachInfo[3]
		if(curEnchatLv >= unLockLevel )then
			for k,vv in pairs(g_LockAttrNameConfig) do

				if(unLockAttrId == tonumber(k)) then
					unLockedAttrBaseValue[vv] = tonumber(unLockAttrValue) + (unLockedAttrBaseValue[vv] or 0)
					logger:debug(unLockedAttrBaseValue)
				end
			end
		end
	end
	logger:debug({unAdvenAttrBaseValue = unLockedAttrBaseValue})
	return unLockedAttrBaseValue
end
...
function getItemInfoByItemId( i_id )
	...
	-- 判断觉醒宝物背包
	for g_id, item_data in pairs(allBagInfo.awake) do
		if (i_id == tonumber(item_data.item_id)) then
			return item_data
		end
	end

	if(table.isEmpty(allBagInfo.excl) == false)then
		for g_id, item_data in pairs(allBagInfo.excl) do
			if(i_id == tonumber(item_data.item_id)) then
				return item_data
			end
		end
	end
	if(table.isEmpty(allBagInfo.exclFrag) == false)then
		for g_id, item_data in pairs(allBagInfo.exclFrag) do
			if(i_id == tonumber(item_data.item_id)) then
				return item_data
			end
		end
		...
		tbInfo.btn = {
			{
				...
				callback = 	function ( ... )
					...
					MainTreaBagCtrl.onExpand(1)
				end
			},
			{
				title = m_i18n[1748], -- 2016-02-23, 修改提示为 "强化饰品"
				callback = 	function ( ... )
					require "script/module/treasure/MainTreaBagCtrl"
					if (not LayerManager.isRunningModule(MainTreaBagCtrl)) then
						...
						-- zhangqi, 2016-02-24, 饰品碎片列表提示背包满时切换到饰品列表
						MainTreaBagCtrl.changeTabWithIndex(1)
					end
				end
			}
		}
...
-- 专属宝物背包是否已满, 2015-09-18, liweidong
function isSpecialTreasBagFull(isShowAlert)
	logger:debug("isSpecialTreasBagFull")
	logger:debug(isShowAlert)
	local isFull = false
	local allBagInfo = DataCache.getRemoteBagInfo()
	local m_number = 0
	...
	if( not table.isEmpty(allBagInfo))then
		-- 装备是否满了
		if(not table.isEmpty(allBagInfo.excl)) then
			for k,v in pairs(allBagInfo.excl) do
				m_number = m_number + 1
			end
		end
		if( m_number >= tonumber(allBagInfo.gridMaxNum.excl)) then
			isFull = true
		end
	end
	if(isFull==true and isShowAlert==true)then
		logger:debug(isShowAlert)
		local tbInfo = {text = m_i18n[6901]}
		tbInfo.btn = {
			{
				title = m_i18n[1525],
				callback = 	function ( ... )
					require "script/module/specialBag/SBListCtrl"
					SBListCtrl.onExpand(1)
				end
			},
			{
				title = m_i18n[1526],
				callback = 	function ( ... )
					if (LayerManager.curModuleName() ~= "SBListCtrl") then
						require "script/module/specialBag/SBListCtrl"
						SBListCtrl.create()
					else
						require "script/module/specialBag/SBListCtrl"
...
-- 专属宝物碎片背包是否已满, 2015-09-18, liweidong
function isSpecialTreasFragBagFull(isShowAlert)
	local isFull = false
	local allBagInfo = DataCache.getRemoteBagInfo()
	local m_number = 0
	...
	if( not table.isEmpty(allBagInfo))then
		-- 装备是否满了
		if(not table.isEmpty(allBagInfo.exclFrag)) then
			for k,v in pairs(allBagInfo.exclFrag) do
				m_number = m_number + 1
			end
		end
		if( m_number >= tonumber(allBagInfo.gridMaxNum.exclFrag)) then
			isFull = true
		end
	end
	if(isFull==true and isShowAlert==true)then
		local tbInfo = {text = m_i18n[6903]}
		tbInfo.btn = {
			{
				title = m_i18n[1525],
				callback = 	function ( ... )
					require "script/module/specialBag/SBListCtrl"
					SBListCtrl.onExpand(2)
				end
			},
			{
				title = m_i18n[1526],
...
-- 觉醒物品背包是否已满 2015-11-18 lvnanchun 					-- TODO
function isAwakeBagFull( isShowAlert )
	local isFull = false
	local allBagInfo = DataCache.getRemoteBagInfo()
	local m_number = 0
	...
	if( not table.isEmpty(allBagInfo))then
		-- 装备是否满了
		if(not table.isEmpty(allBagInfo.awake)) then
			for k,v in pairs(allBagInfo.awake) do
				m_number = m_number + 1
			end
		end
		if( m_number >= tonumber(allBagInfo.gridMaxNum.awake)) then
			isFull = true
		end
	end
	if(isFull==true and isShowAlert==true)then
		local tbInfo = {text = m_i18n[7421]}
		tbInfo.btn = {
			{
				title = m_i18n[1525],
				-- 扩充按钮回调
				callback = 	function ( ... )
					AwakeBagCtrl.fnExpandCallBack()
				end
			},
			{
				title = m_i18n[1526],
...
function isBagFull(isShowAlert)
	...
	isFull = isAwakeBagFull(isShowAlert) or isSpecialTreasBagFull(isShowAlert) or isSpecialTreasFragBagFull(isShowAlert) or isPropBagFull(isShowAlert) or isPartnerFull(isShowAlert) or isEquipBagFull(isShowAlert) or isTreasBagFull(isShowAlert) or isArmFragBagFull(isShowAlert) or isConchBagFull(isShowAlert)
	return isFull
end
...
function isBagFullExPartner(isShowAlert)
	local isFull = false
	isShowAlert = isShowAlert or true
	isFull = isAwakeBagFull(isShowAlert) or isSpecialTreasBagFull(isShowAlert) or isSpecialTreasFragBagFull(isShowAlert) or  isPropBagFull(isShowAlert) or isEquipBagFull(isShowAlert) or isTreasBagFull(isShowAlert) or isArmFragBagFull(isShowAlert)
	return isFull
end
...
function getCacheItemInfoBy( item_template_id )
	...
	if( not table.isEmpty(allBagInfo)) then
		...
		if(item_info==nil and not table.isEmpty( allBagInfo.armFrag)) then
		...
		if(item_info==nil and not table.isEmpty( allBagInfo.treas)) then
		...
		if(item_info==nil and not table.isEmpty( allBagInfo.excl)) then
			for k,item_info in pairs( allBagInfo.excl) do
				if(tonumber(item_info.item_template_id) == item_template_id) then
					cacheItemInfo = item_info
					cacheItemInfo.gid = k
				end
		...
		if(item_info==nil and not table.isEmpty( allBagInfo.exclFrag)) then
			for k,item_info in pairs( allBagInfo.exclFrag) do
				if(tonumber(item_info.item_template_id) == item_template_id) then
					cacheItemInfo = item_info
					cacheItemInfo.gid = k
				end
		...
		-- 觉醒物品背包
		if(item_info==nil and not table.isEmpty( allBagInfo.awake)) then
			for k,item_info in pairs( allBagInfo.awake) do
				if(tonumber(item_info.item_template_id) == item_template_id) then
					cacheItemInfo = item_info
					cacheItemInfo.gid = k
				end
...
function getCacheItemNumBy( item_template_id )
	...
	if( not table.isEmpty(allBagInfo)) then
		...
		if(item_num<=0 and not table.isEmpty( allBagInfo.excl)) then
			for k,item_info in pairs( allBagInfo.excl) do
				if(tonumber(item_info.item_template_id) == item_template_id) then
					item_num = item_num + tonumber(item_info.item_num)
				end
		...
		if(item_num<=0 and not table.isEmpty( allBagInfo.exclFrag)) then
			for k,item_info in pairs( allBagInfo.exclFrag) do
				if(tonumber(item_info.item_template_id) == item_template_id) then
					item_num = item_num + tonumber(item_info.item_num)
				end
		...
		if(item_num<=0 and not table.isEmpty( allBagInfo.awake)) then
			for k,item_info in pairs( allBagInfo.awake) do
				if(tonumber(item_info.item_template_id) == item_template_id) then
					item_num = item_num + tonumber(item_info.item_num)
				end
	...
	logger:debug("item_template_id = %s, number = %s", item_template_id, item_num)
	return item_num
end
			...
			if(tonumber(f_hid)>0)then
				...
				if( (not table.isEmpty(f_hero)) and (not table.isEmpty(f_hero.equip.treasure)) ) then
					for p, equipInfo in pairs(f_hero.equip.treasure) do
						if( not table.isEmpty(equipInfo)) then
							equipInfo.itemDesc = getItemById(equipInfo.item_template_id)
							equipInfo.itemDesc.desc = equipInfo.itemDesc.info
							equipInfo.equip_hid =  tonumber(f_hid)
							table.insert(equipsInfo_t, equipInfo)
						end
...
-- 获取阵容上的专属宝物
function getSpecialOnFormation( ... )
	...
	if( not table.isEmpty(formationInfo))then
		for k,f_hid in pairs(formationInfo) do
			if(tonumber(f_hid)>0)then
				local f_hero = HeroModel.getHeroByHid(f_hid)
				if( (not table.isEmpty(f_hero)) and (not table.isEmpty(f_hero.equip.exclusive)) ) then
					for p, equipInfo in pairs(f_hero.equip.exclusive) do
						if( not table.isEmpty(equipInfo)) then
							equipInfo.itemDesc = getItemById(equipInfo.item_template_id)
							equipInfo.itemDesc.desc = equipInfo.itemDesc.info
							equipInfo.equip_hid =  tonumber(f_hid)
							--table.insert(equipsInfo_t, equipInfo)
							equipsInfo_t[k + 1] = equipInfo
						end
					end
				end
			end
				...
				if( (not table.isEmpty(f_hero)) and (not table.isEmpty(f_hero.equip.exclusive)) ) then
					for p, equipInfo in pairs(f_hero.equip.exclusive) do
						if( not table.isEmpty(equipInfo)) then
							...
							equipInfo.equip_hid =  tonumber(f_hid)
							--table.insert(equipsInfo_t, equipInfo)
							local allSpecTreaNum = MainFormationTools.fnGetSquadNum() + 1 -- table.count(equipsInfo_t) + 1 -- 
							equipsInfo_t[allSpecTreaNum] = equipInfo
						end
					end
				end
			end
	...
	--table.sort( equipsInfo_t, BagUtil.treasSort )
	-- logger:debug({equipsInfo_t = equipsInfo_t})
	return equipsInfo_t
end

-- zhangqi, 2015-09-25, 获取阵容上的空岛贝
function getConchesOnFormation()
	local allConch = {}
			...
			if( tonumber(hid) > 0 )then
				...
				if(not table.isEmpty(conch))then
					for k,v in pairs(conch) do
						tbConchs[k] = v
						tbConchs[k].itemDesc = getItemById(v.item_template_id)
...
-- zhangqi, 2015-09-25, 获得某个伙伴身上的空岛贝
...
function getConchOnFormationExeptHid( hid )
	...
	local conchOnHeros = getConchesOnFormation()
...
-- zhangqi, 2015-09-16, 只计算宝物的评级，构造背包数据时优化计算
function getTreaScoreByData( treasData )
	local treasInfo = getItemById(treasData.item_template_id)
	...
	if (enhanceLv and enhanceLv >= 0) then
		score_t.num = score_t.base+score_t.pl*enhanceLv -- 计算评分
	end
...
function getTreasAttrByTmplId( tmpl_id )
	...
	active_arr_1 = active_arr_1[1] and {active_arr_1[1]} or {}
...
-- zhangqi, 2015-12-26, 解析一个配置表中取出的Affix属性字符串（例如 "2|21,3|21,21|60"）
-- 返回 array 型 table, 每个元素存放一个属性名和基本属性值
function parseAffixString( strAffixAttr, prefix )
	local tbRet = {}
	local tbAttrs = mSplit(strAffixAttr, ",")
	logger:debug(tbAttrs)
	for i, attr in ipairs(tbAttrs) do
		local tbAffix = mSplit(attr, "|")
		local tbValue = {}
		tbValue.name, tbValue.num, tbValue.realNum = getAttrNameAndValueDisplay(tbAffix[1], tbAffix[2], prefix)
		tbRet[#tbRet + 1] = tbValue
	end
...
-- 物品属性的名称和数值的显示
-- zhangqi, 2015-11-13, 增加第3个默认可为nil的参数precision，表示显示数值的精度（默认是1位小数）
function getAtrrNameAndNum( attrId, num, precision )
	local preci = precision or 1
	local preciFmt = "%." .. preci .. "f"
	...
	elseif(affixDesc.type == 2)then
		...
		if(displayNum > math.floor(displayNum))then
			displayNum = string.format(preciFmt, displayNum)
		end
...
--[[desc:zhangqi, 2015-11-13, 二次封装 getAtrrNameAndNum, 加入数值的修饰，例如 “+”
    attrId: 对应Affix表的id;	num: 属性数值;	num_prefix: 数值的前置修饰字符
    return: 是否有返回值，返回值说明  
—]]
function getAttrNameAndValueDisplay( attrId, num, num_prefix )
	local dbAffix, displayNum, realNum = getAtrrNameAndNum(attrId, num, 2) -- 2 表示精确到小数点后2位数字
	if (num_prefix and tonumber(num) == tonumber(displayNum)) then
		displayNum = num_prefix .. displayNum
	end
	return dbAffix.displayName, displayNum, realNum
end
...
function createBtnByTemplateIdWithLevel( itemInfo, fnOnTouch , type)
	...
	elseif(type == 4) then
		pLevel = itemInfo.va_item_text.exclusiveEvolve
	end
	...
	numLabel:setText(pLevel)
	numLabel:setFontSize(18)
	numLabel:setFontName(g_sFontCuYuan)
	...
	local qulityColor = {ccc3(0x34,0x34,0x34),
		ccc3(0x34,0x34,0x34),
		ccc3(0x02,0x2b,0x02),
		ccc3(0x0e,0x01,0x5a),
		ccc3(0x36,0x00,0x40),
	...
	local shadowColor = {ccc3(0xda,0xda,0xda),
		ccc3(0xda,0xda,0xda),
		ccc3(0xc3,0xef,0xb1),
		ccc3(0xb3,0xc6,0xff),
		ccc3(0xff,0xd3,0xfe),
	...
	numLabel:setColor(qulityColor[tbItem.quality])
	--UIHelper.labelNewStroke(numLabel, shadowColor[tbItem.quality], 2)
...
function createBtnByItemAndNum( tbItem,number,fnOnTouch )
	...
	addNumLabel(btnItem, {num = number}) -- zhangqi, 2015-06-19
	return btnItem
end

-- addBy Xufei 2015-11-26 带num1/num2标记
function createBtnByItemAndNumCondition( tbItem,number1,number2,fnOnTouch )
	local btnItem = createBtnByItem(tbItem, fnOnTouch)
	
	local function addNumLabelWithColor( text ,textColor )
		local label = Label:create()
		UIHelper.setLabel(label, {font = g_FontCuYuan, size = 18, color = textColor or ccc3(0xff, 0xff, 0xff)})
		UIHelper.labelAddNewStroke(label, text, ccc3(0x28, 0x00, 0x00))
	...
	if (number1 and number2) then
		...
		if (number1<number2) then
			textColor = ccc3(0xff, 0x00, 0x00)
		end
		local label1 = addNumLabelWithColor( "/"..number2 )
		label1:setPosition(ccp(btnItem:getContentSize().width*.53,-btnItem:getContentSize().height*.53))
		btnItem:addChild(label1)

		local label2 = addNumLabelWithColor( number1 , textColor)
		label2:setPosition(ccp(btnItem:getContentSize().width*.53-label1:getContentSize().width,-btnItem:getContentSize().height*.53))
		btnItem:addChild(label2)
	end
	return btnItem
end
...
-- zhangqi, 2015-06-16, 数字由数字标签替换为普通Label加描边效果
-- zhangqi, 2015-12-24, 修改数字角标的显示方式，超过6位简化为 XX万 显示
function addNumLabel( parent, tbArgs )
	local label = Label:create()
	-- UIHelper.setLabel(label, {font = g_FontCuYuan, size = 18, color = ccc3(0xff, 0xff, 0xff)})
	-- UIHelper.labelAddNewStroke(label, tbArgs.num, ccc3(0x28, 0x00, 0x00))
	-- zhangqi, 2015-12-23, 调整数字角标字体大小和描边颜色
	UIHelper.setLabel(label, {font = g_FontCuYuan, size = 24, color = ccc3(0xff, 0xff, 0xff)})
	UIHelper.labelAddNewStroke(label, UIHelper.longToSixNum(tbArgs.num), ccc3(0x87, 0x2e, 0x11))
	label:setAnchorPoint(tbArgs.anchor or ccp(1,0))
	label:setPosition(ccp(parent:getContentSize().width*.53,-parent:getContentSize().height*.53))
...
function createBtnByItem( tbItem, fnOnTouch ,tbPicSwithContr)
	...
	if tbPicSwithContr == nil or tbPicSwithContr.hideArticleContr == nil  then
		if (tbItem.isTreasureFragment == true or tbItem.isFragment == true or tbItem.isSpeTreasureFragment == true) then
			addSuiSignToWidget(btnItem)
		end
	end
	...
	-- zhangqi, 2015-10-14, 暂时注释掉套装按钮边框效果
...
local function createDirectItem( tbArgs )
	...
	if(tbArgs.number) then
		...
		-- zhangqi, 2015-06-16, 数字由数字标签替换为普通Label加描边效果
		addNumLabel(icomBorder, {num = tbArgs.number})
	end
	iconImgV:setTag(1)
...
--[[desc:zhangjunwu 创建一个带数目的觉醒结晶图标
    arg1: j结晶数目
    return: imgview  
—]]
function getEquipAwakeIconByNum(number)
	return createDirectItem({quality = 5, iconFile = "icon_disillusion_coin.png", number = number})
end

--[[desc:zhangjunwu 创建一个带等级的经验钻石的图标
    arg1: j结晶数目
    return: imgview  
—]]
function getTreasExpIconByLV(tid, _level)
	local tbItem = assert(getItemById(tonumber(tid)), "createBtnByTemplateId--not found id: " .. tid)


	local potentialImgV= ImageView:create()
	potentialImgV:loadTexture(string.format("images/base/potential/color_%d.png", tbItem.quality))
	local iconImgV  = ImageView:create()
	iconImgV:loadTexture(tbItem.imgFullPath)
	local icomBorder = ImageView:create()
	icomBorder:loadTexture(string.format("images/base/potential/equip_%d.png", tbItem.quality))

	local label = Label:create()
	UIHelper.setLabel(label, {font = g_FontCuYuan, size = 24, color = ccc3(0xff, 0xff, 0xff)})
	UIHelper.labelAddNewStroke(label, "Lv." .. _level, ccc3(0x87, 0x2e, 0x11))
	label:setAnchorPoint(ccp(1,0))
	label:setPosition(ccp(icomBorder:getContentSize().width*.53,-icomBorder:getContentSize().height*.53))
	icomBorder:addChild(label)
	iconImgV:setTag(1)
	potentialImgV:addChild(iconImgV)
	potentialImgV:addChild(icomBorder)  

	return potentialImgV,tbItem
end

--[[desc:huxiaozhou 创建一个带数目的贡献度图标
    arg1: 贝里数量
    return: imgview  
—]]
function getContriIconByNum(number)
	return createDirectItem({quality = 5, iconFile = "contribute.png", number = number})
end
...
--[[desc:zhangjunwu 创建一个带数目的宝物结晶图标
    arg1:
    return: imgview  
—]]
function getRimeIconByNum(number)
	return createDirectItem({quality = 5, iconFile = "icon_treasure_coin.png", number = number})
end
...
--[[desc:lvnanchun 创建一个周礼包的图标
    arg1: 礼包品级，图标和数量
    return: imgview
—]]
function getVipGiftWeekReward(quality , filePath , number )
	return createDirectItem({quality = quality, iconFile = filePath, number = number})
end

--[[desc:lvnanchun 创建一个监狱币（装备结晶）图标
    arg1: 装备结晶数量
    return: imgView
—]]
function getPrisonGoldIconByNum( number )
	return createDirectItem({quality = 5, iconFile = "icon_impel_down_coin.png", number = number})
end

--[[desc:sunyunpeng 创建一个英雄图标
    arg1:英雄的hitd
    return: imgview  
—]]
function getHeroIcon(herotid,fnOnTouch)
	local heroDB = DB_Heroes.getDataById(herotid)
	local btnItem = Button:create()

	local bgFullPath = "images/base/potential/color_" .. heroDB.potential .. ".png"
	local borderFullPath = "images/base/potential/officer_" .. heroDB.potential .. ".png"

	btnItem:loadTextures(bgFullPath, bgFullPath, nil) -- 用品质边框初始化按钮
	local imgItem = ImageView:create()
	local imgFullPath = "images/base/hero/head_icon/" .. heroDB.head_icon_id
...
tbEquip[2] = "item_type_shoes.png" 
tbEquip[3] = "item_type_helmet.png"
tbEquip[4] = "item_type_armor.png"
...
-- zhangqi, 2015-09-24, 修改物品类型item_type和文字的对应如下
-- 2改为宝物，之前是空，4改为碎片，之前是道具，8改为道具，之前时碎片，11改为饰品，之前是宝物，2015-09-24
local tbSignText = {tbEquipType, tbTreasureType, 1136, 1137, -- 食品先默认道具                                                
                    1137, 1136, 1138, 1136, 1139, 1136,                                                           
                    "【饰品】", 1137, tbConchType, 1136, } -- 最后一个时装先默认道具
...
function getGiftInfo( item_template_id )
	local itemTableInfo = getItemById(tonumber(item_template_id))
	...
	for k,v in pairs(awardItemIds) do
		...
		item.type = "item"
		item.name = getItemById(tonumber(item.tid)).name -- zhangqi, 增加物品名称
		logger:debug(item)
...
function getGiftData( tbGift )
	...
	local signPath = m_i18n[1136] -- zhangqi, 2015-09-16, 改文字，默认道具
	if (tbGift.type == "item") then
		tbItem = getItemById(tbGift.tid)
		btnIcon = createBtnByItem(tbItem)
		assert(btnIcon, "UIHelper.createDropItemDlg by item.tid: " .. tbGift.tid)
		-- signPath = getSealFileByItem(tbItem)
		signPath = getSignTextByItem(tbItem)
	...
	if(bagInfo)then
		...
		if(not table.isEmpty(armBag)) then
			for mgid, m_arm in pairs(armBag) do
				...
				local pLv = tonumber(m_arm.va_item_text.armReinforceLevel)
		...
		if(not table.isEmpty(conchBag)) then
			for mgid, m_arm in pairs(conchBag) do
				local tempItem = {mid=0,score=0}
				tempItem.mid = m_arm.item_id
				local pLv = tonumber(m_arm.va_item_text.level)
				tempItem.lv = pLv or 0
...
-- add by yucong
-- 修改红点背包的物品等级(目前只有宝物使用)
local function setRedBagItemLV( itemInfo, itemType )
	if(not table.isEmpty(itemInfo)) then
		local tempItem = {}
		local mtid = itemInfo.item_template_id
		local itemDict = fnGetItemDict(itemInfo, itemType)
		local tempid = itemInfo.item_id
		local mtype = itemDict.type -- 四种类型之一
		...
		for i = 1 , itemnum do
			...
			if tonumber(tempid) == tonumber(mmid) then
				-- 当前只有宝物红点与等级变化有关
				if (itemType == 1) then
					logger:debug("yc--修改了装备等级")
					tabItem[tonumber(i)].lv = tonumber(itemInfo.va_item_text.armReinforceLevel)
					armPos[mtype] = tabItem
				elseif (itemType == 2) then
					logger:debug("yc--修改了宝物等级")
					tabItem[tonumber(i)].lv = tonumber(itemInfo.va_item_text.treasureLevel)
					treaPos[mtype] = tabItem
				elseif (itemType == 3) then
					logger:debug("yc--修改了空岛贝等级")
					tabItem[tonumber(i)].lv = tonumber(itemInfo.va_item_text.level)
					conchTypes[mtype] = tabItem
				end
				break
			end
		end
...
function pushitemCallback( itemInfo, itemType )
	...
	logger:debug({pushitemCallback = itemInfo})
	if(not table.isEmpty(itemInfo)) then
		local exit = justiceItemExit(itemInfo, itemType)
		if exit then
			--如果物品已经存在就直接可以结束
			--logger:debug("yc--物品已经存在")
			setRedBagItemLV(itemInfo, itemType)
			return
		end
		...
		local pLv = 0
		if (itemType == 1) then
			pLv = tonumber(itemInfo.va_item_text.armReinforceLevel)
		elseif (itemType == 2) then
			pLv = tonumber(itemInfo.va_item_text.treasureLevel)
		elseif (itemType == 3) then
			pLv = tonumber(itemInfo.va_item_text.level)
		end
		tempItem.lv = pLv or 0
		local mtype  	= itemDict.type
...
local function judgeShowByHid( sq_hid, pOpenConchPos, pOpenTreaPos, onlyID, uniconIDs)
	...
	for apos = 1, 4 do
		if(apos <= mOpenTreaPos) then
			...
			pShowRed = isTreasureCanRefine(pTrea)
			if(pShowRed) then
				return pShowRed
			end
		end
	end
	if(SwitchModel.getSwitchOpenState(ksSwitchBattleSoul, false)) then
		for apos = 1, 6 do
	...
	-- 觉醒判断
	if (SwitchModel.getSwitchOpenState(ksSwitchAwake)) then
		pShowRed = MainAwakeModel.isCanAwakeByHid(sq_hid)
		if (pShowRed) then
			return pShowRed
		end
	end
	-- 宝物判断，是否有穿戴、是否可进阶
	pShowRed = FormationSpecialModel.isShowTip(tonumber(sq_hid))
	if (pShowRed) then
		return pShowRed
	end
	return pShowRed
...
function fnUpdateOneHeroRedByPos( pos )
	...
	local tbCanTransTips = {false,false,false,false,false,false,false,false,}
	local pShowRed = false
	--if(gPos < 5) then
	if (gPos < MainFormationTools.fnGetSquadNum()) then
		...
		for ppos,sq_hid in pairs(mSquad) do
			...
			if(pNum == gPos) then
				...
				pNum = pNum + 1
				heroRedTips[pNum] = judgeShowByHid(sq_hid, pOpenConchPos, pOpenTreaPos, pOnly, pUnion) or mMainFormation.JudgeBondRedPoint(pNum) or false
				tbCanTransTips[pNum] = PartnerTransUtil.checkPartnerCanTransByHid(tonumber(sq_hid))
	...
	if (not table.isEmpty(tbCanTransTips)) then
		for k,v in pairs(tbCanTransTips) do
			heroRedTips[k] = heroRedTips[k] or v
			if (v) then
				pShowRed = true
			end
		end
	...
	MainScene.updateFormTip(pShowRed)
end
...
function justiceBagInfo( ... )
	...
	require "script/module/formation/MainFormation"
	local mMainFormation = MainFormation
	local tbBondRedPoint = mMainFormation.JudgeBondRedPoint()
	local tbSpecialTips = {false,false,false,false,false,false,false,false,}
	local tbCanTransTips = {false,false,false,false,false,false,false,false,}
	...
	if(not table.isEmpty(mSquad)) then
		for pos,sq_hid in pairs(mSquad) do
			...
			-- tbSpecialTips[pNum] = FormationSpecialModel.isShowTip(tonumber(sq_hid))
			tbCanTransTips[pNum] = PartnerTransUtil.checkPartnerCanTransByHid(tonumber(sq_hid))
			if(heroRedTips[pNum]) then
				pShowRed = true
			end
	...
	if (not table.isEmpty(tbBondRedPoint)) then
		for k,v in pairs(tbBondRedPoint) do
			heroRedTips[k] = heroRedTips[k] or v
			if (v) then
				pShowRed = true
			end
	...
	-- if (not table.isEmpty(tbSpecialTips)) then
	-- 	for k,v in pairs(tbSpecialTips) do
	-- 		heroRedTips[k] = heroRedTips[k] or v
	-- 		if (v) then
	-- 			pShowRed = true
	-- 		end
...
function justiceTreaInfo( treaInfo, treaTable, onlyID, uniconIDs)
	...
	--首先判断功能节点，宝物功能节点没开则不显示红点
	if (not SwitchModel.getSwitchOpenState(ksSwitchTreasure,false)) then
		return isShowRed
	end
...
function isTreasureCanRefine( treaInfo )
	if(not treaInfo or tonumber(treaInfo) == 0 or table.isEmpty(treaInfo)) then
		return false
	end
	-- 饰品可精炼
	local canRefine = treaRefineCtrl.checkTreaCanRefineByItemId(tonumber(treaInfo.item_id), true)
	-- logger:debug({treaInfo = treaInfo})
	-- logger:debug("canRefine:")
	-- logger:debug(canRefine)
	return canRefine
end
...
function justiceConchInfo( conchInfo, conchTable)
	...
	for apos,abaginfo in ipairs(tabItem) do
		...
		if(mscore < armscore) then
			isShowRed = true
			break
		elseif (mscore == armscore) then 	-- 品级相同判断强化等级
			local cLv = tonumber(conchInfo.va_item_text.level)
			local pLv = tonumber(abaginfo.lv)
			if (cLv < pLv) then
				isShowRed = true
				break
			end
...
function justiceEquipOrTreasureInfo( itemInfo, mtype, itemType)
	local isShowRed = false
	local pItemType = tonumber(itemType) or 0
	if (pItemType == 1 and not SwitchModel.getSwitchOpenState(ksSwitchWeaponForge, false)) then
		return isShowRed
	end
	if (pItemType == 2 and not SwitchModel.getSwitchOpenState(ksSwitchTreasure, false)) then
		return isShowRed
	end
	--获取背包中所有的装备或者宝物
	...
	for apos,abaginfo in ipairs(tabItem) do
		...
		if(mscore < pscore) then 	-- 品级判断
			isShowRed = true
			break
		elseif (mscore == pscore) then 	-- 品级相同判断强化等级
			local cLv = tonumber(itemInfo.va_item_text.armReinforceLevel)
			local pLv = tonumber(abaginfo.lv)
			if (cLv < pLv) then
				isShowRed = true
				break
...
-- zhangqi, 2015-05-29, 根据配置表的物品id判断是否vip礼包
function isVipGift( tid )
	local nTid = tonumber(tid)
	return nTid >= 12001 and nTid <= 12016
end

-- 获取宝物背包中、伙伴背包中穿戴的、阵容中穿戴的所有宝物
function getAllSpecial( ... )
	local tbAllTreasureList = {}
	local allHeros = HeroModel.getAllHeroes()
	for f_hid, heroInfo in pairs(allHeros or {}) do
		for p, equipInfo in pairs(heroInfo.equip.exclusive) do
			if( not table.isEmpty(equipInfo)) then
				equipInfo.itemDesc = getItemById(equipInfo.item_template_id)
				equipInfo.itemDesc.desc = equipInfo.itemDesc.info
				equipInfo.equip_hid =  tonumber(f_hid)
				table.insert(tbAllTreasureList, equipInfo)
			end
		end
	end
	...
	return tbAllTreasureList
end

-- 获取背包中的专属宝物(包含下阵伙伴穿在身上的)
function getSpecialOnBag( ... )
	local tbAllTreasureList = {}
	local allHeros = HeroModel.getAllHeroes()
	for f_hid, heroInfo in pairs(allHeros or {}) do
		for p, equipInfo in pairs(heroInfo.equip.exclusive) do
			if( not table.isEmpty(equipInfo)) then
				if (not HeroPublicUtil.isBusyByHid(tonumber(f_hid))) then
					equipInfo.itemDesc = getItemById(equipInfo.item_template_id)
					equipInfo.itemDesc.desc = equipInfo.itemDesc.info
					equipInfo.equip_hid =  tonumber(f_hid)
					table.insert(tbAllTreasureList, equipInfo)
...
-- 获取伙伴身上装备的宝物id和itemid，没装备返回nil
function getSpecialOnHero( hid )
	require "script/module/specialTreasure/SpecialConst"
	local heroInfo = HeroModel.getHeroByHid(hid)
	if (table.isEmpty(heroInfo.equip.exclusive[SpecialConst.SPECIAL_POS])) then
		return nil, nil
	end
	local speInfo = heroInfo.equip.exclusive[SpecialConst.SPECIAL_POS]
	return tonumber(speInfo.item_template_id), tonumber(speInfo.item_id)
end

-- zhangqi, 2015-12-16, 将计算出的装备属性格式化后插入指定的table中，用于背包和选择列表的显示
function insertEquipAttr( tbSrcAttr, tbDesAttr )
    local strAttrDesc, j, name, line = "", 1, "", ""
    local allKeys = table.allKeys(tbSrcAttr)
    table.sort(allKeys)

    -- for k, v in pairs(tbSrcAttr) do
    for i, k in ipairs(allKeys) do
        if (tbSrcAttr[k] ~= 0) then
        	name =  g_AttrNameWithoutSign[k] or k
        	line = string.format("%s +%s\n", name, tbSrcAttr[k])
            strAttrDesc = strAttrDesc .. line
            if (j%3 == 0 ) then
    ...
    table.insert(tbDesAttr, strAttrDesc)
    logger:debug({ItemUtil_insertAttr_tbDesAttr = tbDesAttr})
end

-- zhangqi, 2015-12-26, 附魔属性由固定5种变为任意种类，改为由附魔等级计算属性值
-- 参数 dbEquip，传入装备的配置信息
-- 附魔属性(最多3条)，显示值=装备的附魔等级×每级附魔成长值
function addEnchantAttr(dbEquip, nEnhanceLv, tbSrcAttr, tbDesAttr)
	...
	local tbHasAttr = {} -- 已有的属性名称
	for i, key in pairs(table.allKeys(tbSrcAttr)) do
		tbHasAttr[#tbHasAttr + 1] = g_AttrNameWithoutSign[key]
	end
	logger:debug{addEnchantAttr_tbHasAttr = tbHasAttr}
	...
	for i, affix in ipairs(tbAffix) do
		if (not table.include(tbHasAttr, affix.name)) then
			if (string.find(affix.num, "%%")) then
				local displayNum = affix.realNum / 100 * nEnhanceLv
				if(displayNum > math.floor(displayNum))then
					displayNum = string.format("%.1f", displayNum)
				end
				tbSrcAttr[affix.name] = string.format("%s%%", displayNum)
			else
				tbSrcAttr[affix.name] = string.format("%d", nEnhanceLv * affix.num)
			end
		end
	end
	logger:debug({alignEnchantData_tbSrcAttr = tbSrcAttr})

	insertEquipAttr(tbSrcAttr, tbDesAttr)
end
