...
--[[desc:创建CCMenuIteImage类型的 button
    arg1: 参数说明
    return: 是否有返回值，返回值说明  
—]]
local function createImgBtn( normalImg,selectedImg)
	local menuItem = CCMenuItemImage:create(normalImg,selectedImg)
	table.insert(m_tbBtns,menuItem)
	menuItem:setAnchorPoint(ccp(0,0))

	local menu = CCScrollMenu:create()
	menu:setTouchPriority(g_tbTouchPriority.richTextBtn)
	menu:addChild(menuItem)
	menu:setContentSize(menuItem:getContentSize()) -- 设置menu的大小为item大小，富文本排列元素位置才不会出错
	return menu
end
...
function create( tbRich, szRect, nLineSpace, ... )
	...
	for i, str in ipairs(tbText) do
		...
		if (table.count(tbOpt[i]) == 0) then -- 纯文本元素
			...
			elseif (opt.img_btn) then 
				-- 图片按钮
				richEle = RichElementCustomNode:create(tag, ccc3(r,g,b), m_nOpacity, createImgBtn(opt.normalImg,opt.selectedImg) ) -- 文字按钮
				logger:debug("add menu with CCMenuItemImage ")
