...
function getHeroTransferLimitByHid( hid )
	...
	-- zhangqi, 2016-01-09, 处理强化等级最大时的情况
	if (limitLv <= tonumber(heroInfo.level)) then
		return lastTransfNum
	end
...
-- zhangqi, 2016-01-9, 根据hid和属性名获取某个伙伴已穿宝物的精炼等级，costId为空不计算
function getWearedTreasEvolveByHid( hid, sFieldName )
	logger:debug("getWearedTreasEvolveByHid")
	local value = 0
	for k, treas in pairs(getTreasByHid(hid) or {}) do
		if (type(treas) == "table") then
			local dbTrea = DB_Item_treasure.getDataById(treas.item_template_id)
			if (dbTrea.costId) then
				value = value + tonumber(treas.va_item_text[sFieldName] or 0)
			end
			logger:debug("value = %d", value)
		end
...
function getWearedTreasEvolveLevelByHid( hid )
	...
	return getWearedTreasEvolveByHid(hid, "treasureEvolve")
end
function getWearedTreasEvolveMaxByHid( hid ) -- 根据hid获取某个伙伴已穿宝物的精炼等级上限
	...
	for k, treas in pairs(getTreasByHid(hid) or {}) do
		if (type(treas) == "table") then
			local dbTrea = DB_Item_treasure.getDataById(treas.item_template_id)
			if (dbTrea.costId) then
				local dbRefine = DB_Treasurerefine.getDataById(dbTrea.costId)
				value = value + dbRefine.max_upgrade_level
			end
		end
	end
...
--[[desc:zhangqi, 2015-11-03, 根据伙伴tid返回所有可激活加成的物品id
    heroHtid: 伙伴的template_id
    return: table, 存放所有物品的template_id 
—]]
function getTidsOfAddtionalAttrEnabled( heroHtid )
	require "db/DB_Heroes"
	local dbHero = DB_Heroes.getDataById(heroHtid)
	if (not dbHero) then
		return nil
	end

	local linkGroup = dbHero.link_group1
	if (not linkGroup) then
		return nil
	end

	local mSplit = string.split
	local tbItemIds = {} -- 可激活加成的物品tid(可能是装备，饰品等等)

	require "db/DB_Union_profit"
	local unionInfo, unionItems, type_card_arr
	for i, unionId in ipairs(mSplit(linkGroup, ",")) do
		unionInfo = DB_Union_profit.getDataById(unionId)
		unionItems = mSplit(unionInfo.union_card_ids, ",")

		for k, item in pairs(unionItems) do
			itemInfo = mSplit(item, "|") -- [1]是类型，[2]是item tid
			if (tonumber(itemInfo[1]) == 2) then
				tbItemIds[itemInfo[2]] = true
			end
		end
	end
