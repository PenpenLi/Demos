...
-- 获取当前pageView所有英雄
function getHeroPosByHid( findhid )
    local getPos = 1
    ...
    for i=0,squadNum - 1 do
        local hid  = pSquad[i .. ""]
        if (hid and tonumber(hid) > 0 and tonumber(findhid) == tonumber(hid)) then
            getPos = i
            return getPos
        end
    end
...
function getHtidByHid( hid )
	
	for k, v in pairs(_allHeroes) do
		if tonumber(k) == tonumber(hid) then
			return tonumber(v.htid )		
		end
	end
	return 0
end
...
-- 修改指定hero的觉醒装备
function setHeroAwakeEquip( hid, item_id, itemPos, num)
	_allHeroes[tostring(hid)].awake_attr.awake_item[tostring(itemPos)] = {}
	_allHeroes[tostring(hid)].awake_attr.awake_item[tostring(itemPos)][item_id] = tostring(num)
end

-- 修改指定hero的觉醒等级
function setHeroAwakeLv( hid, star, level )
	_allHeroes[tostring(hid)].awake_attr.star_lv = tostring(star)
	_allHeroes[tostring(hid)].awake_attr.level = tostring(level)
	_allHeroes[tostring(hid)].awake_attr.awake_item = {}
	return _allHeroes[tostring(hid)]
end
...
function exchangeEquipInfo( f_hid, s_hid )
	...
	-- yucong 专属宝物不交换
	local f_exclusive = _allHeroes["" .. f_hid].equip.exclusive
	local s_exclusive = _allHeroes["" .. s_hid].equip.exclusive

	local f_equipInfo = _allHeroes["" .. f_hid].equip
	_allHeroes["" .. f_hid].equip = _allHeroes["" .. s_hid].equip
	_allHeroes["" .. s_hid].equip = f_equipInfo
	-- 还原自己的专属宝物
	_allHeroes["" .. f_hid].equip.exclusive = f_exclusive
	_allHeroes["" .. s_hid].equip.exclusive = s_exclusive
end
...
--重置伙伴的觉醒信息
function resetAwakedHeroInfo( pHid )
	_allHeroes[tostring(pHid)].awake_attr.awake_item	= {}
	_allHeroes[tostring(pHid)].awake_attr.level    	= 0
	_allHeroes[tostring(pHid)].awake_attr.star_lv	= 0
end