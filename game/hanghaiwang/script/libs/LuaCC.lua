...
-- 通过前缀和路径创建数字精灵
function createNumberSpriteWithName(dirPath,preName,num)
	local numStr = num .. ""
	preName = preName .. ""
	local numContent = CCSprite:create()
	local width  = 0
	...
	for i=1,string.len(numStr) do
		local singleChar = string.char(string.byte(numStr,i))
		local numSprite = CCSprite:create(dirPath .. "/".. preName ..singleChar .. ".png")
		numSprite:setAnchorPoint(ccp(0, 0.5))
		numSprite:setPosition(width, 23)
		numContent:addChild(numSprite)
		width = width + numSprite:getContentSize().width
...
function createSpriteWithRenderLabel(bgfile, tLabel)
	local ccSprite = CCSprite:create(bgfile)
	local spriteSize = ccSprite:getContentSize()
	local ccLabel = CCLabelTTF:create(tLabel.text, g_sFontName, tLabel.fontsize)--, tLabel.stroke_size, tLabel.stroke_color, type_stroke)
...
-- 创建九宫格按键
-- return CCMenuSprite
-- sample:LuaCC.create9ScaleMenuItem("images/battle/btn_1.png","images/battle/btn_2.png",CCSizeMake(200,70),"测试按键",ccc3(255,222,0))
function create9ScaleMenuItem(normalFileName,selectedFileName,itemSize,labelString,labelColor,labelSize,font,strokeSize,strokeColor,labelOffset,normalCapInsets,selectedCapInsets)
    ...
    local normalLabel = CCLabelTTF:create(labelString, font, labelSize)--, strokeSize, strokeColor)
    ...
    local selectedLabel = CCLabelTTF:create(labelString, font, labelSize*selectedScale)--, strokeSize*selectedScale, strokeColor)
    ...
    local disableLabel = CCLabelTTF:create(labelString, font, labelSize)--, strokeSize, strokeColor)
...
function create9ScaleMenuItemWithoutLabel(normalFileName,selectedFileName,disabledFileName,itemSize)
	...
	disabledFileName = disabledFileName==nil and normalTexture or disabledFileName
