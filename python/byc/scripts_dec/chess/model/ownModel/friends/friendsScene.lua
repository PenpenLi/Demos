...
-- 联盟界面
function FriendsScene:onUnionBtnClick()
...
-- 联盟按钮
function FriendsScene:setHallUnionBtnVisible(visible)
    if not visible then
        -- 收起动画
...
function FriendsScene:getUnionRecommend()
	self:requestCtrlCmd(FriendsController.s_cmds.getUnionRecommend);   
end

function FriendsScene:updataUnionDialog(data,errorMsg)
    if not self.m_union_dialog then
        self.m_union_dialog = new(UnionDialog,self);
    end
...
function FriendsScene:getAllUnionMember()
	self:requestCtrlCmd(FriendsController.s_cmds.getUnionMember);   
end

function FriendsScene:updataUnionMember(data,errorMsg)
	if not self.m_union_dialog then
        self.m_union_dialog = new(UnionDialog,self);
    end
    if not self.m_union_dialog:isShowing() then
...
function FriendsScene:getUnionDialog()
    return self.m_union_dialog;
end

function FriendsScene:getAddFriendDialog()
