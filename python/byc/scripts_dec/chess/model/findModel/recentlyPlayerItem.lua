require(VIEW_PATH.."find_view_recently_players_item");

RecentlyPlayerItem = class(Node)

RecentlyPlayerItem.ctor = function(self,data)
    self.root = SceneLoader.load(find_view_recently_players_item);
    ...
    self.unfollowBtn:setSrollOnClick();
...
RecentlyPlayerItem.updateRelation = function(self,relation)
...
RecentlyPlayerItem.getTargetMid = function(self)
    return tonumber(self.data.mid);
end

RecentlyPlayerItem.setFollowBtnClick = function(self,obj,func)
    self.followBtnClickObj = obj;
    self.followBtnClickFunc = func;
end

RecentlyPlayerItem.onFollowBtnClick= function(self)
