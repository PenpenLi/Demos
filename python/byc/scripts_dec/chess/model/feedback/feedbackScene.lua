...
FeedbackScene.create = function(self)   
	...
	--标题部分
--	self.m_title_view = self.m_root_view:getChildByName("feedback_title_view");
--	self.m_back_btn = self.m_title_view:getChildByName("feedback_back_btn");
--	self.m_back_btn:setOnClick(self,self.back_action);

--    self.m_select_title = self.m_title_view:getChildByName("title_select");

--	self.m_content_view = self.m_root_view:getChildByName("feedback_content_view");

--    self.m_feedback_qq_group = self.m_content_view:getChildByName("feedback_qq_group");
--    self.m_feedback_qq_group:setText(UserInfo.getInstance():getQQGroupString());

--	self.m_send_btn = self.m_content_view:getChildByName("feedback_send_btn");
--	self.m_send_btn:setOnClick(self,self.send_action);

--	self.m_content_edit_bg = self.m_content_view:getChildByName("feedback_content_edit_bg");
--	self.m_content_edit = self.m_content_edit_bg:getChildByName("feedback_content_edit");
--    self.m_content_edit:setHintText("请填写您宝贵的意见 (必填)",165,145,120);
--	self.m_content_edit:setOnTextChange(self,self.contentTextChange);
--	self.m_img_bg = self.m_content_view:getChildByName("feedback_content_img_bg");
--	self.m_img = self.m_img_bg:getChildByName("feedback_content_img");
--	self.m_img:setVisible(false);

--	self.m_concact_info_bg = self.m_content_view:getChildByName("feedback_concact_info_bg");
--	self.m_concact_info = self.m_concact_info_bg:getChildByName("feedback_concact_info");
--    self.m_concact_info:setHintText("感谢留下您的联系方式（选填）",165,145,120);
--	self.m_concact_info:setOnTextChange(self,self.concactTextChange);

--	self.m_feedback_record_placeholder = self.m_content_view:getChildByName("feedback_record_placeholder");
--	local w,h = self.m_feedback_record_placeholder:getSize();
--	FeedbackScene.feedlog_width  = w - 20;
--	FeedbackScene.feedlog_heigth = h - 30;


--    self.m_rule_content_view = self.m_root_view:getChildByName("rule_content_view");
--    local rootW, rootH = self:getSize();
--    --78是tittle的高
--    self.m_rule_content_view:setSize(nil, rootH - 78);
--    self.m_rule_content_scroll_view = self.m_rule_content_view:getChildByName("content");
--    --rootH-78是contentview高，118是bottom（隐私条款等）高
--    self.m_rule_content_scroll_view:setSize(nil, rootH - 78 - 118);


--    self.m_privacy_policy = self.m_rule_content_view:getChildByName("privacy_policy");
--    self.m_terms_of_service = self.m_rule_content_view:getChildByName("terms_of_service");
--    self.m_version = self.m_rule_content_view:getChildByName("text3");
----    self.m_version:setText("文网游备字[2013]M-CBG007号  版本："..VERSIONS);
--    self.m_version:setText("文网游备字[2013]M-CBG007号");

--    self.m_privacy_policy:setOnClick(self,self.toPriPage);
--    self.m_terms_of_service:setOnClick(self,self.toSerPage);


--    self.feedback_select_btn = new(RadioButton,{"common/left_normal.png","common/left_choose.png"});
--    self.rule_select_btn = new(RadioButton,{"common/right_normal.png","common/right_choose.png"});

--    self.feedback_select_btn_icon = new(Image,"rule/feedback_choose.png");
--    self.rule_select_btn_icon = new(Image,"rule/game_rule_choose.png");

--    self.feedback_select_btn:addChild(self.feedback_select_btn_icon);
--    self.rule_select_btn:addChild(self.rule_select_btn_icon);

--    self.m_select_title:addChild(self.feedback_select_btn);
--    self.m_select_title:addChild(self.rule_select_btn);

--    self.feedback_select_btn_icon:setPos(16,12);
--    self.rule_select_btn_icon:setPos(14,12);

--    self.m_select_title:setOnChange(self,self.onSelectTitleChangeClick);

--    local w = self.feedback_select_btn:getSize();
--    self.rule_select_btn:setPos(w);
--    self:changeState(FeedbackScene.s_changeState)
...
FeedbackLogItem.ctor = function(self,data,parent)
	...
	-- (self,str,width,height,align,fontName,fontSize,r, g, b)
