...
--Gallery.getMainNodeW = function(self)
--    local w, h = self.m_mainNode:getSize();
--    return w or 0;
--end
...
Gallery.removeChild = function(self, child, isClean)
    ...
    if itemIndex then
        ...
        while itemIndex <= #self:getChildren() do
            ...
            Log.i("Gallery.removeChild ----------------------------------->>"..self.m_mainNode.m_rchildren[child]);
        end;
        return ret;
    end
end;
...
--    Log.i("ScrollView.rebondMargin...."..scrollerStatus .. "...align..."..align);
--    if self.m_totalOffset >= 0 then
--        if math.abs(self.m_totalOffset) > 200 then
--            if self:leftHasView() then
--                self:leftViewToShow();
--            else
--                self:backToOrignPos();
