...
HandicapDialog.ctor = function(self,room)
	...
	self.m_dialog_bg = self.m_root_view:getChildByName("handicap_full_screen_bg");
...
HandicapDialog.onTouch = function(self)
	print_string("HandicapDialog.onTouch");
end
...
HandicapDialog.setData = function(self,data)
        ...
        self.m_base_money:setText(string.format("%d金币",self.m_baseMoney));

            -- 获取按钮状态
    if data and data.byMultiplyBtn then
        self:initBtnStatus(data.byMultiplyBtn)
    else
	    self.m_handicap_chess1_btn:setOnClick(self,self.handicapChess1);
	    self.m_handicap_chess2_btn:setOnClick(self,self.handicapChess2);
	    self.m_handicap_chess3_btn:setOnClick(self,self.handicapChess3);
...
function HandicapDialog:initBtnStatus(status)
    require("libs/bit");
    local bits = bit.tobits(status);
    -- {0,0,0,0,0,0} 自1 别1 自2 别2 自3 别3
    self:initBtnStatusByData(self.m_handicap_chess1_btn,bits[1] or 0,bits[2] or 0,self.handicapChess1);
    self:initBtnStatusByData(self.m_handicap_chess2_btn,bits[3] or 0,bits[4] or 0,self.handicapChess2);
    self:initBtnStatusByData(self.m_handicap_chess3_btn,bits[5] or 0,bits[6] or 0,self.handicapChess3);
end
-- btn 按钮 mbtn 我的状态 obtn 对手状态
function HandicapDialog:initBtnStatusByData(btn,mbtn,obtn,click)
    if mbtn ~= 1 then
