...
SecondLogGuideDialog.ctor = function(self)
    ...
    self.m_close_btn:setOnClick(self,self.showGuideView);
...
SecondLogGuideDialog.showGuideView = function(self)
...
function SecondLogGuideDialog:showNextGuide()
    self:removeViewProp();
    self:dismiss();
    --如果为true ,弹框消失， false显示下一个新手弹窗
    if not self.showNextGuideStatus then
        return
    end
...
function SecondLogGuideDialog:setShowStatus(index)
    if not index then
        self.m_bg:setVisible(true);
        self.m_black_bg:setVisible(false);
    elseif index == 1 then
        self.showNextGuideStatus = false;
        self.m_bg:setVisible(false);
        self.m_black_bg:setVisible(true);
--        self:timingAnim();
        self:setShieldClick(self,self.onViewClick);
...
--定时关闭动画，已经去掉
function SecondLogGuideDialog:timingAnim()
    self:setShieldClick(self,self.onViewClick);
    self.anim_time = new(AnimInt,kAnimNormal,0,1,6000,SecondLogGuideDialog.DEFAULT_ANIM_TIME); 
    if self.anim_time then
