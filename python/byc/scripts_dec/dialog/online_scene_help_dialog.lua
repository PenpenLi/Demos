...
OnlineSceneHelpDialog.CONTEXT = {
    '#s36#c87645F观战：#n#l可围观专业二级以上（积分超过2000）用户的对局以及所关注的棋友的对局。#l观战中，对局双方用户左边为黑方，右边为红方。#l点击棋盘可以切换为全屏模式显示。',
    '#l#s36#c87645F场次说明：#n',
    ...
    '读秒：局时用完后每步棋可用的走子时间。',
    '若设置不读秒即一方局时结束后棋局结束。',
    '#l棋局中，首先进行“局时”与“步时”的计算。局时超时前，每步都必须在“步时”规定时间内完成走子，走下一步时，“步时”将重新计算。“局时”超时后，“局时”停止计算，每步都必须在“读秒”规定时间内完成走子。“步时”与“读秒”超时都会引起棋局结束，超时者判输。',
    '#l#s36#c87645F其他：#n#l联网游戏悔棋和求和均需要对方同意。',
    '联网对局或观战，可以在棋友信息中选择关注棋友或者屏蔽或取消屏蔽该棋友当局的聊天消息。',
    '#l私人房可以邀请身边好友一起对局，也可以直接在聊天室向棋友发起挑战。',
    '#l私人房对局和好友对局不产生积分变化。',
    '#l棋局中可通过设置修改走子的操作方式。棋盘和棋子的显示样式可在我的个人装扮中修改。'
}
function OnlineSceneHelpDialog:ctor()
    ...
    local data = RoomConfig.getInstance():getRoomConfig();
    if data then
        for i=1,5 do
            if data[i] then
                local index = i+2;
