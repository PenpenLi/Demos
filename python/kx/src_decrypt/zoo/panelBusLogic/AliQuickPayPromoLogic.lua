...
function AliQuickPayPromoLogic:isEntryEnabled()
    ...
    if AliQuickPayPromoLogic:isInPromotion() then
        print('wenkan isEntryEnabled 11111111')
        return true
    end
    ...
    if not _G.kUserLogin then 
        print('wenkan isEntryEnabled 22222222')
        return false 
    end -- 没登录没网就看不到

    if not UserManager:getInstance():isAliNeverSigned() then
        print('wenkan isEntryEnabled 3333333')
        return false
    end 
    ...
    if MaintenanceManager:getInstance():isEnabled(key) ~= true then
        print('wenkan isEntryEnabled 4444444')
        return false
    end
    ...
    uid = tonumber(uid)
    if uid ~= nil then
        print('wenkan isEntryEnabled 55555555 ', (uid % 100) < num)
        return (uid % 100) < num
    else
        print('wenkan isEntryEnabled 6666666')
        return false
    end
end
