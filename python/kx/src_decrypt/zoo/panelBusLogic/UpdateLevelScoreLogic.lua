...
function UpdateLevelScoreLogic:start(...)
	...
	if curUserScore then
		...
		if curUserScore.score > self.score and curUserScore.star >= 1 then
	...
	if self.levelType == GameLevelType.kMainLevel then
		...
		print("UpdateLevelScoreLogic: Change Normal Star Number To: " .. newNormalStar)
	elseif self.levelType == GameLevelType.kHiddenLevel then
		...
		print("UpdateLevelScoreLogic: Change Hide Star Number To: " .. newHideStar)
	end
end
