...
function AddFriendPanelLogic:inRequestedRange(longitude, latitude)
	print("AddFriendPanelLogic:inRequestedRange")
...
function AddFriendPanelLogic:searchUser(code, successCallback, failCallback, cancelCallback, context)
	...
	local function onSuccess(evt)
		...
		if evt.data.profile and evt.data.user then
			table.insert(data, {userLevel = evt.data.user.topLevelId, userName = evt.data.profile.name, uid = evt.data.user.uid, headUrl = evt.data.profile.headUrl or evt.data.user.image})
		end
