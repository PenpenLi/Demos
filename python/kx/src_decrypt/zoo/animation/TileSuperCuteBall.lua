...
function TileSuperCuteBall:showBgMask(time)
	if self.bgMaskSprite then
		...
		self.bgMaskSprite:runAction(CCFadeIn:create(time))
	end
end
...
function TileSuperCuteBall:hideBgMask(time)
	if self.bgMaskSprite then
		time = time or 0
		self.bgMaskSprite:stopAllActions()
		self.bgMaskSprite:runAction(CCFadeOut:create(time))
	end
end
...
function TileSuperCuteBall:playJump(direction, callback)
	...
	actArr:addObject(CCDelayTime:create(5 * kCharacterAnimationTime))

	if direction == SuperCuteBallJumpDirection.kUp then
		actArr:addObject(CCMoveBy:create(4 * kCharacterAnimationTime, ccp(0, 70)))
	elseif direction == SuperCuteBallJumpDirection.kRight then
		actArr:addObject(CCMoveBy:create(4 * kCharacterAnimationTime, ccp(36, 5.5)))
		actArr:addObject(CCMoveBy:create(7 * kCharacterAnimationTime, ccp(34, -5.5)))
	elseif direction == SuperCuteBallJumpDirection.kDown then
		actArr:addObject(CCMoveBy:create(4 * kCharacterAnimationTime, ccp(0, -43)))
		actArr:addObject(CCMoveBy:create(7 * kCharacterAnimationTime, ccp(0, -27)))
