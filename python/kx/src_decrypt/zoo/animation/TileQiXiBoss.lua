...
function TileQiXiBoss:getBossSpriteWorldPosition()
    if self.body and self.sprite then
        ...
        --print("RRR   PPPPPPPPPPPPPPPPPPPP   worldPos.x = " , worldPos.x)
...
function TileQiXiBoss:playHit(fromPosInWorld, callback, isSpecial , hit , totalBlood , index)
    --print("RRR   TileQiXiBoss:playHit   fromPosInWorld = " , fromPosInWorld.x , fromPosInWorld.y)
    ...
    local function onQixiAnimationDone(datas)
	    ...
	    --print("RRR  TileQiXiBoss:playHit  bossTotalProgressDis " , bossTotalProgressDis , "  progress " , progress , "      " , bossTotalProgressDis * progress)
	    --print("RRR  TileQiXiBoss:playHit  hippoNeedMove " , hippoNeedMove , "  foxNeedMove " , foxNeedMove)

	    self.animate = animationList.kHit
	    -- print("RRR  ----------------------  hippo:playHit   hippoNeedMove = " , datas.hippo )
        self.hippo:playHit( datas.hippo , 0.3 , localCB)
        
        -- print("RRR  ----------------------  fox:playHit   foxNeedMove = " , datas.fox * -1 )
