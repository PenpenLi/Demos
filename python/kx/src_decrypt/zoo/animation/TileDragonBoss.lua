...
function TileDragonBoss:buildBossDieDropsAnim(dropBell, dropAddMove, callback)
    ...
    -- print("dxsprite:", dxsprite, dysprite)
    local bells = {}
    local addMoves = {}
    ...
    for i = 1, totalDrop do
        ...
        if not isAddMove then
            sprite = Sprite:createWithSpriteFrameName("target.dig_move_endless_mayday_1 instance 10000")
...
function TileDragonBoss:playCasting(destPositions, callback)
    -- print('TileDragonBoss:playCasting')
    -- print(self.animate)
    -- debug.debug()?
