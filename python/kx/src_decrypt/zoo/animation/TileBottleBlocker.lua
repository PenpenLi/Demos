...
local BodyAdjustColorHSBC = {
	...
	[AnimalTypeConfig.kGreen] = {0.527, 0.505, 0.203, 0.164},
	[AnimalTypeConfig.kPurple] = {-0.45, 0.20, 0, 0.14},
	[AnimalTypeConfig.kYellow] = {0.311, 0.467, 0.335, 0.610},
...
local EyesAdjustColorHSBC = {
	...
	[AnimalTypeConfig.kGreen] = {0.527, 0.505, 0.203, 0.164},
	[AnimalTypeConfig.kPurple] = {-0.45, 0.20, 0, 0.14},
	[AnimalTypeConfig.kYellow] = {0.311, 0.467, 0.311, 0.610},
	[AnimalTypeConfig.kOrange] = {0.240, -0.107, 0.167, 0.287},
}
...
function TileBottleBlocker:init(level, color)
	...
	self.animal = TileBottleBlocker:createAnimalSprite(color)
...
function TileBottleBlocker:createAnimalSprite(color)
	local sprite = Sprite:createEmpty()
	local body = SpriteColorAdjust:createWithSpriteFrameName("bottle_animal_idle_body_0000")
	local eyes = SpriteColorAdjust:createWithSpriteFrameName("bottle_animal_idle_eyes_0000")
	local eyesFrames = SpriteUtil:buildFrames("bottle_animal_idle_eyes_%04d", 0, 9)
	...
	local faceFrame = "bottle_animal_idle_face_0000"
	if color == AnimalTypeConfig.kYellow then
		faceFrame = "bottle_animal_idle_face2_0000"
	end
	local face = Sprite:createWithSpriteFrameName(faceFrame)
	face:setPosition(ccp(0, -8))
...
function TileBottleBlocker:playBottleIdle()
	if self.animal and not self.animal.isDisposed then
		...
		if self.animal.eyes and not self.animal.eyes.isDisposed then
			...
			local eyesAnim1Frames = SpriteUtil:buildFrames("bottle_animal_idle_eyes_%04d", 0, 9)
			local eyesAnim1 = SpriteUtil:buildAnimate(eyesAnim1Frames, kCharacterAnimationTime)

			local eyesAnim2Frames = SpriteUtil:buildFrames("bottle_animal_blink_eyes_%04d", 0, 43)
		...
		local animalAnim1 = CCSpawn:createWithTwoActions(CCMoveBy:create(kCharacterAnimationTime * 20, ccp(0, 3)), CCScaleTo:create(kCharacterAnimationTime * 20, 1, 1.01))
		local animalAnim2 = CCSpawn:createWithTwoActions(CCMoveBy:create(kCharacterAnimationTime * 20, ccp(0, -3)), CCScaleTo:create(kCharacterAnimationTime * 20, 1, 1))
...
function TileBottleBlocker:playAnimalAnimation(level, newColor, onFinish)
	...
	if level > 1 then
		...
		local function clearAdjustColorShader()
			if body and not body.isDisposed then
				body:clearAdjustColorShader()
			end
		end
		local bodyAnimation1 = SpriteUtil:buildAnimate(bodyFrames, kCharacterAnimationTime)
		local bodyAnimation2 = CCSequence:createWithTwoActions(CCDelayTime:create(kCharacterAnimationTime * 12), CCCallFunc:create(clearAdjustColorShader))
		
		body:runAction(CCSequence:createWithTwoActions(CCSpawn:createWithTwoActions(bodyAnimation1, bodyAnimation2), CCCallFunc:create(onAnimationComplete)))

		local face = Sprite:createWithSpriteFrameName("bottle_animal_hit_face_0000")
		local faceFrames = SpriteUtil:buildFrames("bottle_animal_hit_face_%04d", 0, 18)
		face:runAction(SpriteUtil:buildAnimate(faceFrames, kCharacterAnimationTime))
	...
	elseif level == 1 then
		local body = SpriteColorAdjust:createWithSpriteFrameName("bottle_animal_disappear_body_0000")
		local bodyFrames = SpriteUtil:buildFrames("bottle_animal_disappear_body_%04d", 0, 12)
		...
		local face = Sprite:createWithSpriteFrameName("bottle_animal_disappear_face_0000")
		local faceFrames = SpriteUtil:buildFrames("bottle_animal_disappear_face_%04d", 0, 12)
		face:runAction(SpriteUtil:buildAnimate(faceFrames, kCharacterAnimationTime))
	...
	self:adjustColor(anim, self.color)
end
...
function TileBottleBlocker:changeColor(newColor, onFinish)
	...
	local anim = Sprite:createEmpty()
	anim.body = SpriteColorAdjust:createWithSpriteFrameName("bottle_item_change_body_0000")
	anim.body:setPosition(ccp(0.6, -3.7))
	anim:addChild(anim.body)
	anim.face = Sprite:createWithSpriteFrameName("bottle_item_change_face_0000")
	anim.face:setPosition(ccp(-6.4, 7.3))
	anim:addChild(anim.face)

	local bodyFrames = SpriteUtil:buildFrames("bottle_item_change_body_%04d", 0, 7)
	local bodyAnimation = SpriteUtil:buildAnimate(bodyFrames, kCharacterAnimationTime)
	local faceFrames = SpriteUtil:buildFrames("bottle_item_change_face_%04d", 0, 7)
	local faceAnimation = SpriteUtil:buildAnimate(faceFrames, kCharacterAnimationTime)
	anim.body:runAction(CCSequence:createWithTwoActions(bodyAnimation, CCCallFunc:create(onAnimationComplete)))
	anim.face:runAction(faceAnimation)

	self.animal = anim
	self:addChildAt(self.animal, TileBottleZIndex.kAniml)
	self:adjustColor(anim, newColor)
end