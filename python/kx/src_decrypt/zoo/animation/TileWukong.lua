...
function TileWukong:init(data)
	...
	self.colorIndex = AnimalTypeConfig.convertColorTypeToIndex(data.ItemColorType) 
	...
	print("TileWukong:init    needHideMoneyBar = " , data.needHideMoneyBar)
...
function TileWukong:createAnimation(stateString , playTimes , intervalTime)
	...
	if self.stateString ~= stateString then
		...
		if self.body then
			...
			self.body:stopAllActions()
			self.body:removeFromParentAndCleanup(false)
		end
