...
function NumberAnimation:createNumberFlipAnimation(num , font , scale , color , speed , widInterval , heiInterval , align , maskHeight , fixEndY , delayPlay)
	...
	for i = 1 , #textList do
		...
		for ia = 1 , #part do
			...
			t:setColor(color)
			numTextSize = t:getGroupBounds().size
			t:setPosition(  ccp( 0 , ( numTextSize.height + heiInterval) * ia) )
			--print("RRR  ============================   partContainer add " , ( numTextSize.height + heiInterval ) * ia )
			partContainer:addChild(t)
			--print("RRR  ============================   partContainer.size.height " , partContainer:getGroupBounds().size.height )
		end

		partContainer:setPosition( ccp( textContainer:getGroupBounds().size.width + widInterval , numTextSize.height ) )
	...
	if not maskHeight then maskHeight = numTextSize.height + heiInterval end

	progressMask:changeWidthAndHeight( textContainerWidth + 100 ,maskHeight)
	...
	for i = 1 , #textContainer.partList do
		...
		--print("RRR  -------------------------   ")
		--print("RRR  partSize.height  = " , partSize.height , "  heiInterval = " , heiInterval , " numTextSize.height = " , numTextSize.height)
		actArr2:addObject( CCEaseSineOut:create( 
			CCMoveTo:create( speed , ccp( textContainer.partList[i]:getPositionX() , (partSize.height * -1) + numTextSize.height + fixEndY ) ) ) )
