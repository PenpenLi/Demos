...
local function dropAndShiningAnimate( icon, halo, parent, onAnimationFinished, iconSpawn)
	local iconInTime = 0.4
	local winSize = CCDirector:sharedDirector():getVisibleSize()
...
function PrefixPropAnimation:createPropAnimation( icon, positionSrc, animationCallbackFunc )
	local layer = CocosObject:create()	
	local sprite = Sprite:createWithSpriteFrameName("prefix_halo0000")	

	layer:setPosition(positionSrc)
	layer:setScale(1.4)
	dropAndShiningAnimate(icon, sprite, layer, animationCallbackFunc)
	return layer
end
...
function PrefixPropAnimation:createAddMoveAnimation(icon, delay, flyFinishedCallback, animationCallbackFunc, positionSrc )
	delay = delay or 0
	...
	local topright = layer:convertToWorldSpace(ccp(winSize.width - 100, winSize.height - 100))
...
function PrefixPropAnimation:createChangePropAnimation( icon, delay, positionA, positionB, flyFinishedCallback, animationCallbackFunc, positionSrc )
	delay = delay or 0
	...
	container:setPosition(ccp(x, y))
	layer:addChild(container)
	dropAndShiningAnimate(icon, sprite, container, onIconAnimationFinished)
