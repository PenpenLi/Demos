...
--------------------------------------------------------------------------------------
function FlowerNode:createStars(flowerType, starNum)
...
function FlowerNode:addFlowerAndStars(flowerType, starNum ,levelId, autoGray)
	...
	self.stars = FlowerNode:createStars(flowerType, starNum)
	if self.stars then
		local starPosY = -65
		if self.flower then 
			local sizeFlower = self.flower:getContentSize()
			local sizeStarHeight = 23
			starPosY = -sizeFlower.height/2 - sizeStarHeight / 2
		end
...
function FlowerNode:addLabel(flowerType, levelId, faceName)
	...
	if flowerType == kFlowerType.kHidden then -- Òþ²Ø¹Ø¿¨
...
function FlowerNodeUtil:createWithSize(flowerType, levelId, starNum, size , autoGray)
	...
	-- local lc = LayerColor:createWithColor(ccc3(255, 0, 0), size.width, size.height)
