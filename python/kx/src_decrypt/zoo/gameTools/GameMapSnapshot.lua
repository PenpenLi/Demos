...
function GameMapSnapshot:ctor()
	FrameLoader:loadPngOnly("materials/game_bg.png")
end
...
function GameMapSnapshot:buildShareGroup(levelId, playBoard)
	...
	if LevelType:isHideLevel( levelId ) then
		...
		titleText = string.format("隐藏关%s", levelId - LevelConstans.HIDE_LEVEL_ID_START)
	end
...
function GameMapSnapshot:genSnapShot(levelId, saveDir, onFinish)
	local function onGenPlayBoardSnapShot(levelId, playBoard)
		...
		if playBoard then
	    	...
	    	shareGroup:visit()
	  		...
	  		renderTexture:saveToFile(string.format("%s/%s", saveDir, saveName))

	  		print(string.format("success generate share: %d", levelId))
	  		if type(self.onGenerateResult) == "function" then self.onGenerateResult(levelId, true) end
	  	else
	  		print(string.format("failed generate share: %d", levelId))
	...
	GameMapSnapshot:genPlayBoardSnapShot(levelId, levelType, onGenPlayBoardSnapShot)
end
...
function GameMapSnapshot:genPlayBoardSnapShot(levelId, levelType, onSuccess)
	local function callback()
		local scene = GamePlaySceneUI.new()
		scene:init(levelId, levelType, {}, GamePlaySceneUIType.kDev)
		...
		local imgPath = CCFileUtils:sharedFileUtils():fullPathForFilename("materials/game_bg.png")
		...
		local bg = Sprite:createWithTexture(texture)
   		...
   		bg:setPosition(ccp(360, 640 - 120))
   		...
   		gameBoardView:setPosition(ccp(gbPosX, gbPosY))

   		local retSprite = nil
   		if gameBoardView then
	   		local playScenePos = ccp(0 - (gbPosX - gameBoardMargin), 0 - (gbPosY - gameBoardMargin))
	    	...
	    	playScene:visit()
	  		...
	  		renderTexture1:saveToFile(kTempFile)

	  		CCTextureCache:sharedTextureCache():removeTextureForKey(kTempFile)
	  		local texture1 = CCTextureCache:sharedTextureCache():addImage(kTempFile)
	    	...
	    	sprite1:visit()
	  		...
	  		renderTexture:saveToFile(kTempFile)
	  		CCTextureCache:sharedTextureCache():removeTextureForKey(kTempFile)
	  		local texture2 = CCTextureCache:sharedTextureCache():addImage(kTempFile)
...
function GameMapSnapshotTool:createInputBox(text, boxSize, bgColor, fontSize, fontColor)
	...
	print("createInputBox:", boxSize.width, boxSize.height)
...
function GameMapSnapshotTool:init()
	...
	onStartTapped = function()
		...
		local saveDir = kDefaultFileDir or "D:/level_share"
		logic:start(saveDir, onFinish)
	end
	button:addEventListener(DisplayEvents.kTouchTap, onStartTapped)
	self:addChild(button)

	local desc1 = createLabel("默认存储路径: D:/level_share（手动创建）" , 20, ccc3(250, 250, 250))
