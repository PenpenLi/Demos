...
function AndroidOneYuanBuyGoldItem:onBuyBtnTapped()
	...
	local function onCancel()
		print('AndroidOneYuanBuyGoldItem onCancel')
	...
	local function onFail(errCode, errMsg)
		print('AndroidOneYuanBuyGoldItem onFail')
		if self.isDisposed then return end
		if self.procClick then self:setButtonEnable(true) end
	...
	local function onSuccess()
		print('AndroidOneYuanBuyGoldItem onSuccess')
		if self.isDisposed then return end
		if self.procClick then self:setButtonEnable(true) end
	...
	elseif self.payment == Payments.QQ then
		PlatformConfig:setCurrentPayType(PlatformPayType.kQQ)	
	elseif self.payment == Payments.HUAWEI then
		PlatformConfig:setCurrentPayType(PlatformPayType.kHuaWei)	
	elseif self.payment == Payments.QQ_WALLET then
		PlatformConfig:setCurrentPayType(PlatformPayType.kQQWallet)	
	else
		--若是在默认风车币栏里 则置一下这个标识  
