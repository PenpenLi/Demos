...
function QRCodePostPanel:init()
	...
	if PlatformConfig:isPlatform(PlatformNameEnum.kMiTalk) then
		icon = Sprite:createWithSpriteFrameName("QRCodePanel/mitalkiconimg0000")
	else
		icon = Sprite:createWithSpriteFrameName("QRCodePanel/wechaticonimg0000")
	end
...
function QRCodeReceivePanel:init(code, logic, userData, successCallback, failCallback)
	...
	local userName = HeDisplayUtil:urlDecode(userData.userName or "")
...
function QRCodeReceivePanel:addFriend(code)
	...
	self.logic:sendAddMessage(code, onSuccess, onFail, onCancel)
end
