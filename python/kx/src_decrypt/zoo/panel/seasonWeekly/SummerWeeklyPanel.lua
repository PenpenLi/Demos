...
function SummerWeeklyPanel:init( ... )
	...
	descIcon:addEventListener(DisplayEvents.kTouchTap, function()
		DcUtil:UserTrack({category = 'weeklyrace', sub_category = 'weeklyrace_summer_click_info'}, true)
...
function SummerWeeklyPanel:initMyNumber( ui )
	...
	ui:addEventListener(DisplayEvents.kTouchTap,function( ... )
		...
		if myRank > 0 then 
			...
			CommonTip:showTip(Localization:getInstance():getText("weeklyrace.summer.panel.desc9", {num = SeasonWeeklyRaceManager:getInstance():getRankMinScore()}),"negative")
		end
	end)
