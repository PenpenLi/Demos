...
function sellPropsCallback(success)
	if success then
		local bagData = BagManager:getInstance():getUserBagData()
		setBagData(bagData)
	end
...
function usePropsCallback(success)
	if success then
		local bagData = BagManager:getInstance():getUserBagData()
		setBagData(bagData)
	end
...
function createBagPanel(bagBtnPosInWorldSpace)
...
function setBagPanelDataWithUpdate(items)
	...
	print(pvContainerSize.width, pvContainerSize.height)
	...
	for _page=1, numOfPages do 
		...
		if bagSize - (_page - 1) * maxPageSize < maxPageSize then 
			realPageSize = bagSize - (_page - 1) * maxPageSize
		else 
			realPageSize = maxPageSize
		end

		local data = {}
		-- for j=(_page-1)*maxPageSize+1, _page*maxPageSize do
		-- 	if items[j] then 
		-- 		table.insert(data, items[j])
		-- 	end
		-- end 
		for j=(_page-1)*maxPageSize+1, (_page-1)*maxPageSize+realPageSize do
			if items[j] then 
				table.insert(data, items[j])
			end
		end 
