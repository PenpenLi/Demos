...
function TwoYearsGiftEnegy:onKeyBackClicked()
	print("RRR   TwoYearsGiftEnegy:onKeyBackClicked()  ")
	if self.isLocked then return end
	--local anim = HomeSceneFlyToAnimation:sharedInstance():energyFlyToAnimation({number = 10})
	--anim:play()
	self.isLocked = true
...
function TwoYearsGiftEnegy:flyEnergy()
	...
	scene:addChild(energy)
end

function TwoYearsGiftEnegy:onRewardSuccess()
	print("RRR   TwoYearsGiftEnegy:onRewardSuccess()  ")
	self.isLocked = true
...
function TwoYearsGiftEnegy:onRewardFail(type)
	print("RRR   TwoYearsGiftEnegy:onRewardFail()  ")
	self.isLocked = false
	if type == 1 then
