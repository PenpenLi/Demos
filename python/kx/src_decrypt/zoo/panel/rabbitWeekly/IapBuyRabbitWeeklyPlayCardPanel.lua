...
IapBuyRabbitWeeklyPlayCardPanel = class(BasePanel)

function IapBuyRabbitWeeklyPlayCardPanel:create(targetPanel, buyCallback, maxTopPosYInWorldSpace)
	local panel = IapBuyRabbitWeeklyPlayCardPanel.new()
	panel:loadRequiredResource(PanelConfigFiles.panel_energy_bubble)
	if panel:_init(targetPanel, buyCallback, maxTopPosYInWorldSpace) then
		return panel
	else
		panel = nil
		return nil
...
function IapBuyRabbitWeeklyPlayCardPanel:_init(targetPanel, buyCallback, maxTopPosYInWorldSpace)
	-- data
	...
	local sprite = builder:buildGroup("Prop_13")
	...
	local lBound = chargeLine:getGroupBounds().size
...
function IapBuyRabbitWeeklyPlayCardPanel:popout()
...
function IapBuyRabbitWeeklyPlayCardPanel:remove()
...
function IapBuyRabbitWeeklyPlayCardPanel:_calcPosition()
...
function IapBuyRabbitWeeklyPlayCardPanel:_playFadeInAnim()
...
function IapBuyRabbitWeeklyPlayCardPanel:_playFadeOutAnim(animFinishCallback, ...)
...
function IapBuyRabbitWeeklyPlayCardPanel:_onBuyMidEnergy(data, successCallback, failCallback)
	IapBuyPropLogic:buy(data, successCallback, failCallback)
end

function IapBuyRabbitWeeklyPlayCardPanel:onCloseBtnTapped()
	if self.targetPanel then self.targetPanel:onCloseBtnTapped() end
end