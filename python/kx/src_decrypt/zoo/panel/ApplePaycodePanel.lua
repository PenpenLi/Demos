...
function ApplePaycodePanel:buyBtnTapped()
	...
	local function onSuccess()
		...
		if productMeta then
			...
			for k, v in pairs(goodsMeta.items) do
				if v.itemId >= 10000 and v.itemId < 20000 then
			...
			for i, v in ipairs(goodsMeta.items) do
				if v.itemId == 2 then
					...
					local position = self.items[i].icon:getPosition()
					local wPosition = self.items[i].ui:convertToWorldSpace(ccp(position.x, position.y))
					anim.sprites:setPosition(ccp(wPosition.x, wPosition.y))
				...
				elseif v.itemId == 14 then
					...
					local position = self.items[i].icon:getPosition()
					local size = self.items[i].icon:getGroupBounds(self.items[i].ui).size
					local wPosition = self.items[i].ui:convertToWorldSpace(ccp(position.x + size.width / 2,
						position.y - size.height / 2))
					for i,v in ipairs(anim.sprites) do
						v:setPosition(ccp(wPosition.x, wPosition.y))
						scene:addChild(v)
					end
					anim:play()
				else
					...
					local config = {propId = itemId, number = num, updateButton = true,}
					local anim = HomeSceneFlyToAnimation:sharedInstance():jumpToBagAnimation(config)
					local position = self.items[i].icon:getPosition()
					local size = self.items[i].icon:getGroupBounds().size
					local wPosition = self.items[i].ui:convertToWorldSpace(ccp(position.x + size.width / 8,
						position.y - size.height / 8))
					for k, v in ipairs(anim.sprites) do
						v:setPosition(ccp(wPosition.x, wPosition.y))
						scene:addChild(v)
					end
					anim:play()
	...
	if __IOS then
		IosPayment:buy(productMeta.productId, productMeta.iapPrice, productMeta.priceLocale, "", onSuccess, onFail, dcDispatcher)
	else
		onFail()
		self:onCloseBtnTapped()
	end
...
function ApplePaycodePanel:refresh()
	...
	print(self.discountLine:getPositionX(), self.discountLine:getPositionY())
