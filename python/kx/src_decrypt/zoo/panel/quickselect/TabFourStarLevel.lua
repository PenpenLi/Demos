...
function TabFourStarLevel:initData()
end

function TabFourStarLevel:initUI()
...
function TabFourStarLevel:initContent()
	...
	self.contentUI = self.hostPanel:buildInterfaceGroup("tabFourStarLevelContent")
...
function TabFourStarLevel:initShareArea( ... )
	-- body
...
function TabFourStarLevel:initFourStarLevelArea( ... )
	self.contentUI:getChildByName("share_area"):setVisible(false)
	self.hostPanel.txtDesc:setString(Localization:getInstance():getText("mystar_tag_1.2"))
	...
	print("~~VerticalScrollable~~",size.width,size.height,_width,_height,"panelScale",panelScale,pos.x,pos.y)
	print("VerticalScrollable pos",pos.x,pos.y)
	...
	for k = 1, #dataList do 
		...
		local function onTapped( evt )	
			...
			if node.scrollable and node.scrollable.touchLayer:hitTestPoint(pos) then
				...
				-- print("levelId",levelId,"topId",UserManager.getInstance().user:getTopLevelId())
				...
				DcUtil:UserTrack({
					category = "ui",
					sub_category = "click_see_fourstar",
				},true)
	...
	layer.getHeight = function( self )
		-- print(">>>>>>>>>>>>>>>>>",self:getGroupBounds().size.width,self:getGroupBounds().size.height,panelScale)
...
-- function TabFourStarLevel:popout( ... )
-- 	-- body
...
function TabFourStarLevel:onShowLevelStartPanel( ... )
	-- body
	self:dispatchEvent(Event.new(FourStarGuideEvent.kCloseAllStarGuidePanel))
	self:onCloseBtnTapped()
end

function TabFourStarLevel:onCloseBtnTapped( ... )
	-- -- body
...
function TabFourStarLevel:onShareBtnTap( ... )
	-- body
...
function TabFourStarLevel:screenShotShareImage( ... )
	-- body
...
function TabFourStarLevel:onShareFailed( ... )
	-- body
...
function TabFourStarLevel:onShareSucceed( ... )
	-- body
	if PlatformConfig:isPlatform(PlatformNameEnum.kMiTalk) then
