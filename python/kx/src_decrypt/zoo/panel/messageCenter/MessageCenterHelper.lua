...
function MessageCenterHelper:initDengchaoEnergyRequestTask()
    ...
    MessageCenterHelper:addNotification(notifiTime, LocalNotificationType.kDengchaoEnergy, localize('christmas.energy.notification.1'))
...
function MessageCenterHelper:initPushEnergyRequestTask()
    if DengchaoPushEnergy.isSupport() then
        debug_log('initPushEnergyRequestTask 9')
        MessageCenterHelper:initDengchaoEnergyRequestTask()
        return 
    end
    debug_log('initPushEnergyRequestTask')
    local function meetConditions()
        ...
        and UserManager:getInstance().user:getTopLevelId() >= 15 then
            debug_log('initPushEnergyRequestTask 6')
            return true
        end
        if FriendManager:getInstance():getFriendCount() > 10
    ...
    local myUid = UserManager:getInstance().user.uid or "12345"
    local config = readConfig()
    if tostring(config.lastNotificationUid) ~= myUid then
        MessageCenterHelper:cancelNotification(LocalNotificationType.kPushEnergy)
    end
    ...
    local today = os.date('*t', now())
    local tomorrow = os.date('*t', now() + 24 * 3600)
    local todayTime = {year = today.year, month = today.month, day = today.day, hour = 13, min = 30, sec = 0}
    local tomorrowTime = {year = tomorrow.year, month = tomorrow.month, day = tomorrow.day, hour = 13, min = 30, sec = 0}
    ...
    local function start()
        ...
        local function opSuccess()
            debug_log('OpNotifyHttp success NPC_Push')
            writeFlag(true)
            local function onGetRequestNumSuccess(evt)
                UserManager:getInstance().requestNum = evt.data.requestNum
                GlobalEventDispatcher:getInstance():dispatchEvent(Event.new(kGlobalEvents.kMessageCenterUpdate))
        ...
        local function opFail()
            debug_log('OpNotifyHttp fail NPC_Push')
        end
        if not readFlag() or __WIN32 then
            debug_log('initPushEnergyRequestTask 4')
            local http = OpNotifyHttp.new()
            http:ad(Events.kComplete, opSuccess)
            http:ad(Events.kError, opFail)
    ...
    local notifiTime
    if now() < os.time(todayTime) then
        debug_log('initPushEnergyRequestTask 1')
        notifiTime = os.time(todayTime) + math.random(3600)
    else
        debug_log('initPushEnergyRequestTask 2')
        notifiTime = os.time(tomorrowTime) + math.random(3600)
    end
    MessageCenterHelper:addNotification(notifiTime, LocalNotificationType.kPushEnergy, localize('notification.energy.push.text'))
    config.lastNotificationUid = myUid
    writeConfig(config)

    if now() > os.time(todayTime) then
        debug_log('initPushEnergyRequestTask 3')
        start()
    end
    debug_log('initPushEnergyRequestTask 8')
    

end

function MessageCenterHelper:addNotification(notifiTime, notifiType, text)
    MessageCenterHelper:cancelNotification()
    LocalNotificationManager:getInstance():addNotify(notifiType, notifiTime, 
                text, 
