...
function HorizontalScrollable:create(width, height, useClipping, useBlockingLayers)
...
function HorizontalScrollable:init(width, height, useClipping, useBlockingLayers)
...
if useBlockingLayers then
    ...
    leftBlocking:setTouchEnabled(true, 1, true)
    leftBlocking:ad(DisplayEvents.kTouchTap, function() print('leftBlocking') end)

    rightBlocking:setContentSize(CCSizeMake(500, self.height))
    rightBlocking:setPosition(ccp(0, 0))
    rightBlocking:setTouchEnabled(true, 1, true)
    rightBlocking:ad(DisplayEvents.kTouchTap, function() print('rightBlocking') end)
end
...
function HorizontalScrollable:updateContentViewArea()
    if self.content and self.content.updateViewArea then
        local top = self.xOffset
        local bottom = self.xOffset + self.height
        self.content:updateViewArea(top, bottom)
    end
end

function HorizontalScrollable:onStartMoving()
    -- print('HorizontalScrollable:onStartMoving')
...
function HorizontalScrollable:onEndMoving()
    -- print('HorizontalScrollable:onEndMoving')
...
function HorizontalScrollable:dispose()
    -- print('HorizontalScrollable:dispose()')
    if self.schedId ~= nil then
        -- print('unschedule1')
        Director:sharedDirector():getScheduler():unscheduleScriptEntry(self.schedId)
        self.schedId = nil
...
function HorizontalScrollable:scrollToLeftEnd()
    self:stopSlide()
    self:__moveTo(self.rightMostOffset, 0.3)
end

function HorizontalScrollable:scrollToRightEnd()
    self:stopSlide()
    self:__moveTo(self.leftMostOffset, 0.3)
end
