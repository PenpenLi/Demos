...
SwipeEvents = {kLeftSwipe = 'LEFT_SWIPE', kRightSwipe = 'RIGHT_SWIPE'}
...
if useClipping then
	self.clipping = ClippingNode:create(rect)
else
...
function PagedView:onPageTouchEnd(event)
	...
	-- if user swipes, dispatch a swipe event
		-- print('speed :', self.speedometer:getMeasuredVelocityX())
	...
	elseif hasRightSwipe(self.speedometer, touchMoveDistanceX) 
		...
		if self.xOffset > (self.pageIndex - 1 + 1/2) * self.width  then
			-- print('xOffset = ', self.xOffset, 'next') 
			self:nextPage()
		...
		then
			-- print('xOffset = ', self.xOffset, 'stay') 
...
function PagedView:nextPage()
	...
	if self:canNextPage() then 
		...
		print(self.xOffset, targetOffset)
...
function PagedView:gotoPage(index)
	-- print('PagedView:gotoPage', index)
