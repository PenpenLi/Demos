SummerWeeklyRaceRulePanel = class(BasePanel)

function SummerWeeklyRaceRulePanel:create()
	local panel = SummerWeeklyRaceRulePanel.new()
	panel:init()
	return panel
end

function SummerWeeklyRaceRulePanel:init()
    ...
    rule1:getChildByName("title"):setString(Localization:getInstance():getText("weeklyrace.summer.panel.detail.title1"))
    rule1:getChildByName("text1"):setString(Localization:getInstance():getText("weeklyrace.summer.panel.detail1"))
    rule1:getChildByName("text2"):setString(Localization:getInstance():getText("weeklyrace.summer.panel.detail2"))
    rule1:getChildByName("text3"):setString(Localization:getInstance():getText("weeklyrace.summer.panel.detail3"))
    ...
    rule2:getChildByName("title"):setString(Localization:getInstance():getText("weeklyrace.summer.panel.detail.title2"))
	...
	for i = 1, 6 do
		...
		field:setString(Localization:getInstance():getText("weeklyrace.summer.panel.detail"..tostring(i + 3),
			{num = SummerWeeklyMatchManager:getInstance():getRankMinScore()}))
...
function SummerWeeklyRaceRulePanel:createPager(ui)
	local count, items = 1, {}
	while true do
...
function SummerWeeklyRaceRulePanel:popout()
	PopoutManager:sharedInstance():add(self)
	self.allowBackKeyTap = true
end

function SummerWeeklyRaceRulePanel:onCloseBtnTapped()
