...
SummerWeeklyRaceChampPanel = class(BasePanel)

function SummerWeeklyRaceChampPanel:create()
	local panel = SummerWeeklyRaceChampPanel.new()
	panel:init()
	return panel
end
...
function SummerWeeklyRaceChampPanel:init()
...
function SummerWeeklyRaceChampPanel:buildUI(ui)
	local shareTitle = ui:getChildByName("shareTitle")
	local title = shareTitle:getChildByName("shareTitle")
	title:setText(Localization:getInstance():getText("weeklyrace.summer.panel.achievement1"))
	local size = title:getContentSize()
	title:setPositionX(-size.width / 2)
end

function SummerWeeklyRaceChampPanel:onBtnTapped()
...
function SummerWeeklyRaceChampPanel:setAnimation(ui)
	local shareTitle = ui:getChildByName("shareTitle")
...
function SummerWeeklyRaceChampPanel:runPaperGroupAction(paperGroupUi)
...
function SummerWeeklyRaceChampPanel:popout()
	PopoutManager:sharedInstance():add(self)
	self.allowBackKeyTap = true
end

function SummerWeeklyRaceChampPanel:onCloseBtnTapped()
	self.allowBackKeyTap = false
	PopoutManager:sharedInstance():remove(self)
end

function SummerWeeklyRaceChampPanel:playAnim()
...
function SummerWeeklyRaceChampPanel:shareMessage(successCallback, failCallback, cancelCallback)
	local function onSuccess(isAddCount)
