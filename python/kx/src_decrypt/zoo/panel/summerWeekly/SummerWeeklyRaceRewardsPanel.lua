...
SummerWeeklyRaceRewardsPanel = class(BasePanel)

function SummerWeeklyRaceRewardsPanel:ctor()

end

function SummerWeeklyRaceRewardsPanel:create(rewards, levelId)
	local panel = SummerWeeklyRaceRewardsPanel.new()
	panel:init(rewards, levelId)
	return panel
end

function SummerWeeklyRaceRewardsPanel:init(rewards, levelId)
	assert(type(rewards) == "table")
	assert(#rewards > 0)
	self.levelId = levelId
	self:loadRequiredResource("ui/panel_summer_weekly.json")
	local ui = self:buildInterfaceGroup('summerWeeklyPanel/lastWeekRewardsPanel')
	...
	desc:setString(Localization:getInstance():getText("weeklyrace.summer.panel.tip6"))
	...
	if type(rewards) == "table" then
		for i, v in ipairs(rewards) do
			...
			bubble:setPosition(ccp(i * rewardBoxSize.width + rewardBoxSize.width / 2 + 10, 0))
		end
...
function SummerWeeklyRaceRewardsPanel:receiveRewards()
	self.okBtn:setEnabled(false)
	local function onSuccess()
		CommonTip:showTip(Localization:getInstance():getText("weeklyrace.summer.panel.tip7"), "positive")
	...
	SummerWeeklyMatchManager:getInstance():receiveLastWeekRewards(self.levelId, onSuccess, onFail)
end

function SummerWeeklyRaceRewardsPanel:buildRewardBubble(reward)
	local rewardId = reward.id
	local itemList = reward.items

	local bubble = self:buildInterfaceGroup('summerWeeklyPanel/rewardsPanel_reward')
	...
	local function onIconTouched(evt)
		...
		tipPanel:setTipString(Localization:getInstance():getText("weeklyrace.summer.panel.tip2"))
