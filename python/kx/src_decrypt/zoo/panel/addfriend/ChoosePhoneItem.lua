...
function ChoosePhoneItem:create(itemData, onTapCallback)
	...
	instance:loadRequiredResource("ui/add_friend_panel.json")
...
function ChoosePhoneItem:init(itemData, onTapCallback)
	...
	self.ui = self.builder:buildGroup("AddFriendPanelRecommendItem")
...
function ChoosePhoneItem:initContent(elem,data)
	...
	local name = HeDisplayUtil:urlDecode(data.name)
