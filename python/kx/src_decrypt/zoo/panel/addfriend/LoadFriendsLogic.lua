...
function LoadFriendsLogic:loadFriendsByPhoneNOs(phoneNumbers, onSuccess, onError, onCancel)
	...
	local function onRequestSuccess(evt)
		...
		print("phoneNumberUids: ", table.tostring(phoneNumberUids))
		...
		if onSuccess then onSuccess(uid2PhoneNumberMap, initProfiles) end
	end
...
function LoadFriendsLogic:loadProfilesByUids(uids, onSuccess, onError)
	local function onRequestSuccess(evt)
		...
		if onSuccess then onSuccess(items4Append) end
	end
...
function LoadFriendsLogic:loadFriendsInContacts(onSuccess, onError, onCancel)
	...
	local function loadFriendSuccess(uid2PhoneNumberMap, profiles)
		...
		--todo: create the items data for display;
		print("uid2PhoneNumberMap: ",table.tostring(uid2PhoneNumberMap))
		for k,v in pairs(profiles) do
			if not FriendManager.getInstance().friends[tostring(v.uid)] then
	...
	local callback = {
		onSuccess = function(result) 
        	...
        	if #phoneNumberList == 0 then
        		...
        		removeLoading()
        		print("phoneNumberList is empty!!!!!!!!!!!!!!")
        		if onCancel then 
        			onCancel() 
	...
	local contactDelegate = require("zoo.panel.addfriend.ContactReader")
