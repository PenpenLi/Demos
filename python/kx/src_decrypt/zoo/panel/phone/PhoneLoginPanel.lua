...
function PhoneLoginPanel:create(phoneLoginInfo)
...
function PhoneLoginPanel:init(phoneLoginInfo)
	...
	linkContract:getChildByName("text"):setString(Localization:getInstance():getText("login.panel.intro.3"))
	linkContract:addEventListener(DisplayEvents.kTouchTap,PreloadingSceneUI.showUserAgreement)
	self.ui:getChildByName("tipConcact"):setString(Localization:getInstance():getText("login.panel.intro.2"))
...
function PhoneLoginPanel:onLoginTapped( ... )
	...
	local method = nil
	local function popoutVerifyPhoneConfirmPanel()
		local panel = VerifyPhoneConfirmPanel:create(self.phoneLoginInfo,self.phoneNumber,method)
	...
	local function onSuccess( data )
		...
		if not isVerifyPhoneNumberMethod then
			local panel = SendCodeConfirmPanel:create(self.phoneNumber,true,self.phoneLoginInfo.mode)
			panel:setOkCallback(function( ... )
				popoutVerifyPhoneConfirmPanel()
			end)
			...
			if data.isNewDevice or not data.isPhoneBound then
				local panel = SendCodeConfirmPanel:create(self.phoneNumber,data.isPhoneBound,self.phoneLoginInfo.mode)
				panel:setOkCallback(function( ... )
					popoutVerifyPhoneConfirmPanel()
				end)
				panel:popout()
