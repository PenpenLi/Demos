...
function PropsView:playHammerAnimation(boardView, position)
	...
	local hammer, ring = PropsView:buildHammerAnimation()
	hammer:setPosition(ccp(position.x + GamePlayConfig_Hammer_Pos.x, position.y + GamePlayConfig_Hammer_Pos.y))
	ring:setPosition(position)

	local function ViewViberation()
		boardView:viberate()
	end
	local vib = CCSequence:createWithTwoActions(CCDelayTime:create(BoardViewAction:getActionTime(GamePlayConfig_Hammer_Viberate_Time)),
		CCCallFunc:create(ViewViberation))
	boardView:runAction(vib)

	if boardView.PlayUIDelegate and boardView.PlayUIDelegate.effectLayer then
		boardView.PlayUIDelegate.effectLayer:addChild(hammer)
		boardView.PlayUIDelegate.effectLayer:addChild(ring)
	else
		boardView:addChild(hammer)
		boardView:addChild(ring)
	end
end
...
function PropsView:buildPropDisableAnimation(itemId, position)
	local hammerIcon = Sprite:createWithSpriteFrameName("Prop_".. itemId .." instance 10000")
...
function PropsView:playLineBrushAnimation(boardView, position, targetpos)
	...
	local wand, comet, star1, star2, star3 = PropsView:buildLineBrushAnimation(boardView, position, targetpos)
	if boardView.PlayUIDelegate and boardView.PlayUIDelegate.effectLayer then
		boardView.PlayUIDelegate.effectLayer:addChild(wand)
		boardView.PlayUIDelegate.effectLayer:addChild(comet)
		boardView.PlayUIDelegate.effectLayer:addChild(star1)
		boardView.PlayUIDelegate.effectLayer:addChild(star2)
		boardView.PlayUIDelegate.effectLayer:addChild(star3)
	else
		boardView:addChild(wand)
		boardView:addChild(comet)
		boardView:addChild(star1)
		boardView:addChild(star2)
		boardView:addChild(star3)
	end
end
