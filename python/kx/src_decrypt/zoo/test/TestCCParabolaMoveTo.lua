...
function TestScene:init(...)
	...
	local function testClipping()
		local spBigger = LayerColor:create()
		spBigger:setColor(ccc3(255, 0, 0))
		spBigger:setContentSize(CCSizeMake(500,500))
		self:addChild(spBigger)

		local sp = LayerColor:create()
		sp:setColor(ccc3(0, 0, 255))
		sp:setContentSize(CCSizeMake(150,150))

		local clipping = SimpleClippingNode:create()
		clipping:setContentSize(CCSizeMake(200,200))
		spBigger:addChild(clipping)
		clipping:doRecalcPosition()
		clipping:setRecalcPosition(true)
		clipping:addChild(sp)


		local action = CCMoveTo:create(5, ccp(800, 0))
		spBigger:runAction(action)
