...
function YYBTMSelfUpdateManager:getInstance( ... )
	if not manager then
		...
		print("RRR  YYBTMSelfUpdateManager:getInstance   javaProxyInstance = " .. tostring(manager.javaProxyInstance))
	...
	yybcallback = luajava.createProxy("com.happyelements.animal.yybtmselfupdatesdk.IStartSelfUpdateCallback",
            {
                onDownloadAppProgressChanged = function(receiveDataLen,totalDataLen)
                     print("RRR  onDownloadAppProgressChanged  " .. tostring(receiveDataLen) .. "/" .. tostring(totalDataLen))
                     local yybText = 
                     "已下载   " 
                ...
                onDownloadAppStateChanged = function(state,errorCode,errorMsg)
                    print("RRR  onDownloadAppStateChanged state = " .. tostring(state))

                    if state == 1 then
                        local winSize = CCDirector:sharedDirector():getVisibleSize()
                        local origin = CCDirector:sharedDirector():getVisibleOrigin()

                        print("RRR  winSize  width = " .. tostring(winSize.width) .. "  height = " .. tostring(winSize.height) .. "   x = " .. tostring(winSize.x) .. "   y = " .. tostring(winSize.y))
                        print("RRR  origin  width = " .. tostring(origin.width) .. "  height = " .. tostring(origin.height) .. "   x = " .. tostring(origin.x) .. "   y = " .. tostring(origin.y))
                ...
                onUpdateFailed = function(errorId)
                	print("RRR   YYB Update Fin   id = " .. tostring(errorId))
                    if YYBTMSelfUpdateManager:getInstance().isSkiped == false then
                        preloadingscene:loadAnnouncement()
                        preloadingscene.progressBar:setVisible(true)
                        preloadingscene:doLoadResource()
