...
function EggsManager:initData( ... )
	...
	print("eggsData.show",eggsData.show[1],eggsData.show[2])
	-- debug.debug()
end
...
function EggsManager:canShow( animalType )
	if _G.kUserLogin then
		...
		if MaintenanceManager:getInstance():isEnabled("ChildrensDayUI") then
			return table.exist(eggsData.show,animalType)
			-- return true
		end
	else
		return false
