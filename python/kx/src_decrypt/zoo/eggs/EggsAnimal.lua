...
function EggsAnimal:onCicked( ... )
	...
	local function onSuccess( evt )
		...
		if not reward then
			...
			self.tipLabel:setString(self:getRandomTipText())
			self:playTipAnimation()
		else
			DcUtil:UserTrack({ 
				category="activity",
				sub_category="children_day_click_animal",
			...
			UserManager:getInstance():addRewards({reward})
			UserService:getInstance():addRewards({reward})


			if self.isDisposed then
				if EggsManager:hasAllClicked() then
					AchievementManager:onDataUpdate(AchievementManager.COLLECT_ALL_61_EGGS,true)
				end
				return
			end
			...
			self.tipLabel:setString(Localization:getInstance():getText("children.day.click.animal",{n="\n"}))
			self:playTipAnimation(function( ... )
				if EggsManager:hasAllClicked() then
					AchievementManager:onDataUpdate(AchievementManager.COLLECT_ALL_61_EGGS,true)
				end
			end)
    ...
    local function onFail(evt)
		...
		self.tipLabel:setString(Localization:getInstance():getText("children.day.click.offline",{n="\n"}))
		self:playTipAnimation()
	end
...
function EggsAnimal:playTipAnimation( cb )
	if self.isDisposed then
		return
	end
	...
	if self.animalType == EggsAnimalType.kChicken then
		...
		self.animal:runAction(CCRepeatForever:create(CCSequence:createWithTwoActions(
			CCCallFunc:create(function( ... )
				if self.state == "playNormalEnd" then
					play()
				end
			end),
