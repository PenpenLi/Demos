...
function ErrorLogScene:init(crashLogPath)
  ...
  local msg = ""
  if crashLogPath then
    local file = io.open(crashLogPath, "r")
    if file then
      msg = file:read("*a") 
      file:close()
    else
      msg = "Failed to read crash log from file:"..crashLogPath
    end
  else
    msg = "Crash log file path needed!"
  end
...
function ErrorLogScene:create(crashLogPath)
