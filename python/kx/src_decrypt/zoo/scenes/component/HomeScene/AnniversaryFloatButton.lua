...
local function readLocalActivityData(uid)
	...
	if compareDate(os.date("*t", Localhost:timeInSec()), os.date("*t", data.lastGetDailyRewardTime)) ~= 0 then
		-- print(">>>>>>>>>>>>>>>>>>>>> diff day reset data ~")
		data.getDailyRewardNum = 0
		saveLocalActivityData(uid, data)
	end
	-- print(">>>>>>>>>>>>>>>>>>>>>>>readLocalActivityData:", table.tostring(data))
	return data
end
...
local function playRewardsFlyAnim(rewards, pos, activityIconPos, callback)
	...
	for _,itemConfig in pairs(rewards) do
		...
		function config.finishCallback( ... )
			...
			-- if c >= #rewards and StartupConfig:getInstance():getPlatformName() ~= "mitalk" then 
		...
		elseif itemConfig.itemId == ItemType.XMAS_BELL then 
			config.propId = itemConfig.itemId
			config.createSprite = function()
				local sprite = ResourceManager:sharedInstance():buildItemSprite(config.propId)
				-- sprite:setAnchorPoint(ccp(0, 1))
				return sprite
			end
			if activityIconPos then
				config.toPos = activityIconPos
				config.toSize = CCSizeMake(90, 90)
				anim = HomeSceneFlyToAnimation:sharedInstance():playItemFlyToAnimation(config)
			end
		else
			config.propId = itemConfig.itemId
...
function AnniversaryFloatButton:initData()
	...
	local function updateTopOneInfo(evt)
		...
		if not self.isDisposed and evt and evt.data then
			...
			-- print(">>>>>>>>>>>>>>>>>>>updateTopOneInfo", rankType, table.tostring(rankData))
