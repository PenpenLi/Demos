...
function Trunks:buildTrunk(...)
	...
	-- ----------------------------------
	local intermediateSprite = Sprite:createWithSpriteFrameName("intermediate.png")
	assert(intermediateSprite)
	local bottomAvailabelY = branchRootPosY + branchRootHeight
  
  if __WP8 then 
    bottomAvailabelY = bottomAvailabelY - 1
    intermediateSprite:getTexture():setAliasTexParameters()
  end
    bottomAvailabelY = bottomAvailabelY - 1

	intermediateSprite:setAnchorPoint(ccp(0,0))
	...
	bottomAvailabelY = bottomAvailabelY + size.height
	bottomAvailabelY = bottomAvailabelY - 50
	...
	---- Re Order To Top
	intermediateSprite:removeFromParentAndCleanup(false)
	self:addChild(intermediateSprite)
end
