...
function Fruit:_pick()
	...
	if FruitModel:sharedInstance():askEnergyPick(self.id) then
		local text = {
			tip = Localization:getInstance():getText("fruit.tree.scene.pick.energy.tip"),
			yes = Localization:getInstance():getText("fruit.tree.scene.pick.energy.yes"),
			no = Localization:getInstance():getText("fruit.tree.scene.pick.energy.no"),
		}
		CommonTipWithBtn:showTip(text, "positive", pick)
	else pick() end
end
...
function Fruit:_speed()
	local function onPropNotEnough()
		...
		local text = {
			tip = Localization:getInstance():getText("fruit.tree.scene.no.speed.tip", {number = count}),
			yes = Localization:getInstance():getText("fruit.tree.scene.no.speed.yes"),
...
function FruitLogic:pick(id, successCallback, failCallback)
	...
	local function onSuccess(evt)
		if evt.data and evt.data.reward then
			FruitModel:sharedInstance():incPickCount()
			local manager = UserManager:getInstance()
			local user = manager:getUserRef()
			local reward = evt.data.reward
			if type(reward) == "table" then
				if reward.itemId == 2 then
					local coin = user:getCoin()
					coin = coin + reward.num
					user:setCoin(coin)
				elseif reward.itemId == 4 then
					manager:refreshEnergy()
					local _, __, maxEnergy = manager:refreshEnergy()
					local energy = user:getEnergy()
					energy = energy + reward.num
					if energy > maxEnergy then energy = maxEnergy end
					user:setEnergy(energy)
				elseif reward.itemId == 14 then
					local gold = user:getCash()
					gold = gold + reward.num
					user:setCash(gold)
				else
					manager:addUserPropNumber(reward.itemId, reward.num)
				end
			end
		end
	...
	if FruitModel:sharedInstance():canPick() then
...
function FruitModel:getFruitName(id)
	...
	if data.level == 0 then return "fruit/fruit0"
	else
		local fType = data.type
		if fType == kFruitType.kCoin then return "fruit/fruit"..tostring(data.level)..'s'
		elseif fType == kFruitType.kEnergy then return "fruit/fruit"..tostring(data.level)..'e'
		elseif fType == kFruitType.kGold then return "fruit/fruit"..tostring(data.level)..'g' end
	end
...
function FruitModel:getMethodVisibility(id)
	...
	local data = self.data[id]
	local regen, pick, speed = true, self:canPick(), true
...
function FruitModel:incPickCount()
...
function FruitModel:askEnergyPick(id)
	if not self.data[id] then return false end
	local data = self.data[id]
	if data.level <= 0 or data.type ~= kFruitType.kEnergy then return false end
	return UserEnergyRecoverManager:sharedInstance():isEnergyFull()
end
...
function FruitModel:getFruitPickCount()
	...
	return self.upgrade[level].pickCount - dailyData.pickFruitCount
end
