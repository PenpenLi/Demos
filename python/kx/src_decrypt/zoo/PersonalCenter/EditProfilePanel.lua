...
function EditProfilePanel:selectTableView(selectType)
    ...
    scrollable.bg = bg
    local normalTextColor = ccc3(255, 186, 72)
    ...
    local function buildItem( index )
        ...
        if selectType == SelectType.CONS then
            ...
            local t = TextField:create(str, nil, 40, nil, hAlignment, kCCVerticalTextAlignmentCenter)
            t:setColor(normalTextColor)
            t:setPosition(ccp(ITEM_WIDTH / 2 - 100, ITEM_HEIGHT / 2))
            ...
            data:setPosition(ccp(ITEM_WIDTH / 2 + 90, ITEM_HEIGHT / 2))
            ...
            content:ignoreAnchorPointForPosition(false)
            content:setAnchorPoint(ccp(0,1))
            local text = TextField:create(str, nil, 40, nil, hAlignment, kCCVerticalTextAlignmentCenter)
            text:setColor(normalTextColor)
            text:setPosition(ccp(ITEM_WIDTH / 2, ITEM_HEIGHT / 2))
...
function EditProfilePanel:setData( selectType, index )
    print("type : "..selectType.."index : "..index)
...
function EditProfilePanel:removeSelf()
    PopoutManager:sharedInstance():remove(self, true)
    self.allowBackKeyTap = false

    if self.parentPanel then
        self.parentPanel:updateProfile()
    end
end
