...
function Processor:start(loginType)
    ...
    if not uid or type(uid) ~= 'number' or (uid and type(uid) == 'number' and Cookie.getInstance():read("loginInfo" .. uid)) then
		self:dispatchEvent(Event.new(Processor.events.kFail, nil, self))
		return
	else
...
function Processor:getAnotherServerInfo()
	if Cookie.getInstance():read("loginAnotherServer") then
	...
	local function onResponse(response)
        Cookie.getInstance():write("loginAnotherServer", "1")
