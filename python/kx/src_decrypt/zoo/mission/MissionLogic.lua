...
function MissionLogic:getReward(indices, successCallback, failCallback, cancelCallback)
	local function onSuccess(evt)
		-- 更新用户数据
		local function addItems(itemId, num)
			if itemId == 2 then
				local user = UserManager:getInstance():getUserRef()
				local oNum = user:getCoin()
				user:setCoin(user:getCoin() + num)
				user = UserService:getInstance():getUserRef()
				user:setCoin(user:getCoin() + num)
				DcUtil:logCreateCoin("reward_job", num, oNum, -1)
			elseif itemId == 14 then
				local user = UserManager:getInstance():getUserRef()
				local oNum = user:getCash()
				user:setCash(user:getCash() + num)
				user = UserService:getInstance():getUserRef()
				user:setCash(user:getCash() + num)
				DcUtil:logCreateCash("reward_job", num, oNum, -1)
...
function MissionLogic:getMissionData(position)
	...
	if mission then
		...
		if CMFData then
			...
			--print("RRR   WWWWWWWWWWWWWWWWWWW  getMissionData by CMFData " , table.tostring(data) )
		end
	end
