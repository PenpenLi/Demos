...
function MissionPanelLogic:tryToUpdateMissionButton()
	print("RRR  MissionPanelLogic:tryToUpdateMissionButton  111")
	...
	print("RRR  MissionPanelLogic:tryToUpdateMissionButton  222")
	if self.missionBtn then
		missionData = MissionLogic:getInstance():getMissionData(4)
		print("RRR  MissionPanelLogic:tryToUpdateMissionButton  333")
		if missionData then
			print("RRR  MissionPanelLogic:tryToUpdateMissionButton  444")
	...
	if self:checkAllMissionRewarded(false) then
		print("RRR  MissionPanelLogic:tryToUpdateMissionButton  555")
		if self.missionBtn then
			print("RRR  MissionPanelLogic:tryToUpdateMissionButton  666")
			...
			function () 
				--print(table.tostring( HomeSceneButtonsManager.getInstance():getBtnTypeInfoTable() ))
				HomeSceneButtonsManager.getInstance():setButtonShowPosState(HomeSceneButtonType.kMission, true)
			end )

		else
			print("RRR  MissionPanelLogic:tryToUpdateMissionButton  777")
			HomeSceneButtonsManager.getInstance():setButtonShowPosState(HomeSceneButtonType.kMission, true)
		end

	else
		print("RRR  MissionPanelLogic:tryToUpdateMissionButton  888")
		HomeSceneButtonsManager.getInstance():setButtonShowPosState(HomeSceneButtonType.kMission, false)
		if not self.missionBtn then
			print("RRR  MissionPanelLogic:tryToUpdateMissionButton  999")
...
function MissionPanelLogic:openPanel(closeCallback)
	...
	--print("WTF FFFFFFFFFFFFFFFFFF     MissionPanelLogic:openPanel     " , waterLevel , MissionLogic:getInstance().countTotal , MissionLogic:getInstance().countCurrent  )
...
function MissionPanelLogic:onMissionReward(bubblePosition)
	print("RRR  MissionPanelLogic:onMissionReward  " , bubblePosition)
...
function MissionPanelLogic:onDoMission(bubblePosition)
	print("RRR  MissionPanelLogic:onDoMission  " , bubblePosition)
	...
	if missionData.doActions and #missionData.doActions > 0 then
		--print("RRR  TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT " , table.tostring( missionData.doActions ) )
...
function MissionPanelLogic:onUpdateMission(positionIndex , oldMissionState)
	print("RRR   MissionPanelLogic:onUpdateMission   " , positionIndex , oldMissionState )
...
function MissionPanelLogic:onMissionProgressChanged(data)
	...
	print("RRR   MissionPanelLogic:onMissionProgressChanged   " , data.position , data.current , data.total )
...
function MissionPanelLogic:onMissionRewardEvent(positionIndex , newBottleReward , bottleCurr , bottleTotal )
	print("RRR   MissionPanelLogic:onMissionRewardEvent   " , positionIndex , newBottleReward , bottleCurr , bottleTotal )
	if self.panel then
		local dataInfo = self:createMissionDataByMissionLogicData(positionIndex)
...
function MissionPanelLogic:onTankProgress(waterLevel)
	print("RRR   MissionPanelLogic:onTankProgress   " , waterLevel )
	if self.panel then
