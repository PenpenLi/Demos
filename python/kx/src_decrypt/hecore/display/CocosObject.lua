...
function CocosObject:dispose()
  --print("dispose", self:toString());
    ...
    if self.refCocosObj and not self.isRefCocosObjReleased then
        ...
        if self.debugTag == 1 then print("dispose" .. self:toString()) end
    end
...
function CocosObject:getPosition() 
	--if self.isDisposed then error("CocosObject disposed!!!") end
...
function CocosObject:stopActionByTag(v) self.refCocosObj:stopActionByTag(v) end
...
function CocosObject:hitTestPoint(worldPosition, useGroupTest)
    ...
    --print("self.refCocosObj: " .. tostring(self.refCocosObj))
    --print("worldPosition: " .. tostring(worldPosition))

    --if not self.refCocosObj then
    --        print("!!!!!!!!!!! self.refCocosObj == nil !!!!!!!!!")
...
function CocosObject:debugPrintAllChildren()
	...
	print("================ Debug Output All Children =================")
	print(tostring(self.name) .. " 's All Child:")

	for i,v in pairs(self.list) do

		print("i: "..i .. " , " .. "name: " .. (tostring(v.name) or "NO NAME"))
	end
end
