...
local lastfile
local watchescnt = 0
local abort = nil -- default value is nil; this is used in start/loop distinction
...
local function set_breakpoint(file, line)
    if g_UseCHookFilter then
       bp_wapper:set_breakpoint(file, line)
    end
...
local function remove_breakpoint(file, line)
  if g_UseCHookFilter then
     bp_wapper:del_breakpoint(file, line)
  end
  if file == '-' and lastfile then file = lastfile end
...
function debug_hook(event, line)
  ...
  elseif event == "line" then
    ...
    --print((lastfile or "nil") .. " " .. (lastsource or "nil") .. " " .. caller.source .. " " .. event .. " " .. line)
    ...
    file = string.gsub(file, '^'..q(basedir), '')
    ...
    if getin then
      ...
      step_into = false
      step_over = false
      --print "getin is ture resume called!"
    ...
    if status and res and res ~= 'stack' then
      if abort == nil and res == "exit" then
        print("******* exit *******")
        lua_stub_exit(0)
        print("******* exit *******")
        return
...
local function debugger_loop(sfile, sline)
  ...
  while true do
    ...
    elseif command == "BASEDIR" then
      ...
      if dir then
        --basedir = --dir
        --"/Users/xul/Library/Application Support/iPhone Simulator/5.1/Applications/38AE2CBF-078A-4EA4-9455-B0AE4E9804F8/Documents/Assets/"
        server:send("200 OK\n")
      else
        server:send("400 Bad Request\n")
      end
...
local function start(controller_host, controller_port)
  ...
  if server then
    ...
    do
      ...
      debug.traceback = function (err) local r = dtraceback(err, 3) genv.print(r) return r end
    end
    coro_debugger = coroutine.create(debugger_loop)
    --hedebug.sethook(debug_hook, "lcr")
    hedebug.sethook("debug_hook")
    --return coroutine.resume(coro_debugger, file, info.currentline)
    return coroutine.resume(coro_debugger, "launcher.lua", 1)
  else
...
local function controller(controller_host, controller_port)
  ...
  if server then
    ...
    while true do
      ...
      --hedebug.sethook(coro_debugee, debug_hook, "lcr")
      hedebug.sethook("debug_hook")
...
local function on()
  ...
  if co then
    if not coroutines[co] then
      coroutines[co] = true
      --hedebug.sethook(co, debug_hook, "lcr")
      hedebug.sethook("debug_hook")
    end
  else
    --hedebug.sethook(debug_hook, "lcr")
...
local function setBaseDir( _basedir )
	basedir = _basedir
   bp_wapper:setBaseDir(_basedir)
end

local function _logError(errorString)
  server:send("401 Error in Execution " .. #errorString .. "\n")
  server:send(errorString)
  print("\n401 Error in Execution " .. errorString .. "\n")
end

local function _logWarning(errorString)
  local outputString = "WARNING:" .. errorString .. "\n"
  server:send("204 Output " .. "stdout" .. " " .. #outputString .. "\n" .. outputString)
  print(outputString)
end

local function _log(infoString)
  local outputString = "INFO:" .. infoString .. "\n"
  server:send("204 Output " .. "stdout" .. " " .. #outputString .. "\n" .. outputString)
  print(infoString)
end

local function _send_log(str)
  if server then
    server:send("204 Output " .. "stdout" .. " " .. #str .. "\n" .. str)
  end
end

-- make public functions available
mobdebug.basedir = setBaseDir
mobdebug.logError = _logError
mobdebug.logWarning = _logWarning
mobdebug.log = _log
mobdebug.send_log = _send_log
