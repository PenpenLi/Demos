...
function LayoutBuilder:createWithContentsOfFile(filePath)
  ...
  if not config then
    ...
    if not file then
      print("LayoutBuilder fail, file not exist: "..filePath)
      assert(false)
      return;
    end
...
function LayoutBuilder:buildBatchGroup_version1(batchMode, groupName, imageSuffix, ...)
	assert(batchMode == "batch" or batchMode == "sprite")
	assert(type(groupName) == "string")
	assert(#{...} == 0)
	...
	for k,symbol in ipairs(group) do
		if symbol.type == kGroupLayoutType.kImage then
			...
			if type(symbol.scalingGrid) == "boolean" and symbol.scalingGrid then
				...
				image = Sprite:createWithSpriteFrameName(symbol.image .. imageSuffix)
				assert(image)
...
function LayoutBuilder:buildBatchGroup_version2(batchMode, groupName, imageSuffix, ...)
	assert(batchMode == "batch" or batchMode == "sprite")
	assert(type(groupName) == "string")
	assert(#{...} == 0)
	...
	for k,symbol in ipairs(group) do
		...
		elseif symbol.type == kGroupLayoutType.kGroup then
			...
			local layout = self:buildBatchGroup_version2("sprite", symbol.image, imageSuffix)
			if layout then
				setBasicTransformation(layout, symbol, batchNode)
			end
		end
...
function LayoutBuilder:build_version2(groupName, imageSuffix, cascade, ...)
	assert(#{...} == 0)
	...
	for k, symbol in ipairs(group) do
		...
		if symbol.type == kGroupLayoutType.kImage then
			...
			if type(symbol.scalingGrid) == "boolean" and symbol.scalingGrid then
				...
				image = assert(Sprite:createWithSpriteFrameName(symbol.image..imageSuffix))
		...
		elseif symbol.type == kGroupLayoutType.kGroup then
			local layout = self:build_version2(symbol.image, imageSuffix, cascadeEnabled)
...
function LayoutBuilder:buildWithCustomeProperty(groupName, imageSuffix, customPropertyFunc, ...)
	...
	--print("build group name: " .. groupName)
...
function LayoutBuilder:build_version1(groupName, imageSuffix)
  ...
  for k, symbol in ipairs(group) do
    ...
    elseif symbol.type == kGroupLayoutType.kGroup then
      local layout = self:build_version1(symbol.image, imageSuffix)
      if layout then
        setBasicTransformation(layout, symbol, groupLayer)
        addDebugBounds(layout, groupLayer)
      end
...
function addGlobalDynamicFontMap(designedFont, bitMapFontFile, ...)
