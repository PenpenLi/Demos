   ...
   public class ImageLoader extends FeathersControl
   {
      ...
      protected function autoSizeIfNeeded() : Boolean
      {
         ...
         if(_loc2_)
         {
            if(this._currentTextureWidth == this._currentTextureWidth)
            {
               _loc3_ = this._currentTextureWidth * this._textureScale;
               if(!_loc4_)
               {
                  _loc6_ = this.explicitHeight / (this._currentTextureHeight * this._textureScale);
         ...
         if(_loc4_)
         {
            if(this._currentTextureHeight == this._currentTextureHeight)
            {
               _loc1_ = this._currentTextureHeight * this._textureScale;
               if(!_loc2_)
               {
                  _loc5_ = this.explicitWidth / (this._currentTextureWidth * this._textureScale);
      ...
      {
         var _loc2_:Texture = this._texture;
         if(!_loc2_)
         {
            if(this.loader)
            {
               _loc2_ = this._loadingTexture;
            }
            else
            {
               _loc2_ = this._errorTexture;
            }
         }
         if(this._currentTexture == _loc2_)
         {
            return;
         }
         this._currentTexture = _loc2_;
         ...
         var _loc1_:Rectangle = this._currentTexture.frame;
         if(_loc1_)
         {
            this._currentTextureWidth = _loc1_.width;
            this._currentTextureHeight = _loc1_.height;
         }
         else
         {
      ...
      protected function removeFromTextureQueue() : void
      {
         ...
         var _loc2_:feathers.controls.ImageLoader = this._textureQueuePrevious;
         var _loc3_:feathers.controls.ImageLoader = this._textureQueueNext;
         ...
         if(_loc2_)
         {
            _loc2_._textureQueueNext = _loc3_;
         }
         if(_loc3_)
         {
            _loc3_._textureQueuePrevious = _loc2_;
         }
         var _loc1_:* = textureQueueHead == this;
         ...
         if(_loc1_)
         {
            textureQueueHead = _loc3_;
            if(_loc4_)
            {
               textureQueueTail = _loc3_;
            }
         }
         if(_loc1_ && textureQueueHead)
         {
