package lzm.starling.gestures
{
   ...
   public class ZoomGestures extends Gestures
   {
      ...
      public function ZoomGestures(param1:DisplayObject, param2:Function = null)
      {
         super(param1,param2);
      }
      ...
      override public function checkGesturesByTouches(param1:Vector.<Touch>) : void
      {
         ...
         var _loc15_:Number = _loc9_.length / _loc6_.length;
         _target.scaleX = _target.scaleX * _loc15_;
         _target.scaleY = _target.scaleY * _loc15_;
         if(!isNaN(minScale))
         {
            if(_target.scaleX < minScale)
            {
               _target.scaleX = minScale;
            }
            if(_target.scaleY < minScale)
            {
               _target.scaleY = minScale;
            }
         }
         if(!isNaN(maxScale))
         {
            if(_target.scaleX > maxScale)
            {
               _target.scaleX = maxScale;
            }
            if(_target.scaleY > maxScale)
            {
               _target.scaleY = maxScale;
            }
         }
      }
   }
